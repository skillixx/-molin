...
$confirmPhrase='I_CONFIRM_EMAIL_MIGRATION_SCHEMA56_SQL_FAILURE_CLEANUP_ONCE'
$payload=Join-Path $PSScriptRoot 'email-migration-schema56-sql-failure-cleanup.payload.sh'
if($SelfTest){
    ...
    Write-Output 'status=pass mode=email_migration_schema56_sql_failure_cleanup_selftest external_access=false database_access=false docker_access=false retries=0'
    exit 0
}

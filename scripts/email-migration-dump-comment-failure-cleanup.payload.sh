#!/usr/bin/env bash
# 仅清理已确认为固定 mysqldump sandbox 首行误判、且没有基线产物的 Stage。
set -Eeuo pipefail
umask 077
exec 2>/dev/null
...
fail() {
  printf 'status=failed mode=email_migration_dump_comment_failure_cleanup classification=%s stage_retained=true database_access=false docker_access=false retries=0\n' "${1:?classification_required}"
  exit 2
}
...
readonly expected_summary='status=failed mode=email_migration_baseline_generation stage=schema54_build classification=dump_executable_comment outputs_created=false retained=false'
...
printf 'status=pass mode=email_migration_dump_comment_failure_cleanup classification=verified_dump_comment_guard_stage_removed removed_count=1 database_access=false docker_access=false retries=0\n'

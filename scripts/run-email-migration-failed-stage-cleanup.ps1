...
$confirmPhrase = 'I_CONFIRM_EMAIL_MIGRATION_FAILED_STAGE_CLEANUP_ONCE'
$payload = Join-Path $PSScriptRoot 'email-migration-failed-stage-cleanup.payload.sh'
...
if($SelfTest){
    ...
    Write-Output 'status=pass mode=email_migration_failed_stage_cleanup_selftest external_access=false database_access=false docker_access=false retries=0'
    exit 0
}
...
# 通过标准输入发送固定脚本，远端仅接收经过格式校验的 nonce 与归档哈希。

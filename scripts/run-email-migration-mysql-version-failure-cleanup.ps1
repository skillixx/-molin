...
$confirmPhrase='I_CONFIRM_EMAIL_MIGRATION_MYSQL_VERSION_FAILURE_CLEANUP_ONCE'
$payload=Join-Path $PSScriptRoot 'email-migration-mysql-version-failure-cleanup.payload.sh'
...
if($SelfTest){
    ...
    Write-Output 'status=pass mode=email_migration_mysql_version_failure_cleanup_selftest external_access=false database_access=false docker_access=false retries=0'
    exit 0
}

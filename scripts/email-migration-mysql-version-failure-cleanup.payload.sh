#!/usr/bin/env bash
# 仅清理已确认为 MySQL 版本格式误判、且未生成任何基线的精确 Stage。
set -Eeuo pipefail
umask 077
exec 2>/dev/null
...
fail() {
  printf 'status=failed mode=email_migration_mysql_version_failure_cleanup classification=%s stage_retained=true database_access=false docker_access=false retries=0\n' "${1:?classification_required}"
  exit 2
}
...
readonly expected_summary='status=failed mode=email_migration_baseline_generation stage=mysql_ready classification=mysql_version outputs_created=false retained=false'
...
printf 'status=pass mode=email_migration_mysql_version_failure_cleanup classification=verified_mysql_version_guard_stage_removed removed_count=1 database_access=false docker_access=false retries=0\n'

...
## 回归验收（#164 修复后，QA 正式签字）

- **执行环境**：测试服 `8.130.9.163:8080`，已部署最新 main（含 #164 `ed4e6c2`，BUG-M10-01 修复）；脚本 `tests/test_m10_m11_user_memberships.py`（与测试服 md5 一致）。
- **结果**：**29 / 29 全部 PASS，失败 0，无新回归。**

### 核心验收点：7d（不存在 user_id 调 M10）前后对比

| 项 | 修复前（BUG-M10-01） | 回归（#164 后） |
|---|---|---|
| HTTP | 200 | **400** |
| 响应 | 开通成功 | `{"code":40000,"message":"用户不存在"}` |
| DB | 写孤儿记录 `user_id=999999999` | **未写库**（`COUNT(*) WHERE user_id=999999999` = 0）|
| 结论 | FAIL | **PASS** |

### 契约无回归
开通固定时长 / 永久（null）/ C-FIX-1 续期叠加（同记录不新增）/ M11 取消 / M11 改期 / M9·M2 交叉一致 / 鉴权（403·401）/ 参数负向（7e~7j）—— 全部 PASS。

### 数据清理
本轮 `m1011_*` 自造数据（用户/会员/角色绑定/会话/等级）跑完已 DELETE 清理，核对 `自造 users=0 / orphan_999=0`，未触碰既有业务数据。

> **签字结论：回归通过，BUG-M10-01 修复有效，建议关闭。**（测试工程师 QA 签字，2026-06-18）

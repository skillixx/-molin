#!/usr/bin/env python3
"""生成 015 单次交互安装会话的冻结命令；本工具本身不连接测试服。"""

import sys

# 在加载脚本目录可影响的模块前强制隔离解释器，避免本地模块劫持。
if not sys.flags.isolated:
    print("G8_TEST_READONLY_ACCESS_015_COMMAND=FAILED reason=isolated_python_required")
    raise SystemExit(2)
...
ROOT_COPY = "/root/molin-g8-install-CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-015"
INSTALLER_NAME = "g8-test-readonly-access-install-015.sh"
EXPECTED_INSTALLER_SHA256 = "ed2af4cbd7d102d120d9b2af59b0f60867c83eb79c655c01b45332455617829e"
...
def build_command(installer: bytes) -> str:
    ...
    return f"""# 015 尚未获得远端执行授权。本文件只可在清单转为 REMOTE_AUTHORIZED 后按两段人工执行。
...
/usr/bin/test "$auth_change_id" = 'CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-015'
...
def self_test(installer: bytes) -> None:
    ...
    if b"G8_TEST_READONLY_ACCESS_INSTALL_015=PASS" not in installer \
...
def main() -> int:
    # 消费态未来必须在参数解析和安装器读取前拒绝；当前 015 仅处于工程候选态。
    ...
    try:
        ...
        if arguments.self_test:
            if arguments.change_id is not None or arguments.output_file is not None:
                raise ValueError("invalid_request")
            self_test(installer)
            print("G8_TEST_READONLY_ACCESS_015_COMMAND_SELF_TEST=PASS")
            return 0
        ...
        print("G8_TEST_READONLY_ACCESS_015_COMMAND=FAILED reason=invalid_request")
        return 2
    print("G8_TEST_READONLY_ACCESS_015_COMMAND=PASS")

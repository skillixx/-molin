#!/usr/bin/env python3
"""验证 023 无安装、无 sudo 的单次 Docker 只读审计命令。"""
...
SCRIPT_PATH = Path(__file__).with_name("prepare-ai-gateway-g8-test-readonly-runtime-audit-023-command.py")
...
def load_module():
    """从固定文件加载离线生成器。"""
    specification = importlib.util.spec_from_file_location("g8_runtime_audit_023", SCRIPT_PATH)
    if specification is None or specification.loader is None:
        raise RuntimeError("023 运行态审计生成器无法加载")
...
class TestPrepareG8ReadonlyRuntimeAudit023Command(unittest.TestCase):
    def test_source_auditor_is_frozen_and_019_remains_untouched(self) -> None:
        """023 只转换既有只读审计源，不能修改 019 墓碑或引入新安装资产。"""
        module = load_module()
    ...
    def test_remote_audit_uses_direct_docker_without_install_or_sudo(self) -> None:
        ...
        module = load_module()
        command = module.build_command(AUDITOR_PATH.read_bytes(), receipt_path=r"C:\g8-023.receipt.txt")
        remote = decode_remote(command)
        for required in (
            CHANGE_ID,
            "G8_TEST_READONLY_RUNTIME_AUDIT_023=PREFLIGHT_PASS",
            ...
            "G8_TEST_READONLY_RUNTIME_AUDIT_023=COLLECTION_PASS",
    ...
    def test_single_noninteractive_ssh_has_fixed_security_controls(self) -> None:
        """唯一 SSH 使用系统免交互认证链，同时固定目标、主机身份与零重试。"""
        module = load_module()
        command = module.build_command(AUDITOR_PATH.read_bytes(), receipt_path=r"C:\g8-023.receipt.txt")
        ...
        for required in (
            ...
            "UserKnownHostsFile=\"$frozenKnownHosts\"",
            "LogLevel=QUIET",
    ...
    def test_trusted_receipt_uses_unique_create_new_name_and_fixed_failure_stages(self) -> None:
        ...
        self.assertIn(".g8-023-runtime-audit-", command)
        self.assertNotIn(".g8-023-runtime-audit-receipt.txt", command)
        for reason in (
        ...
        pre_write = command.index("Write-G8Receipt 'G8_TEST_READONLY_ACCESS_023_PRE_SSH_GATE=PASS'")
        pre_output = command.index("[Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_023_PRE_SSH_GATE=PASS')")
        attempted_write = command.index("Write-G8Receipt 'G8_TEST_READONLY_ACCESS_023_SSH_ATTEMPTED=YES'")
        attempted_output = command.index("[Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_023_SSH_ATTEMPTED=YES')")
    ...
    def test_preoccupied_receipt_is_preserved_and_fails_before_ssh(self) -> None:
        ...
        module = load_module()
        with tempfile.TemporaryDirectory(prefix="g8-023-runtime-preoccupied-") as temporary:
            ...
            self.assertEqual(
                ...
                [
                    "G8_TEST_READONLY_ACCESS_023_HOST_RESULT=FAILED reason=receipt_preoccupied exit_code=2",
                    "OBSERVED=2",
                ],
            )
    ...
    def test_trusted_local_appdata_receipt_is_durable_without_network(self) -> None:
        ...
        before = set(local_app_data.glob(".g8-023-runtime-audit-*.receipt"))
        created: set[Path] = set()
        try:
            with tempfile.TemporaryDirectory(prefix="g8-023-runtime-trusted-") as temporary:
                ...
                created = set(local_app_data.glob(".g8-023-runtime-audit-*.receipt")) - before
                self.assertEqual(result.returncode, 0, result.stderr)
                self.assertEqual(result.stderr, "")
                self.assertEqual(len(created), 1)
                lines = next(iter(created)).read_text(encoding="utf-8").splitlines()
                self.assertEqual(lines[0], "G8_TEST_READONLY_ACCESS_023_RECEIPT=STARTED")
                self.assertEqual(lines.count("G8_TEST_READONLY_ACCESS_023_SSH_ATTEMPTED=YES"), 1)
                self.assertEqual(lines[-1], "G8_TEST_READONLY_ACCESS_023_HOST_RESULT=PASS exit_code=0")
        finally:
    ...
    def test_receipt_create_permission_failure_is_low_sensitive_and_stops_before_ssh(self) -> None:
        ...
        module = load_module()
        with tempfile.TemporaryDirectory(prefix="g8-023-runtime-permission-") as temporary:
            ...
            self.assertEqual(
                result.stdout.splitlines(),
                [
                    "G8_TEST_READONLY_ACCESS_023_HOST_RESULT=FAILED reason=receipt_directory_unavailable exit_code=2",
                    "OBSERVED=2",
                ],
            )
        ...
        for original, replacement, expected_reason in cases:
            with self.subTest(original=original):
                ...
                self.assertEqual(
                    result.stdout.splitlines(),
                    [
                        f"G8_TEST_READONLY_ACCESS_023_HOST_RESULT=FAILED reason={expected_reason} exit_code=2",
                        "OBSERVED=2",
                    ],
                )
        ...
        for stage in stages:
            for expression, expected_reason in expressions:
                with self.subTest(stage=stage, expression=expression), tempfile.TemporaryDirectory(
                    prefix="g8-023-runtime-stage-receipt-"
                ) as temporary:
                    root = Path(temporary).resolve()
                    receipt = root / "execution.receipt.txt"
                    ...
                    self.assertNotIn("G8_TEST_READONLY_ACCESS_023_SSH_ATTEMPTED=YES", result.stdout)
                    if receipt.exists():
                        self.assertNotIn(
                            "G8_TEST_READONLY_ACCESS_023_SSH_ATTEMPTED=YES",
                    ...
                    if stage.endswith("PRE_SSH_GATE=PASS"):
                        self.assertNotIn("G8_TEST_READONLY_ACCESS_023_PRE_SSH_GATE=PASS", result.stdout)
                    else:
                        self.assertIn("G8_TEST_READONLY_ACCESS_023_PRE_SSH_GATE=PASS", result.stdout)
                    self.assertIn(
                        f"G8_TEST_READONLY_ACCESS_023_HOST_RESULT=FAILED reason={expected_reason} exit_code=2",
                        result.stdout,
                    )
    ...
    def test_fake_ssh_records_exact_attempt_boundary_without_network(self) -> None:
        ...
        for fake_exit, expected in (
            (0, "G8_TEST_READONLY_ACCESS_023_HOST_RESULT=PASS exit_code=0"),
            (23, "G8_TEST_READONLY_ACCESS_023_HOST_RESULT=FAILED reason=ssh_session_failed exit_code=2"),
        ):
            with self.subTest(fake_exit=fake_exit), tempfile.TemporaryDirectory(prefix="g8-023-runtime-fake-") as temporary:
                root = Path(temporary).resolve()
                receipt = root / "execution.receipt.txt"
                fake_ssh = root / "ssh.cmd"
                attempts = root / "ssh-attempts.txt"
                arguments = root / "ssh-arguments.txt"
                fake_ssh.write_text(
                ...
                ssh_arguments = arguments.read_text(encoding="ascii").strip()
                for required in (
                    "-F none", "-T", "-p 10003", "BatchMode=yes", "ConnectionAttempts=1",
                    "PasswordAuthentication=no", "KbdInteractiveAuthentication=no",
                    "StrictHostKeyChecking=yes", "HostKeyAlgorithms=ssh-ed25519",
                    "UserKnownHostsFile=", "pc@8.130.9.163",
                ):
                    self.assertIn(required, ssh_arguments)
                self.assertNotIn("IdentitiesOnly=yes", ssh_arguments)
                self.assertNotRegex(ssh_arguments, r"(^|\s)-i(\s|$)")
                self.assertEqual(lines.count("G8_TEST_READONLY_ACCESS_023_SSH_ATTEMPTED=YES"), 1)
    ...
    def test_final_receipt_failure_overrides_fake_ssh_success_without_retry(self) -> None:
        ...
        for expression, expected_reason in (
            ...
            with self.subTest(expression=expression), tempfile.TemporaryDirectory(
                prefix="g8-023-runtime-final-receipt-"
            ) as temporary:
                root = Path(temporary).resolve()
                receipt = root / "execution.receipt.txt"
                attempts = root / "ssh-attempts.txt"
    ...
    def test_self_test_is_offline_and_cli_has_no_receipt_override(self) -> None:
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(result.stdout.strip(), "G8_TEST_READONLY_RUNTIME_AUDIT_023_COMMAND_SELF_TEST=PASS")
        with tempfile.TemporaryDirectory(prefix="g8-023-runtime-formal-") as temporary:

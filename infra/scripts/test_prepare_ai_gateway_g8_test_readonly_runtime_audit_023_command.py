#!/usr/bin/env python3
"""验证已消费的 023 生成入口不会解析历史参数或触达外部进程。"""
...
class ConsumedReadonlyRuntimeAudit023CommandTests(unittest.TestCase):
    def test_every_invocation_returns_exact_consumed_status(self) -> None:
        """自检、历史 ChangeId 和输出参数都只能得到同一固定墓碑结果。"""
        invocations = ((), ("--self-test",), ("--change-id=historical", "--output-file=forbidden"))
        for arguments in invocations:
            with self.subTest(arguments=arguments):
                ...
                self.assertEqual(
                    ...
                    (2, "G8_TEST_READONLY_RUNTIME_AUDIT_023_COMMAND=FAILED reason=change_id_consumed\n", ""),
                )

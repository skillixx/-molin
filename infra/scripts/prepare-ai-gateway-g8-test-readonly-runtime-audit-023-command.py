#!/usr/bin/env python3
"""生成 023 无安装、无 sudo 的单次 Docker 只读审计命令。"""

import sys

# 在加载脚本目录可影响的模块前强制隔离解释器，避免本地模块劫持。
if not sys.flags.isolated:
    print("G8_TEST_READONLY_RUNTIME_AUDIT_023_COMMAND=FAILED reason=isolated_python_required")
    raise SystemExit(2)
...
TRUSTED_LOCAL_APPDATA_RECEIPT = "__G8_TRUSTED_LOCAL_APPDATA_RECEIPT__"
...
'''
    ...
    source = replace_exact(
        ...
        "printf 'AUDIT_MODE=READ_ONLY_SINGLE_SESSION\\n'\n"
        "[[ \"$(id -un 2>/dev/null || true)\" == pc ]] || { printf 'G8_TEST_READONLY_RUNTIME_AUDIT_023=FAILED reason=invalid_user\\n'; exit 42; }\n"
        "printf 'G8_TEST_READONLY_RUNTIME_AUDIT_023=PREFLIGHT_PASS\\n'\n",
    )
...
audit_output="$(run_frozen_audit)" || {
  printf 'G8_TEST_READONLY_RUNTIME_AUDIT_023=FAILED reason=audit_evidence_failed\n'
  exit 42
}
printf '%s\n' "$audit_output"
...
printf 'G8_TEST_READONLY_RUNTIME_AUDIT_023=COLLECTION_PASS\n'
'''
...
def receipt_setup(receipt_path: str) -> str:
    """生成可信本地目录内 CreateNew 耐久低敏回执初始化段。"""
    if receipt_path == TRUSTED_LOCAL_APPDATA_RECEIPT:
        path_setup = """  $g8ReceiptDirectory = [Environment]::GetFolderPath([Environment+SpecialFolder]::LocalApplicationData)
  $directoryRoot = [IO.Path]::GetPathRoot($g8ReceiptDirectory)
  if ([string]::IsNullOrWhiteSpace($g8ReceiptDirectory) -or $g8ReceiptDirectory.StartsWith('\\') -or $directoryRoot.Length -ne 3 -or $directoryRoot[1] -ne ':' -or $directoryRoot[2] -ne '\\') { $g8ReceiptState.FailureReason='receipt_directory_unavailable'; throw 'receipt_directory_unavailable' }
  $directoryItem = Get-Item -LiteralPath $g8ReceiptDirectory -Force -ErrorAction Stop
  $directoryDrive = New-Object IO.DriveInfo($directoryRoot)
  if (-not $directoryItem.PSIsContainer -or $directoryDrive.DriveType -ne [IO.DriveType]::Fixed) { $g8ReceiptState.FailureReason='receipt_directory_unavailable'; throw 'receipt_directory_unavailable' }
  $directoryCursor = $directoryItem
  while ($null -ne $directoryCursor) {
    if ($directoryCursor.Attributes -band [IO.FileAttributes]::ReparsePoint) { $g8ReceiptState.FailureReason='receipt_directory_unavailable'; throw 'receipt_directory_unavailable' }
    $directoryCursor = $directoryCursor.Parent
  }
  $g8ReceiptPath = Join-Path $g8ReceiptDirectory ('.g8-023-runtime-audit-' + [Guid]::NewGuid().ToString('N') + '.receipt')
"""
    else:
        path_setup = f"""  $g8ReceiptPath = '{receipt_path.replace("'", "''")}'
  $g8ReceiptDirectory = [IO.Path]::GetDirectoryName($g8ReceiptPath)
  if ([string]::IsNullOrWhiteSpace($g8ReceiptDirectory) -or -not (Test-Path -LiteralPath $g8ReceiptDirectory -PathType Container)) {{ $g8ReceiptState.FailureReason='receipt_directory_unavailable'; throw 'receipt_directory_unavailable' }}
"""
    return f"""$g8ReceiptStream = $null
$g8ReceiptWriter = $null
$g8ReceiptWritable = $true
$g8ReceiptState = @{{ FailureReason = 'receipt_directory_unavailable' }}
try {{
{path_setup}  if (Test-Path -LiteralPath $g8ReceiptPath) {{ $g8ReceiptState.FailureReason='receipt_preoccupied'; throw 'receipt_preoccupied' }}
  try {{
    $g8ReceiptStream = [IO.File]::Open($g8ReceiptPath, [IO.FileMode]::CreateNew, [IO.FileAccess]::Write, [IO.FileShare]::Read)
  }} catch {{
    if (Test-Path -LiteralPath $g8ReceiptPath) {{ $g8ReceiptState.FailureReason='receipt_preoccupied' }} else {{ $g8ReceiptState.FailureReason='receipt_directory_unavailable' }}
    throw 'receipt_create_failed'
  }}
  try {{
    $g8ReceiptWriter = New-Object IO.StreamWriter($g8ReceiptStream, (New-Object Text.UTF8Encoding($false)))
  }} catch {{
    $g8ReceiptState.FailureReason='receipt_write_failed'
    throw 'receipt_writer_failed'
  }}
}} catch {{
  if ($null -ne $g8ReceiptWriter) {{ try {{ $g8ReceiptWriter.Dispose() }} catch {{}} }}
  if ($null -ne $g8ReceiptStream) {{ try {{ $g8ReceiptStream.Dispose() }} catch {{}} }}
  [Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_023_HOST_RESULT=FAILED reason=' + $g8ReceiptState.FailureReason + ' exit_code=2')
  $global:LASTEXITCODE = 2
  $ErrorActionPreference = $g8PreviousErrorActionPreferenceName
  return
}}
function Write-G8Receipt([string]$line) {{
  $checkpoint = $g8ReceiptStream.Position
  try {{
    $g8ReceiptWriter.WriteLine($line)
  }} catch {{
    try {{ $g8ReceiptStream.SetLength($checkpoint); $g8ReceiptStream.Position=$checkpoint; $g8ReceiptStream.Flush([bool]$true) }} catch {{}}
    $g8ReceiptState.FailureReason='receipt_write_failed'
    return $false
  }}
  try {{
    ...
    try {{ $g8ReceiptStream.SetLength($checkpoint); $g8ReceiptStream.Position=$checkpoint; $g8ReceiptStream.Flush([bool]$true) }} catch {{}}
    $g8ReceiptState.FailureReason='receipt_flush_failed'
    return $false
  }}
  return $true
}}
if (-not (Write-G8Receipt 'G8_TEST_READONLY_ACCESS_023_RECEIPT=STARTED')) {{
  $g8ReceiptWritable = $false
  [Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_023_HOST_RESULT=FAILED reason=' + $g8ReceiptState.FailureReason + ' exit_code=2')
  $global:LASTEXITCODE = 2
...
def build_test_command(receipt_path: str, scenario: str, ssh_path: str | None) -> str:
    ...
    if scenario == "fake_ssh":
        ...
        known_hosts_literal = (receipt_path + ".known-hosts").replace("'", "''")
        action = f"""  if (-not (Write-G8Receipt 'G8_TEST_READONLY_ACCESS_023_PRE_SSH_GATE=PASS')) {{ $g8ReceiptWritable = $false; $g8FailureReason = $g8ReceiptState.FailureReason; throw $g8ReceiptState.FailureReason }}
  [Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_023_PRE_SSH_GATE=PASS')
  if (-not (Write-G8Receipt 'G8_TEST_READONLY_ACCESS_023_SSH_ATTEMPTED=YES')) {{ $g8ReceiptWritable = $false; $g8FailureReason = $g8ReceiptState.FailureReason; throw $g8ReceiptState.FailureReason }}
  [Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_023_SSH_ATTEMPTED=YES')
  & '{literal}' `
    -F none -T `
    -p 10003 `
    ...
    -o UserKnownHostsFile='{known_hosts_literal}' `
    pc@8.130.9.163 `
    fixed-readonly-audit
...
try {{
{action}}} catch {{
  [Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_023_LOCAL_GATE=FAILED reason=' + $g8FailureReason)
  if ($g8ReceiptWritable -and -not (Write-G8Receipt ('G8_TEST_READONLY_ACCESS_023_LOCAL_GATE=FAILED reason=' + $g8FailureReason))) {{ $g8ReceiptWritable = $false }}
}} finally {{ $ErrorActionPreference = $g8PreviousErrorActionPreferenceName }}
if ($g8LocalGatePassed) {{ $g8HostExitCode=0; $g8HostResult='G8_TEST_READONLY_ACCESS_023_HOST_RESULT=PASS exit_code=0' }} else {{ $g8HostExitCode=2; $g8HostResult='G8_TEST_READONLY_ACCESS_023_HOST_RESULT=FAILED reason=' + $g8FailureReason + ' exit_code=2' }}
if ($g8ReceiptWritable -and -not (Write-G8Receipt $g8HostResult)) {{
  $g8ReceiptWritable = $false
  $g8HostExitCode = 2
  $g8HostResult = 'G8_TEST_READONLY_ACCESS_023_HOST_RESULT=FAILED reason=' + $g8ReceiptState.FailureReason + ' exit_code=2'
}}
[Console]::Out.WriteLine($g8HostResult)
$global:LASTEXITCODE = $g8HostExitCode
if ($g8ReceiptWritable) {{ try {{ $g8ReceiptWriter.Dispose() }} catch {{}} }}
try {{ $g8ReceiptStream.Dispose() }} catch {{}}
$global:LASTEXITCODE = $g8HostExitCode
"""
...
def build_command(
    ...
    *,
    ...
    return fr"""# 023 尚未获得远端执行授权；本文件只可在清单转为 REMOTE_AUTHORIZED 后执行。
...
try {{
  ...
  foreach ($path in @($ssh,$sshKeygen,$knownHosts)) {{
  ...
  try {{
    ...
    $g8FailureReason = 'ssh_session_failed'
    if (-not (Write-G8Receipt 'G8_TEST_READONLY_ACCESS_023_PRE_SSH_GATE=PASS')) {{ $g8ReceiptWritable=$false; $g8FailureReason=$g8ReceiptState.FailureReason; throw $g8ReceiptState.FailureReason }}
    [Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_023_PRE_SSH_GATE=PASS')
    if (-not (Write-G8Receipt 'G8_TEST_READONLY_ACCESS_023_SSH_ATTEMPTED=YES')) {{ $g8ReceiptWritable=$false; $g8FailureReason=$g8ReceiptState.FailureReason; throw $g8ReceiptState.FailureReason }}
    [Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_023_SSH_ATTEMPTED=YES')
    & $ssh `
...
def self_test(auditor: bytes) -> None:
    ...
    command = build_command(auditor, receipt_path=TRUSTED_LOCAL_APPDATA_RECEIPT)
    remote = build_remote_audit(auditor)
    ...
    try:
        ...
        if arguments.self_test:
            if arguments.change_id is not None or arguments.output_file is not None:
                raise ValueError("invalid_request")
            self_test(auditor)
            print("G8_TEST_READONLY_RUNTIME_AUDIT_023_COMMAND_SELF_TEST=PASS")
            return 0
        ...
        print("G8_TEST_READONLY_RUNTIME_AUDIT_023_COMMAND=FAILED reason=invalid_request")
        return 2
    print("G8_TEST_READONLY_RUNTIME_AUDIT_023_COMMAND=PASS")

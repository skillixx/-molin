...
MSYS_NO_PATHCONV=1 docker run -d --pull="${pull_policy}" --network "${network}" --network-alias bifrost-1 --name "${bifrost_1_container}" -e G8_FAKE_PORT=8080 -v "${docker_repo_root}:/workspace:ro" -w /workspace python:3.12-alpine python infra/scripts/g8_fake_text_upstream.py >/dev/null
MSYS_NO_PATHCONV=1 docker run -d --pull="${pull_policy}" --network "${network}" --network-alias bifrost-2 --name "${bifrost_2_container}" -e G8_FAKE_PORT=8080 -v "${docker_repo_root}:/workspace:ro" -w /workspace python:3.12-alpine python infra/scripts/g8_fake_text_upstream.py >/dev/null
...
echo "G8_PRODUCTION_REHEARSAL=PASS source_commit=$(git -C "${repo_root}" rev-parse HEAD) candidate_image_id=${candidate_image_id} baseline_image_id=${baseline_image_id} candidate_binary_sha256=${candidate_binary_sha} backup_sha256=${backup_sha} schema_tables=${schema_before} readiness_sql_mysql8=true bifrost_nodes=2 bifrost_lb_auth=true bifrost_upstream=fake tls=true sse_buffering=false request_body_limit=20m log_rotation=true metrics_public_status=404 authenticated_traffic_gate_status=503 authenticated_traffic_gate_code=50330 old_version_started=true candidate_started=true rollback_started=true candidate_restored=true database_preserved=true production=false paid_upstream=false"

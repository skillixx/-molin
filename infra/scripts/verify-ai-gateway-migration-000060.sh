...
up_file="${repo_root}/server/migrations/000060_create_ai_gateway_ledger_expand.up.sql"
down_file="${repo_root}/server/migrations/000060_create_ai_gateway_ledger_expand.down.sql"
...
container_name="molin-g1-mysql-000060-${RANDOM}-$$"
...
docker cp "${up_file}" "${container_name}:/tmp/000060.up.sql" >/dev/null
docker cp "${down_file}" "${container_name}:/tmp/000060.down.sql" >/dev/null
...
apply_up() {
  ...
  docker exec -e "MYSQL_PWD=${root_password}" "${container_name}" sh -c \
    "mysql --protocol=socket -uroot --database='${database_name}' < /tmp/000060.up.sql"
  mysql_exec -e 'UPDATE schema_migrations SET version=60,dirty=0 WHERE version=59 AND dirty=1'
}

apply_down() {
  mysql_exec -e 'UPDATE schema_migrations SET dirty=1 WHERE version=60 AND dirty=0'
  docker exec -e "MYSQL_PWD=${root_password}" "${container_name}" sh -c \
    "mysql --protocol=socket -uroot --database='${database_name}' < /tmp/000060.down.sql"
  mysql_exec -e 'UPDATE schema_migrations SET version=59,dirty=0 WHERE version=60 AND dirty=1'
}
...
apply_up
assert_scalar "SELECT CONCAT(version,':',dirty) FROM schema_migrations" "60:0" "first_up_version"
...
apply_down
assert_scalar "SELECT CONCAT(version,':',dirty) FROM schema_migrations" "59:0" "down_version"
...
apply_up
assert_scalar "SELECT CONCAT(version,':',dirty) FROM schema_migrations" "60:0" "reup_version"
...
if docker exec -e "MYSQL_PWD=${root_password}" "${container_name}" sh -c \
  "mysql --protocol=socket -uroot --database='molin_g1_drift' < /tmp/000060.up.sql" >/dev/null 2>&1; then

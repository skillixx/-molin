...
g1_up="${repo_root}/server/migrations/000059_create_ai_gateway_ledger_expand.up.sql"
g2_up="${repo_root}/server/migrations/000060_add_ai_gateway_g2_projects_keys.up.sql"
g2_down="${repo_root}/server/migrations/000060_add_ai_gateway_g2_projects_keys.down.sql"
for file in "${g1_up}" "${g2_up}" "${g2_down}"; do
  test -f "${file}" || { echo "G2_MYSQL_MIGRATION=FAILED reason=migration_file_missing"; exit 2; }
done

container_name="molin-g2-mysql-000060-${RANDOM}-$$"
...
docker cp "${g1_up}" "${container_name}:/tmp/000059.up.sql" >/dev/null
docker cp "${g2_up}" "${container_name}:/tmp/000060.up.sql" >/dev/null
docker cp "${g2_down}" "${container_name}:/tmp/000060.down.sql" >/dev/null
...
apply_file /tmp/000060.down.sql
mysql_exec -e "UPDATE schema_migrations SET version=59,dirty=0;"

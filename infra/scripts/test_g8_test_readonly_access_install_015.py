#!/usr/bin/env python3
"""验证 015 安装器的冻结来源、no-clobber、最小 sudo 与回滚边界。"""
...
class TestG8ReadonlyAccessInstall015(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_bash_syntax_and_two_change_ids_are_frozen(self) -> None:
        ...
        self.assertIn("AUTH_CHANGE_ID='CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-015'", self.source)
        ...
        self.assertIn("molin-g8-install-CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-015", self.source)
    ...
    def test_manifest_gate_accepts_only_frozen_crlf_lines(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-015-manifest-") as temporary:
            root = Path(temporary)
    ...
    def test_copy_no_clobber_preserves_existing_target(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-015-copy-") as temporary:
            root = Path(temporary)
            source = root / "source"
            target = root / "target"
    ...
    def test_partial_live_install_is_removed_by_exit_trap(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-015-rollback-") as temporary:
            root = Path(temporary)
            parent = root / "molin"
            parent.mkdir()
            source = root / "source"
    ...
    def test_global_rollback_removes_sudoers_first_and_revalidates(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-015-global-rollback-") as temporary:
            root = Path(temporary)
            parent = root / "molin"
            parent.mkdir()
            auditor = parent / "auditor"
    ...
    def test_auditor_entry_failure_rolls_back_all_live_files(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-015-entry-rollback-") as temporary:
            root = Path(temporary)
            parent = root / "molin"
            parent.mkdir()
            auditor = parent / "auditor"
    ...
    def test_sudo_scope_accepts_only_one_frozen_nopasswd_command(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-015-sudo-scope-") as temporary:
            root = Path(temporary)
            fake_sudo = root / "sudo"
            harness = root / "harness.sh"
            harness.write_text(

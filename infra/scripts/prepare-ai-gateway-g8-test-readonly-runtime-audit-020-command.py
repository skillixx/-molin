#!/usr/bin/env python3
"""生成 020 无安装、无 sudo 的单次 Docker 只读审计命令。"""

import sys

# 在加载脚本目录可影响的模块前强制隔离解释器，避免本地模块劫持。
if not sys.flags.isolated:
    print("G8_TEST_READONLY_RUNTIME_AUDIT_020_COMMAND=FAILED reason=isolated_python_required")
    raise SystemExit(2)
...
'''
    ...
    source = replace_exact(
        ...
        "printf 'AUDIT_MODE=READ_ONLY_SINGLE_SESSION\\n'\n"
        "[[ \"$(id -un 2>/dev/null || true)\" == pc ]] || { printf 'G8_TEST_READONLY_RUNTIME_AUDIT_020=FAILED reason=invalid_user\\n'; exit 42; }\n"
        "printf 'G8_TEST_READONLY_RUNTIME_AUDIT_020=PREFLIGHT_PASS\\n'\n",
    )
...
audit_output="$(run_frozen_audit)" || {
  printf 'G8_TEST_READONLY_RUNTIME_AUDIT_020=FAILED reason=audit_evidence_failed\n'
  exit 42
}
printf '%s\n' "$audit_output"
...
printf 'G8_TEST_READONLY_RUNTIME_AUDIT_020=COLLECTION_PASS\n'
'''
...
def receipt_setup(receipt_path: str) -> str:
  ...
  $g8ReceiptPath = Join-Path $profile '.g8-020-runtime-audit-receipt.txt'
"""
    else:
...
def build_test_command(receipt_path: str, scenario: str, ssh_path: str | None) -> str:
    ...
    if scenario == "fake_ssh":
        ...
        action = f"""  [Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_020_PRE_SSH_GATE=PASS')
...
if ($g8LocalGatePassed) {{ $g8HostExitCode=0; $g8HostResult='G8_TEST_READONLY_ACCESS_020_HOST_RESULT=PASS exit_code=0' }} else {{ $g8HostExitCode=2; $g8HostResult='G8_TEST_READONLY_ACCESS_020_HOST_RESULT=FAILED reason=' + $g8FailureReason + ' exit_code=2' }}
...
def build_command(
    ...
    *,
    ...
    return fr"""# 020 尚未获得远端执行授权；本文件只可在清单转为 REMOTE_AUTHORIZED 后执行。
    ...
    try:
        ...
        if arguments.self_test:
            if arguments.change_id is not None or arguments.output_file is not None:
                raise ValueError("invalid_request")
            self_test(auditor)
            print("G8_TEST_READONLY_RUNTIME_AUDIT_020_COMMAND_SELF_TEST=PASS")
            return 0
        ...
        print("G8_TEST_READONLY_RUNTIME_AUDIT_020_COMMAND=FAILED reason=invalid_request")
        return 2
    print("G8_TEST_READONLY_RUNTIME_AUDIT_020_COMMAND=PASS")

...
if not sys.flags.isolated:
    print("G8_TEST_READONLY_RUNTIME_AUDIT_021_RUNNER=FAILED reason=isolated_python_required")
    raise SystemExit(2)
...
GENERATOR_NAME = "prepare-ai-gateway-g8-test-readonly-runtime-audit-021-command.py"
AUDITOR_NAME = "audit-ai-gateway-g8-test-server-readonly.sh"
RUNNER_RELATIVE = "infra/scripts/run-ai-gateway-g8-test-readonly-runtime-audit-021.py"
...
FIXED_CHILD_LINES = frozenset(
    {
        ...
        "G8_TEST_READONLY_RUNTIME_AUDIT_021=FAILED reason=invalid_user",
        "G8_TEST_READONLY_RUNTIME_AUDIT_021=FAILED reason=audit_evidence_failed",
        "G8_TEST_READONLY_ACCESS_021_PRE_SSH_GATE=PASS",
        "G8_TEST_READONLY_ACCESS_021_SSH_ATTEMPTED=YES",
        "G8_TEST_READONLY_ACCESS_021_HOST_RESULT=PASS exit_code=0",
        *(
            f"G8_TEST_READONLY_ACCESS_021_LOCAL_GATE=FAILED reason={reason}"
            for reason in FIXED_FAILURE_REASONS
        ),
        *(
            f"G8_TEST_READONLY_ACCESS_021_HOST_RESULT=FAILED reason={reason} exit_code=2"
            for reason in FIXED_FAILURE_REASONS
        ),
    }
)
...
def load_generator():
    """从固定同目录路径加载 021 生成器，不接受调用方提供模块路径。"""
    path = Path(__file__).with_name(GENERATOR_NAME)
    specification = importlib.util.spec_from_file_location("g8_runtime_audit_021_generator", path)
...
def self_test() -> None:
    ...
    if os.name != "nt":
        return
    with tempfile.TemporaryDirectory(prefix="g8-021-fixed-runner-") as temporary:
        ...
        if (
            ...
            or "G8_TEST_READONLY_ACCESS_021_SSH_ATTEMPTED=YES" in invalid.stdout
        ):
        ...
        expected = "G8_TEST_READONLY_ACCESS_021_HOST_RESULT=PASS exit_code=0"
        ...
        if lines.count("G8_TEST_READONLY_ACCESS_021_SSH_ATTEMPTED=YES") != 1 or lines[-1] != expected:
            raise RuntimeError("invalid_fixed_runner")
...
def execute(arguments: argparse.Namespace) -> int:
    ...
    generator = load_generator()
    auditor = generator.read_frozen_auditor()
    command = generator.build_command(auditor, receipt_path=generator.TRUSTED_PROFILE_RECEIPT)
    ...
    if hashlib.sha256(encoded).hexdigest() != arguments.expected_command_sha256:
        raise RuntimeError("command_drift")
    print("G8_TEST_READONLY_RUNTIME_AUDIT_021_RUNNER=LOCAL_GATE_PASS")
    print("G8_TEST_READONLY_RUNTIME_AUDIT_021_POWERSHELL_ATTEMPTED=YES")
    try:
        completed = invoke_powershell(command)
    except Exception:
        print("G8_TEST_READONLY_RUNTIME_AUDIT_021_RUNNER=FAILED reason=powershell_session_failed")
        return 2
    forwarded = []
    ...
    success = "G8_TEST_READONLY_ACCESS_021_HOST_RESULT=PASS exit_code=0"
    if completed.returncode != 0 or success not in forwarded:
        print("G8_TEST_READONLY_RUNTIME_AUDIT_021_RUNNER=FAILED reason=powershell_session_failed")
        return 2
    print("G8_TEST_READONLY_RUNTIME_AUDIT_021_RUNNER=PASS")
    return 0
    ...
    try:
        ...
        if arguments.self_test:
            if any(
                ...
                raise ValueError("invalid_request")
            self_test()
            print("G8_TEST_READONLY_RUNTIME_AUDIT_021_RUNNER_SELF_TEST=PASS")
            return 0
        ...
        print("G8_TEST_READONLY_RUNTIME_AUDIT_021_RUNNER=FAILED reason=invalid_request")
        return 2
    except RunnerFailure as failure:
        print(f"G8_TEST_READONLY_RUNTIME_AUDIT_021_RUNNER=FAILED reason={failure.reason}")
        return 2
    except Exception:
        print("G8_TEST_READONLY_RUNTIME_AUDIT_021_RUNNER=FAILED reason=local_gate_failed")
        return 2

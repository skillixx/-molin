#!/usr/bin/env python3
"""验证 020 无安装、无 sudo 的单次 Docker 只读审计命令。"""
...
def load_module():
    """从固定文件加载离线生成器。"""
    specification = importlib.util.spec_from_file_location("g8_runtime_audit_020", SCRIPT_PATH)
...
class TestPrepareG8ReadonlyRuntimeAudit020Command(unittest.TestCase):
    def test_source_auditor_is_frozen_and_019_remains_untouched(self) -> None:
        """020 只转换既有只读审计源，不能修改 019 墓碑或引入新安装资产。"""
        module = load_module()
    ...
    def test_remote_audit_uses_direct_docker_without_install_or_sudo(self) -> None:
        ...
        module = load_module()
        command = module.build_command(AUDITOR_PATH.read_bytes(), receipt_path=r"C:\g8-020.receipt.txt")
        remote = decode_remote(command)
        for required in (
            CHANGE_ID,
            "G8_TEST_READONLY_RUNTIME_AUDIT_020=PREFLIGHT_PASS",
            ...
            "G8_TEST_READONLY_RUNTIME_AUDIT_020=COLLECTION_PASS",
    ...
    def test_single_noninteractive_ssh_has_fixed_security_controls(self) -> None:
        ...
        for required in (
            ...
            "LogLevel=QUIET",
            "G8_TEST_READONLY_ACCESS_020_PRE_SSH_GATE=PASS",
            "G8_TEST_READONLY_ACCESS_020_SSH_ATTEMPTED=YES",
        ):
    ...
    def test_fake_ssh_records_exact_attempt_boundary_without_network(self) -> None:
        ...
        for fake_exit, expected in (
            ...
            with self.subTest(fake_exit=fake_exit), tempfile.TemporaryDirectory(prefix="g8-020-runtime-fake-") as temporary:
    ...
    def test_self_test_is_offline_and_cli_has_no_receipt_override(self) -> None:
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(result.stdout.strip(), "G8_TEST_READONLY_RUNTIME_AUDIT_020_COMMAND_SELF_TEST=PASS")
        with tempfile.TemporaryDirectory(prefix="g8-020-runtime-formal-") as temporary:
            ...
            self.assertIn(".g8-020-runtime-audit-receipt.txt", command)

...
for path in "${repo_root}"/server/migrations/*.up.sql; do
  ...
  # G6运行当前HEAD，需装配000072共享媒体兼容层；000073权限seed不属于HTTP隔离夹具。

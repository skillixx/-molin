#!/usr/bin/env python3
"""生成 019 单次交互安装会话的冻结命令；本工具本身不连接测试服。"""

import sys

# 在加载脚本目录可影响的模块前强制隔离解释器，避免本地模块劫持。
if not sys.flags.isolated:
    print("G8_TEST_READONLY_ACCESS_019_COMMAND=FAILED reason=isolated_python_required")
    raise SystemExit(2)
...
ROOT_COPY = "/root/molin-g8-install-CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-019"
INSTALLER_NAME = "g8-test-readonly-access-install-019.sh"
EXPECTED_INSTALLER_SHA256 = "c1178bbc5b566357b5862484fab62dc9f267d8e341792eb8aa6871602e212935"
...
def build_command(installer: bytes) -> str:
    ...
    local_command = f"""# 019 尚未获得远端执行授权。本文件只可在清单转为 REMOTE_AUTHORIZED 后执行。
...
try {{
  ...
  try {{
    ...
    [Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_019_PRE_SSH_GATE=PASS')
    [Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_019_SSH_ATTEMPTED=YES')
    & $ssh `
      -F none -tt -p 10003 `
  ...
  [Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_019_LOCAL_GATE=FAILED reason=' + $g8FailureReason)
}} finally {{
  $ErrorActionPreference = $g8PreviousErrorActionPreference
}}
if ($g8LocalGatePassed) {{
  $g8HostExitCode = 0
  [Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_019_HOST_RESULT=PASS exit_code=0')
}} else {{
  $g8HostExitCode = 2
  [Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_019_HOST_RESULT=FAILED exit_code=2')
}}
# 只更新调用方可读取的退出码，不终止父 PowerShell，也不关闭用户当前窗口。
...
/usr/bin/test "$auth_change_id" = 'CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-019'
...
def self_test(installer: bytes) -> None:
    ...
    local_required = (
        ...
        "  -tt `",
        "RequestTTY=force",
        "G8_TEST_READONLY_ACCESS_019_HOST_RESULT=PASS exit_code=0",
    ...
    if any(value not in command for value in local_required) or any(value not in remote for value in remote_required):
        raise RuntimeError("invalid_command")
    if b"G8_TEST_READONLY_ACCESS_INSTALL_019=PASS" not in installer \
...
def main() -> int:
    # 消费态未来必须在参数解析和安装器读取前拒绝；当前 019 仅处于工程候选态。
    ...
    try:
        ...
        if arguments.self_test:
            if arguments.change_id is not None or arguments.output_file is not None:
                raise ValueError("invalid_request")
            self_test(installer)
            print("G8_TEST_READONLY_ACCESS_019_COMMAND_SELF_TEST=PASS")
            return 0
        ...
        print("G8_TEST_READONLY_ACCESS_019_COMMAND=FAILED reason=invalid_request")
        return 2
    print("G8_TEST_READONLY_ACCESS_019_COMMAND=PASS")

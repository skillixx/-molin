#!/usr/bin/env python3
"""通过公开 CLI 验证 023 固定启动器的离线失败关闭边界。"""
...
RUNNER_PATH = Path(__file__).with_name("run-ai-gateway-g8-test-readonly-runtime-audit-023.py")
CHANGE_ID = "CHG-G8-TEST-READONLY-RUNTIME-AUDIT-DROP-20260815-023"


def load_runner():
    """从固定同目录路径加载启动器，便于验证不触发子进程的纯函数门禁。"""
    specification = importlib.util.spec_from_file_location("g8_runtime_audit_023_runner", RUNNER_PATH)
...
class TestG8ReadonlyRuntimeAudit023Runner(unittest.TestCase):
    @classmethod
    ...
    def test_self_test_proves_fixed_launcher_without_network(self) -> None:
        ...
        self.assertEqual(
            ...
            (0, "G8_TEST_READONLY_RUNTIME_AUDIT_023_RUNNER_SELF_TEST=PASS\n", ""),
        )
    ...
    def test_formal_mode_fails_before_materials_without_explicit_execution_flag(self) -> None:
        ...
        self.assertEqual(
            (result.returncode, result.stdout, result.stderr),
            (2, "G8_TEST_READONLY_RUNTIME_AUDIT_023_RUNNER=FAILED reason=invalid_request\n", ""),
        )
    ...
    def test_missing_engineering_object_is_low_sensitive_and_never_attempts_powershell(self) -> None:
        ...
        self.assertEqual(
            (result.returncode, result.stdout, result.stderr),
            (2, "G8_TEST_READONLY_RUNTIME_AUDIT_023_RUNNER=FAILED reason=engineering_material_unavailable\n", ""),
        )
    ...
    def test_child_output_uses_exact_low_sensitive_marker_allowlist(self) -> None:
        ...
        required = ("api_health",)
        self.assertTrue(self.runner.allowed_child_line("G8_TEST_READONLY_ACCESS_023_PRE_SSH_GATE=PASS", required))
        self.assertTrue(self.runner.allowed_child_line("G8_TEST_READONLY_RUNTIME_AUDIT_023=PREFLIGHT_PASS", required))
        self.assertTrue(self.runner.allowed_child_line("G8_TEST_READONLY_RUNTIME_AUDIT_023=COLLECTION_PASS", required))
        self.assertTrue(
            self.runner.allowed_child_line(
                "G8_TEST_READONLY_RUNTIME_AUDIT_023=FAILED reason=audit_evidence_failed", required
            )
        )
        ...
        self.assertFalse(
            self.runner.allowed_child_line(
                "G8_TEST_READONLY_ACCESS_023_HOST_RESULT=FAILED reason=receipt_unavailable exit_code=2",
                required,
            )
        )
        self.assertFalse(self.runner.allowed_child_line("G8_TEST_READONLY_ACCESS_023_SECRET=leak", required))
        self.assertFalse(self.runner.allowed_child_line("G8_TEST_READONLY_RUNTIME_AUDIT_023=FAILED path=C:\\secret", required))

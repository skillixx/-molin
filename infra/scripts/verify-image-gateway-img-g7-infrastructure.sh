...
echo "IMAGE_G7_INFRA=PASS mysql=8.0.46 full_chain_1_to_71=true network_internal=true host_ports=0 provider=fake real_provider_calls=0 minio_private_buckets=3 signed_url=true rabbit_durable_queue=true rabbit_dlq=true prompt_in_rabbit=false async_once=true missing_prompt_release=true cleanup_worker=true legal_hold_preserved=true compensation_worker=true image_metrics=true reconciliation_difference_zero=true prometheus_rules=true traffic_default_closed=true test_server=false real_credentials=0"

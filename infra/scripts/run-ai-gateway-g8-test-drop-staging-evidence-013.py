#!/usr/bin/env python3
"""G8 Drop 固定 011 暂存的一次性 013 只读取证包装器。"""
...
LOCAL_DIAGNOSTIC_SHA256 = "aa1bb957e9b950eed263424a0b1e104695f68cad076e33b74fe5b70e54b320ed"
...
def render_result(result: EvidenceResult):
    ...
    text = (
        f"G8_TEST_READONLY_DROP_STAGING_EVIDENCE_013={status}\n"
...
def fixed_ssh_path() -> Path:
    if os.name == "nt":
        system_root = os.environ.get("SystemRoot")
        if not system_root:
            raise EvidenceError("ssh_unavailable")
        path = Path(system_root) / "System32" / "OpenSSH" / "ssh.exe"
    else:
...
def run_once(helper, materials, ssh_path: Path) -> EvidenceResult:
    """启动唯一固定 SSH；无重试分支。"""
    remote_program = build_remote_program().encode("utf-8")
    with tempfile.TemporaryDirectory(prefix="g8-013-") as directory:
    ...
    try:
        ...
        if arguments.self_test:
            if any((arguments.change_id, arguments.known_hosts, arguments.identity_file, arguments.identity_public_key)):
                raise EvidenceError("invalid_request")
            compile(build_remote_program(), "<g8-013-remote>", "exec")
            load_local_diagnostic()
            print("G8_TEST_READONLY_DROP_STAGING_EVIDENCE_013_SELF_TEST=PASS")
            return 0
        ...
        print("G8_TEST_READONLY_DROP_STAGING_EVIDENCE_013=FAILED reason=evidence_unavailable")
        return 2

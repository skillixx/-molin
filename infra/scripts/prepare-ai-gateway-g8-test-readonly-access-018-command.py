#!/usr/bin/env python3
"""生成 018 单次交互安装会话的冻结命令；本工具本身不连接测试服。"""

import sys

# 在加载脚本目录可影响的模块前强制隔离解释器，避免本地模块劫持。
if not sys.flags.isolated:
    print("G8_TEST_READONLY_ACCESS_018_COMMAND=FAILED reason=isolated_python_required")
    raise SystemExit(2)
...
ROOT_COPY = "/root/molin-g8-install-CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-018"
INSTALLER_NAME = "g8-test-readonly-access-install-018.sh"
EXPECTED_INSTALLER_SHA256 = "3232f3265da00d0a8f531798c32917bc77efd4725c30b4c9a99022d91484de85"
...
def build_command(installer: bytes) -> str:
    ...
    return f"""# 018 尚未获得远端执行授权。本文件只可在清单转为 REMOTE_AUTHORIZED 后按两段人工执行。
...
try {{
  ...
  try {{
    ...
    [Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_018_PRE_SSH_GATE=PASS')
    [Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_018_SSH_ATTEMPTED=YES')
    & $ssh `
      -F none -tt -p 10003 `
  ...
  [Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_018_LOCAL_GATE=FAILED reason=' + $g8FailureReason)
}} finally {{
...
/usr/bin/test "$auth_change_id" = 'CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-018'
...
def self_test(installer: bytes) -> None:
    ...
    if b"G8_TEST_READONLY_ACCESS_INSTALL_018=PASS" not in installer \
...
def main() -> int:
    # 消费态未来必须在参数解析和安装器读取前拒绝；当前 018 仅处于工程候选态。
    ...
    try:
        ...
        if arguments.self_test:
            if arguments.change_id is not None or arguments.output_file is not None:
                raise ValueError("invalid_request")
            self_test(installer)
            print("G8_TEST_READONLY_ACCESS_018_COMMAND_SELF_TEST=PASS")
            return 0
        ...
        print("G8_TEST_READONLY_ACCESS_018_COMMAND=FAILED reason=invalid_request")
        return 2
    print("G8_TEST_READONLY_ACCESS_018_COMMAND=PASS")

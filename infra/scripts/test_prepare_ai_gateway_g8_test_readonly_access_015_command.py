#!/usr/bin/env python3
"""验证 015 命令生成器只生成冻结人工步骤，不读取密码或连接网络。"""
...
SCRIPT_PATH = Path(__file__).with_name("prepare-ai-gateway-g8-test-readonly-access-015-command.py")
INSTALLER_PATH = Path(__file__).with_name("g8-test-readonly-access-install-015.sh")
CHANGE_ID = "CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-015"
...
def load_module():
    """从固定路径加载生成器，仅调用离线纯函数。"""
    specification = importlib.util.spec_from_file_location("g8_command_015", SCRIPT_PATH)
...
class TestPrepareG8ReadonlyAccess015Command(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_self_test_is_local_and_exact(self) -> None:
        ...
        self.assertEqual(result.stdout.strip(), "G8_TEST_READONLY_ACCESS_015_COMMAND_SELF_TEST=PASS")
    ...
    def test_formal_generation_creates_one_new_low_sensitive_file(self) -> None:
        with tempfile.TemporaryDirectory(prefix="g8-015-command-") as temporary:
            ...
            self.assertIn("G8_TEST_READONLY_ACCESS_015_COMMAND=PASS", result.stdout)
            ...
            self.assertEqual(duplicate.stdout.strip(), "G8_TEST_READONLY_ACCESS_015_COMMAND=FAILED reason=invalid_request")
    ...
    def test_command_separates_authorization_from_frozen_011_source(self) -> None:
        ...
        self.assertIn("auth_change_id=CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-015", self.command)
    ...
    def test_trusted_windows_path_prefix_ignores_forged_environment(self) -> None:
        ...
        prefix = self.command.split("$previousSystemRoot =", 1)[0]
        probe = (
    ...
    def test_remote_block_has_one_manual_sudo_and_only_noninteractive_followups(self) -> None:
        ...
        self.assertIn("G8_TEST_READONLY_ACCESS_PREFLIGHT_015=PASS", command)
        self.assertIn("G8_TEST_READONLY_ACCESS_POSTCHECK_015=PASS", command)
        self.assertIn("G8_015_INSTALL_B64", command)
        self.assertEqual(command.count("G8_015_REMOTE"), 2)
        self.assertIn("/bin/bash -s <<'G8_015_REMOTE'", command)
    ...
    def test_remote_here_doc_has_valid_bash_syntax(self) -> None:
        ...
        remote = self.command.split("/bin/bash -s <<'G8_015_REMOTE'\n", 1)[1].split("\nG8_015_REMOTE\n", 1)[0]
        result = subprocess.run(
            [bash_executable(), "-n"],
            input=remote,
            capture_output=True,
    ...
    def test_embedded_installer_is_exact_and_secrets_are_absent(self) -> None:
        encoded = self.command.split("G8_015_INSTALL_B64'\n", 1)[1].split("\nG8_015_INSTALL_B64", 1)[0]
    ...
    def test_invalid_arguments_fail_closed_without_echo(self) -> None:
        for arguments in (
            ...
            self.assertEqual(result.stdout.strip(), "G8_TEST_READONLY_ACCESS_015_COMMAND=FAILED reason=invalid_request")
    ...
    def test_future_consumed_gate_precedes_installer_read_and_argument_parse(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-015-consumed-") as temporary:
        ...
        self.assertEqual(result.returncode, 2)
        self.assertEqual(result.stdout.strip(), "G8_TEST_READONLY_ACCESS_015_COMMAND=FAILED reason=change_id_consumed")
        self.assertEqual(result.stderr, "")

#!/usr/bin/env python3
"""验证 020 命令生成器只生成冻结人工步骤，不读取密码或连接网络。"""
...
class TestPrepareG8ReadonlyAccess020SecurityControls(unittest.TestCase):
    def setUp(self) -> None:
        self.module = load_module()
        self.installer = INSTALLER_PATH.read_bytes()
        # 旧控制回归使用无回执纯构造，避免固定回执路径干扰各段的独立故障注入；
        # 正式 CLI 的强制回执行为由 020 主测试文件单独覆盖。
    ...
    def test_formal_generation_creates_one_new_low_sensitive_file(self) -> None:
        with tempfile.TemporaryDirectory(prefix="g8-020-command-") as temporary:
            ...
            self.assertEqual(
                generated,
                self.module.build_command(self.installer, receipt_path=self.module.TRUSTED_PROFILE_RECEIPT),
            )
            self.assertIn("G8_TEST_READONLY_ACCESS_020_COMMAND=PASS", result.stdout)
            ...
            self.assertEqual(duplicate.stdout.strip(), "G8_TEST_READONLY_ACCESS_020_COMMAND=FAILED reason=invalid_request")
    ...
    def test_command_separates_authorization_from_frozen_011_source(self) -> None:
        ...
        self.assertIn("auth_change_id=CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260815-020", remote)
    ...
    def test_windows_paths_come_from_trusted_api_and_ssh_is_single_attempt(self) -> None:
        ...
        self.assertIn("$ErrorActionPreference = $g8PreviousErrorActionPreferenceName", command)
    ...
    def test_host_shell_never_exits_and_always_reports_fixed_result(self) -> None:
        ...
        self.assertIn("G8_TEST_READONLY_ACCESS_020_HOST_RESULT=PASS exit_code=0", self.command)
        self.assertIn("G8_TEST_READONLY_ACCESS_020_HOST_RESULT=FAILED reason=", self.command)
    ...
    def test_host_result_tail_keeps_parent_process_alive(self) -> None:
        ...
        for passed, marker, code in (
            ("$true", "G8_TEST_READONLY_ACCESS_020_HOST_RESULT=PASS exit_code=0", "0"),
            ("$false", "G8_TEST_READONLY_ACCESS_020_HOST_RESULT=FAILED reason=test_failure exit_code=2", "2"),
        ):
            with self.subTest(passed=passed):
                probe = f"$g8LocalGatePassed={passed};$g8FailureReason='test_failure';" + tail + ";Write-Output ('OBSERVED=' + $LASTEXITCODE)"
    ...
    def test_low_sensitive_stage_reasons_and_ssh_attempt_boundary_are_fixed(self) -> None:
        ...
        stage_contract = (
            ...
            ("ssh_session_failed", "[Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_020_PRE_SSH_GATE=PASS')"),
        )
        ...
        pre_ssh = "[Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_020_PRE_SSH_GATE=PASS')"
        attempted = "[Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_020_SSH_ATTEMPTED=YES')"
        invocation = "    & $ssh `"
    ...
    def test_frozen_known_hosts_ignores_forged_temp_and_blocks_replacement(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-020-known-hosts-lock-") as temporary:
    ...
    def test_preoccupied_frozen_known_hosts_is_not_removed_or_marked_as_ssh_attempt(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-020-known-hosts-preoccupied-") as temporary:
            ...
            probe = (
                "$g8PreviousErrorActionPreferenceName=[string]$ErrorActionPreference;$ErrorActionPreference='Stop';"
        ...
        self.assertEqual(
            ...
            [
                "G8_TEST_READONLY_ACCESS_020_LOCAL_GATE=FAILED reason=known_hosts_failed",
                "G8_TEST_READONLY_ACCESS_020_HOST_RESULT=FAILED reason=known_hosts_failed exit_code=2",
            ],
        )
    ...
    def test_each_stage_exception_is_reduced_to_fixed_low_sensitive_output(self) -> None:
        ...
        for reason in reasons:
            with self.subTest(reason=reason):
                ...
                self.assertEqual(
                    result.stdout.strip(),
                    "G8_TEST_READONLY_ACCESS_020_LOCAL_GATE=FAILED reason=" + reason,
                )
    ...
    def test_encrypted_identity_is_rejected_without_extra_passphrase_prompt(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-020-encrypted-identity-") as temporary:
    ...
    def test_local_material_failure_does_not_disclose_absolute_path(self) -> None:
        ...
        local_gate = self.command
        fake_identity = str(Path(tempfile.gettempdir()).resolve() / "g8-020-sensitive-identity" / "id_ed25519")
        ...
        self.assertEqual(
            result.stdout.splitlines(),
            [
                "G8_TEST_READONLY_ACCESS_020_LOCAL_GATE=FAILED reason=material_evidence_failed",
                "G8_TEST_READONLY_ACCESS_020_HOST_RESULT=FAILED reason=material_evidence_failed exit_code=2",
            ],
        )
    ...
    def test_trusted_windows_path_prefix_runs_from_standard_input(self) -> None:
        ...
        probe = (
            prefix
            + "\nWrite-Output 'G8_020_WINDOWS_PATH_GUARD=PASS'\n"
            + "} finally { $ErrorActionPreference = $g8PreviousErrorActionPreferenceName }\n"
        )
        result = subprocess.run(
            [
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(result.stderr, "")
        self.assertIn("G8_020_WINDOWS_PATH_GUARD=PASS", result.stdout)
    ...
    def test_material_hash_does_not_depend_on_module_autoload(self) -> None:
        ...
        probe = (
            ...
            "  [Console]::Out.WriteLine('G8_020_MODULE_ISOLATED_HASH=PASS')\n"
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(result.stderr, "")
        self.assertIn("G8_020_MODULE_ISOLATED_HASH=PASS", result.stdout)
        ...
        for original, replacement, label in scenarios:
            with self.subTest(label=label):
                ...
                probe = (
                    ...
                    f"  [Console]::Out.WriteLine('G8_020_HASH_{label}_RELEASE=PASS')\n"
                ...
                self.assertEqual(result.returncode, 0, result.stderr)
                self.assertEqual(result.stderr, "")
                self.assertIn(f"G8_020_HASH_{label}_RELEASE=PASS", result.stdout)
    ...
    def test_path_guard_error_cannot_reach_identity_or_ssh_stage(self) -> None:
        ...
        probe = (
            corrupted
            + "\nWrite-Output 'G8_020_IDENTITY_OR_SSH_STAGE_REACHED'\n"
            + "} finally { $ErrorActionPreference = $g8PreviousErrorActionPreferenceName }\n"
        )
        result = subprocess.run(
            [
        ...
        self.assertNotIn("G8_020_IDENTITY_OR_SSH_STAGE_REACHED", result.stdout)
    ...
    def test_remote_block_has_one_manual_sudo_and_only_noninteractive_followups(self) -> None:
        ...
        self.assertIn("G8_TEST_READONLY_ACCESS_PREFLIGHT_020=PASS", remote)
        self.assertIn("G8_TEST_READONLY_ACCESS_POSTCHECK_020=PASS", remote)
        self.assertIn("G8_020_INSTALL_B64", remote)
        self.assertNotIn("G8_020_REMOTE", self.command)
    ...
    def test_invalid_arguments_fail_closed_without_echo(self) -> None:
        for arguments in (
            ...
            self.assertEqual(result.stdout.strip(), "G8_TEST_READONLY_ACCESS_020_COMMAND=FAILED reason=invalid_request")
            self.assertEqual(result.stderr, "")
    ...
    def test_unc_and_device_output_paths_are_rejected_before_filesystem_probe(self) -> None:
        ...
        for unsafe in (
            r"\\forged-host\share\g8-020.txt",
            r"\\?\UNC\forged-host\share\g8-020.txt",
            r"\\.\C:\g8-020.txt",
            "//forged-host/share/g8-020.txt",
            r"C:\g8\NUL",
            r"C:\g8\CON.txt",
            r"C:\g8\AUX. ",
            r"C:\g8\COM1.log",
    ...
    def test_future_consumed_gate_precedes_installer_read_and_argument_parse(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-020-consumed-") as temporary:
        ...
        self.assertEqual(result.returncode, 2)
        self.assertEqual(result.stdout.strip(), "G8_TEST_READONLY_ACCESS_020_COMMAND=FAILED reason=change_id_consumed")
        self.assertEqual(result.stderr, "")

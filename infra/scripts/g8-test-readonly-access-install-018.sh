#!/bin/bash
# 018 只从已由 014 证明完整的 011 暂存安装最小只读入口；不接受调用方参数。
set -euo pipefail
...
AUTH_CHANGE_ID='CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-018'
...
fail() {
    /usr/bin/printf '%s\n' 'G8_TEST_READONLY_ACCESS_INSTALL_018=FAILED reason=install_gate_failed'
    return 1
}
...
check_candidate() {
    ...
    if [ "$allow_installer" -eq 1 ]; then
        actual=$(/usr/bin/find "$directory" -mindepth 1 -maxdepth 1 ! -name 'g8-test-readonly-access-install-018.sh' -printf '%f\n' | /usr/bin/sort | /usr/bin/tr '\n' ' ')
        check_file "$directory/g8-test-readonly-access-install-018.sh" || return 1
        [ "$(/usr/bin/stat -c '%U:%G:%a' -- "$directory/g8-test-readonly-access-install-018.sh")" = 'root:root:700' ] || return 1
    else
...
main() {
    ...
    install_complete=1
    trap - EXIT
    /usr/bin/printf '%s\n' 'G8_TEST_READONLY_ACCESS_INSTALL_018=PASS'
}

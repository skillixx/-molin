#!/usr/bin/env python3
"""验证 017 命令生成器只生成冻结人工步骤，不读取密码或连接网络。"""
...
SCRIPT_PATH = Path(__file__).with_name("prepare-ai-gateway-g8-test-readonly-access-017-command.py")
INSTALLER_PATH = Path(__file__).with_name("g8-test-readonly-access-install-017.sh")
CHANGE_ID = "CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-017"
...
def load_module():
    """从固定路径加载生成器，仅调用离线纯函数。"""
    specification = importlib.util.spec_from_file_location("g8_command_017", SCRIPT_PATH)
...
class TestPrepareG8ReadonlyAccess017Command(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_self_test_is_local_and_exact(self) -> None:
        ...
        self.assertEqual(result.stdout.strip(), "G8_TEST_READONLY_ACCESS_017_COMMAND_SELF_TEST=PASS")
    ...
    def test_formal_generation_creates_one_new_low_sensitive_file(self) -> None:
        with tempfile.TemporaryDirectory(prefix="g8-017-command-") as temporary:
            ...
            self.assertIn("G8_TEST_READONLY_ACCESS_017_COMMAND=PASS", result.stdout)
            ...
            self.assertEqual(duplicate.stdout.strip(), "G8_TEST_READONLY_ACCESS_017_COMMAND=FAILED reason=invalid_request")
    ...
    def test_command_separates_authorization_from_frozen_011_source(self) -> None:
        ...
        self.assertIn("auth_change_id=CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-017", self.command)
    ...
    def test_trusted_windows_path_prefix_runs_from_standard_input(self) -> None:
        ...
        probe = (
            prefix
            + "\nWrite-Output 'G8_017_WINDOWS_PATH_GUARD=PASS'\n"
            + "} finally { $ErrorActionPreference = $g8PreviousErrorActionPreference }\n"
        )
        result = subprocess.run(
            [
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(result.stderr, "")
        self.assertIn("G8_017_WINDOWS_PATH_GUARD=PASS", result.stdout)
    ...
    def test_material_hash_does_not_depend_on_module_autoload(self) -> None:
        ...
        probe = (
            ...
            "  [Console]::Out.WriteLine('G8_017_MODULE_ISOLATED_HASH=PASS')\n"
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(result.stderr, "")
        self.assertIn("G8_017_MODULE_ISOLATED_HASH=PASS", result.stdout)
        ...
        for original, replacement, label in scenarios:
            with self.subTest(label=label):
                ...
                probe = (
                    ...
                    f"  [Console]::Out.WriteLine('G8_017_HASH_{label}_RELEASE=PASS')\n"
                ...
                self.assertEqual(result.returncode, 0, result.stderr)
                self.assertEqual(result.stderr, "")
                self.assertIn(f"G8_017_HASH_{label}_RELEASE=PASS", result.stdout)
    ...
    def test_path_guard_error_cannot_reach_identity_or_ssh_stage(self) -> None:
        ...
        probe = (
            corrupted
            + "\nWrite-Output 'G8_017_IDENTITY_OR_SSH_STAGE_REACHED'\n"
            + "} finally { $ErrorActionPreference = $g8PreviousErrorActionPreference }\n"
        )
        result = subprocess.run(
            [
        ...
        self.assertNotIn("G8_017_IDENTITY_OR_SSH_STAGE_REACHED", result.stdout)
    ...
    def test_remote_block_has_one_manual_sudo_and_only_noninteractive_followups(self) -> None:
        ...
        self.assertIn("G8_TEST_READONLY_ACCESS_PREFLIGHT_017=PASS", command)
        self.assertIn("G8_TEST_READONLY_ACCESS_POSTCHECK_017=PASS", command)
        self.assertIn("G8_017_INSTALL_B64", command)
        self.assertEqual(command.count("G8_017_REMOTE"), 2)
        self.assertIn("/bin/bash -s <<'G8_017_REMOTE'", command)
    ...
    def test_remote_here_doc_has_valid_bash_syntax(self) -> None:
        ...
        remote = self.command.split("/bin/bash -s <<'G8_017_REMOTE'\n", 1)[1].split("\nG8_017_REMOTE\n", 1)[0]
        result = subprocess.run(
            [bash_executable(), "-n"],
            input=remote,
            capture_output=True,
    ...
    def test_embedded_installer_is_exact_and_secrets_are_absent(self) -> None:
        encoded = self.command.split("G8_017_INSTALL_B64'\n", 1)[1].split("\nG8_017_INSTALL_B64", 1)[0]
    ...
    def test_invalid_arguments_fail_closed_without_echo(self) -> None:
        for arguments in (
            ...
            self.assertEqual(result.returncode, 2)
            self.assertEqual(result.stdout.strip(), "G8_TEST_READONLY_ACCESS_017_COMMAND=FAILED reason=invalid_request")
            self.assertEqual(result.stderr, "")
    ...
    def test_future_consumed_gate_precedes_installer_read_and_argument_parse(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-017-consumed-") as temporary:
        ...
        self.assertEqual(result.returncode, 2)
        self.assertEqual(result.stdout.strip(), "G8_TEST_READONLY_ACCESS_017_COMMAND=FAILED reason=change_id_consumed")
        self.assertEqual(result.stderr, "")

#!/usr/bin/env python3
"""验证 020 安装器的冻结来源、no-clobber、最小 sudo 与回滚边界。"""
...
SCRIPT_PATH = Path(__file__).with_name("g8-test-readonly-access-install-020.sh")
...
class TestG8ReadonlyAccessInstall020(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_bash_syntax_and_two_change_ids_are_frozen(self) -> None:
        ...
        self.assertIn("AUTH_CHANGE_ID='CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260815-020'", self.source)
        ...
        self.assertIn("molin-g8-install-CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260815-020", self.source)
    ...
    def test_manifest_gate_accepts_only_frozen_crlf_lines(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-020-manifest-") as temporary:
            root = Path(temporary)
    ...
    def test_async_termination_after_target_creation_removes_live_file(self) -> None:
        ...
        for signal_name, expected_code in (("HUP", 129), ("TERM", 143), ("INT", 130)):
            with self.subTest(signal=signal_name), tempfile.TemporaryDirectory(
                prefix=f"g8-020-{signal_name.lower()}-rollback-"
            ) as temporary:
                root = Path(temporary)
                parent = root / "molin"
                parent.mkdir()
    ...
    def test_second_signal_cannot_interrupt_exit_rollback(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-020-double-signal-") as temporary:
            root = Path(temporary)
            parent = root / "molin"
            parent.mkdir()
            source = root / "source"
    ...
    def test_copy_no_clobber_preserves_existing_target(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-020-copy-") as temporary:
            root = Path(temporary)
            source = root / "source"
            target = root / "target"
    ...
    def test_partial_live_install_is_removed_by_exit_trap(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-020-rollback-") as temporary:
            root = Path(temporary)
            parent = root / "molin"
            parent.mkdir()
            source = root / "source"
    ...
    def test_global_rollback_removes_sudoers_first_and_revalidates(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-020-global-rollback-") as temporary:
            root = Path(temporary)
            parent = root / "molin"
            parent.mkdir()
            auditor = parent / "auditor"
    ...
    def test_auditor_entry_failure_rolls_back_all_live_files(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-020-entry-rollback-") as temporary:
            root = Path(temporary)
            parent = root / "molin"
            parent.mkdir()
            auditor = parent / "auditor"
    ...
    def test_sudo_scope_accepts_only_one_frozen_nopasswd_command(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-020-sudo-scope-") as temporary:
            root = Path(temporary)
            fake_sudo = root / "sudo"
            harness = root / "harness.sh"
            harness.write_text(

...
up_file="${repo_root}/server/migrations/000058_create_ai_gateway_ledger_expand.up.sql"
down_file="${repo_root}/server/migrations/000058_create_ai_gateway_ledger_expand.down.sql"
...
container_name="molin-g1-mysql-000058-${RANDOM}-$$"
...
docker cp "${up_file}" "${container_name}:/tmp/000058.up.sql" >/dev/null
docker cp "${down_file}" "${container_name}:/tmp/000058.down.sql" >/dev/null

apply_up() {
  mysql_exec -e 'UPDATE schema_migrations SET dirty=1 WHERE version=57 AND dirty=0'
  docker exec -e "MYSQL_PWD=${root_password}" "${container_name}" sh -c \
    "mysql --protocol=socket -uroot --database='${database_name}' < /tmp/000058.up.sql"
  mysql_exec -e 'UPDATE schema_migrations SET version=58,dirty=0 WHERE version=57 AND dirty=1'
}

apply_down() {
  mysql_exec -e 'UPDATE schema_migrations SET dirty=1 WHERE version=58 AND dirty=0'
  docker exec -e "MYSQL_PWD=${root_password}" "${container_name}" sh -c \
    "mysql --protocol=socket -uroot --database='${database_name}' < /tmp/000058.down.sql"
  mysql_exec -e 'UPDATE schema_migrations SET version=57,dirty=0 WHERE version=58 AND dirty=1'
}
...
apply_up
assert_scalar "SELECT CONCAT(version,':',dirty) FROM schema_migrations" "58:0" "first_up_version"
...
apply_down
assert_scalar "SELECT CONCAT(version,':',dirty) FROM schema_migrations" "57:0" "down_version"
...
apply_up
assert_scalar "SELECT CONCAT(version,':',dirty) FROM schema_migrations" "58:0" "reup_version"
...
if docker exec -e "MYSQL_PWD=${root_password}" "${container_name}" sh -c \
  "mysql --protocol=socket -uroot --database='molin_g1_drift' < /tmp/000058.up.sql" >/dev/null 2>&1; then

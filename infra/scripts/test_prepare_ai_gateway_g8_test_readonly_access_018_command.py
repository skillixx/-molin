#!/usr/bin/env python3
"""验证 018 命令生成器只生成冻结人工步骤，不读取密码或连接网络。"""
...
SCRIPT_PATH = Path(__file__).with_name("prepare-ai-gateway-g8-test-readonly-access-018-command.py")
INSTALLER_PATH = Path(__file__).with_name("g8-test-readonly-access-install-018.sh")
...
def load_module():
    """从固定路径加载生成器，仅调用离线纯函数。"""
    specification = importlib.util.spec_from_file_location("g8_command_018", SCRIPT_PATH)
...
class TestPrepareG8ReadonlyAccess018Command(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_self_test_is_local_and_exact(self) -> None:
        ...
        self.assertEqual(result.stdout.strip(), "G8_TEST_READONLY_ACCESS_018_COMMAND_SELF_TEST=PASS")
    ...
    def test_formal_generation_creates_one_new_low_sensitive_file(self) -> None:
        with tempfile.TemporaryDirectory(prefix="g8-018-command-") as temporary:
            ...
            self.assertIn("G8_TEST_READONLY_ACCESS_018_COMMAND=PASS", result.stdout)
            ...
            self.assertEqual(duplicate.stdout.strip(), "G8_TEST_READONLY_ACCESS_018_COMMAND=FAILED reason=invalid_request")
    ...
    def test_command_separates_authorization_from_frozen_011_source(self) -> None:
        ...
        self.assertIn("auth_change_id=CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-018", self.command)
    ...
    def test_low_sensitive_stage_reasons_and_ssh_attempt_boundary_are_fixed(self) -> None:
        ...
        stage_contract = (
            ...
            ("ssh_session_failed", "[Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_018_PRE_SSH_GATE=PASS')"),
        )
        ...
        pre_ssh = "[Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_018_PRE_SSH_GATE=PASS')"
        attempted = "[Console]::Out.WriteLine('G8_TEST_READONLY_ACCESS_018_SSH_ATTEMPTED=YES')"
        invocation = "    & $ssh `"
    ...
    def test_frozen_known_hosts_ignores_forged_temp_and_blocks_replacement(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-018-known-hosts-lock-") as temporary:
    ...
    def test_preoccupied_frozen_known_hosts_is_not_removed_or_marked_as_ssh_attempt(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-018-known-hosts-preoccupied-") as temporary:
        ...
        self.assertEqual(
            result.stdout.strip(),
            "G8_TEST_READONLY_ACCESS_018_LOCAL_GATE=FAILED reason=known_hosts_failed",
        )
    ...
    def test_each_stage_exception_is_reduced_to_fixed_low_sensitive_output(self) -> None:
        ...
        for reason in reasons:
            with self.subTest(reason=reason):
                ...
                self.assertEqual(
                    result.stdout.strip(),
                    "G8_TEST_READONLY_ACCESS_018_LOCAL_GATE=FAILED reason=" + reason,
                )
    ...
    def test_encrypted_identity_is_rejected_without_extra_passphrase_prompt(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-018-encrypted-identity-") as temporary:
    ...
    def test_local_material_failure_does_not_disclose_absolute_path(self) -> None:
        ...
        local_gate = self.command.split("# 第二步", 1)[0]
        fake_identity = str(Path(tempfile.gettempdir()).resolve() / "g8-018-sensitive-identity" / "id_ed25519")
        ...
        self.assertEqual(
            result.stdout.strip(),
            "G8_TEST_READONLY_ACCESS_018_LOCAL_GATE=FAILED reason=material_evidence_failed",
        )
    ...
    def test_trusted_windows_path_prefix_runs_from_standard_input(self) -> None:
        ...
        probe = (
            prefix
            + "\nWrite-Output 'G8_018_WINDOWS_PATH_GUARD=PASS'\n"
            + "} finally { $ErrorActionPreference = $g8PreviousErrorActionPreference }\n"
        )
        result = subprocess.run(
            [
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(result.stderr, "")
        self.assertIn("G8_018_WINDOWS_PATH_GUARD=PASS", result.stdout)
    ...
    def test_material_hash_does_not_depend_on_module_autoload(self) -> None:
        ...
        probe = (
            ...
            "  [Console]::Out.WriteLine('G8_018_MODULE_ISOLATED_HASH=PASS')\n"
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(result.stderr, "")
        self.assertIn("G8_018_MODULE_ISOLATED_HASH=PASS", result.stdout)
        ...
        for original, replacement, label in scenarios:
            with self.subTest(label=label):
                ...
                probe = (
                    ...
                    f"  [Console]::Out.WriteLine('G8_018_HASH_{label}_RELEASE=PASS')\n"
                ...
                self.assertEqual(result.returncode, 0, result.stderr)
                self.assertEqual(result.stderr, "")
                self.assertIn(f"G8_018_HASH_{label}_RELEASE=PASS", result.stdout)
    ...
    def test_path_guard_error_cannot_reach_identity_or_ssh_stage(self) -> None:
        ...
        probe = (
            corrupted
            + "\nWrite-Output 'G8_018_IDENTITY_OR_SSH_STAGE_REACHED'\n"
            + "} finally { $ErrorActionPreference = $g8PreviousErrorActionPreference }\n"
        )
        result = subprocess.run(
            [
        ...
        self.assertNotIn("G8_018_IDENTITY_OR_SSH_STAGE_REACHED", result.stdout)
    ...
    def test_remote_block_has_one_manual_sudo_and_only_noninteractive_followups(self) -> None:
        ...
        self.assertIn("G8_TEST_READONLY_ACCESS_PREFLIGHT_018=PASS", command)
        self.assertIn("G8_TEST_READONLY_ACCESS_POSTCHECK_018=PASS", command)
        self.assertIn("G8_018_INSTALL_B64", command)
        self.assertEqual(command.count("G8_018_REMOTE"), 2)
        self.assertIn("/bin/bash -s <<'G8_018_REMOTE'", command)
    ...
    def test_remote_here_doc_has_valid_bash_syntax(self) -> None:
        ...
        remote = self.command.split("/bin/bash -s <<'G8_018_REMOTE'\n", 1)[1].split("\nG8_018_REMOTE\n", 1)[0]
        result = subprocess.run(
            [bash_executable(), "-n"],
            input=remote,
            capture_output=True,
    ...
    def test_embedded_installer_is_exact_and_secrets_are_absent(self) -> None:
        encoded = self.command.split("G8_018_INSTALL_B64'\n", 1)[1].split("\nG8_018_INSTALL_B64", 1)[0]
    ...
    def test_invalid_arguments_fail_closed_without_echo(self) -> None:
        for arguments in (
            ...
            self.assertEqual(result.returncode, 2)
            self.assertEqual(result.stdout.strip(), "G8_TEST_READONLY_ACCESS_018_COMMAND=FAILED reason=invalid_request")
            self.assertEqual(result.stderr, "")
    ...
    def test_unc_and_device_output_paths_are_rejected_before_filesystem_probe(self) -> None:
        ...
        for unsafe in (
            r"\\forged-host\share\g8-018.txt",
            r"\\?\UNC\forged-host\share\g8-018.txt",
            r"\\.\C:\g8-018.txt",
            "//forged-host/share/g8-018.txt",
            r"C:\g8\NUL",
            r"C:\g8\CON.txt",
            r"C:\g8\AUX. ",
            r"C:\g8\COM1.log",
    ...
    def test_future_consumed_gate_precedes_installer_read_and_argument_parse(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="g8-018-consumed-") as temporary:
        ...
        self.assertEqual(result.returncode, 2)
        self.assertEqual(result.stdout.strip(), "G8_TEST_READONLY_ACCESS_018_COMMAND=FAILED reason=change_id_consumed")
        self.assertEqual(result.stderr, "")

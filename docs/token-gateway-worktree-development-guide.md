...
当前 G2 分支：feature/bifrost-ai-gateway-g2（G0/G1 基线分支：feature/bifrost-ai-gateway-v2）
```
...
```text
阶段 Goal 指定分支；当前 G2 为 feature/bifrost-ai-gateway-g2
```
...
→ 检查差异和敏感信息
→ 提交当前阶段 Goal 指定分支（G2：feature/bifrost-ai-gateway-g2）
...
目标分支使用阶段 Goal 明确指定的分支；未指定时才使用基线 feature/bifrost-ai-gateway-v2。

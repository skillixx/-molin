...
非流式成功响应保持 OpenAI 兼容结构。Bifrost 驱动同时检查 HTTP 状态、`error`、`is_bifrost_error`、`choices` 和 JSON 合法性；HTTP 200 携带业务错误仍按失败处理。对外错误统一为受控错误，不透传供应商堆栈。
...
- 每次调用生成独立 `ExecutionAttempt` 元数据；当前不新增 `ai_execution_attempts` 表。

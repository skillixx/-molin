...
后续 G0/G1 增量已通过 `000060_create_ai_gateway_ledger_expand` 新增商业请求账本 Schema 和 Go 模型，但仍未把现有调用链切换为新账本写入。执行驱动契约见本文，账本与阶段出口见 `docs/ai-gateway-g0-g1-contract.md`；两者都不代表 G2 RequestOrchestrator 已实现。
...
- 每次调用生成独立 `ExecutionAttempt` 元数据；G0/G1 的 `000060` 已冻结 `ai_execution_attempts` 持久化表，但当前驱动尚不写入该表，正式落库由 G2 RequestOrchestrator 负责。

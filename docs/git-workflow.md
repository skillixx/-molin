...
开发者创建 feature 分支
  -> 开发完成，本地自测通过
  -> 推送分支，创建 PR
  -> CI 先按精确变更路径分类，再运行适用的构建、测试与 lint
...
✗ 禁止跳过变更分类门禁或任何被分类为适用的 CI（--no-verify）
✗ 禁止在代码中硬编码密钥或密码
...
- 文档 PR 不改代码、不影响构建产物；CI 只运行变更分类、差异格式和敏感信息扫描等轻量门禁。若因账户/CI 基础设施问题（非内容失败）连轻量门禁也无法启动，可经用户确认后由具备权限者合并——**这是第 5/6 节 CI 门槛的唯一例外，且仅适用纯文档 PR**；任何含代码、脚本、配置或工作流改动的 PR 不享此例外。
...
| `web/user-console/**` | 用户控制台构建；命中文字网关页面时追加 G8 真实后端浏览器验收 |
| `web/shared/**` | 管理后台、用户控制台、阶段 5 与 G8 真实后端浏览器验收 |
| 普通 `server/**` | 后端构建与测试 |
| `sms` 模块后端 | 后端构建与测试、阶段 5 短信发布安全门禁 |
| `token_gateway` 相关后端 | 后端构建与测试、G3/G4/G7/G8 及 G8 真实后端浏览器验收 |

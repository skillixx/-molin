...
> G0/G1 说明：`000059` 新商业账本是后端 Expand Schema，当前不改变前端请求和用量查询。G2 切换新读写前，前端不得依赖 `ai_requests`、`ai_usage_items` 或执行模型内部字段；阶段契约见 [`ai-gateway-g0-g1-contract.md`](./ai-gateway-g0-g1-contract.md)。
...
生产环境永不返回 code。既有显式非生产调试模式可能额外返回 `data.code`，但该字段不属于稳定契约；前端类型、页面、
console、埋点和错误上报均不得读取或记录。验证码和过期时间均由服务端生成，前端不得生成 code 或自行决定有效期。
...
### 5.2.8 公开邮件发码来源 IP（前端不传来源头，Phase 1 delta 待 QA/PM 签署）

公开邮件验证码发码的来源 IP 由应用与受信反向代理判定。浏览器、用户控制台和管理后台均不得主动发送、复制或信任 `X-Real-IP`、`X-Forwarded-For`、`Forwarded`；这些 Header 不是前端参数，也不能用于规避 IP 限流。
...
- Header 拒绝不消耗发码限流次数，不进入邮件业务服务，Adapter 调用为 0；来源解析器运行时不可用固定 `503/51003「邮件发送服务未就绪」`，同样无副作用。
- 应用永不使用 XFF 做安全判定；Nginx 覆盖 `X-Real-IP=$remote_addr` 并删除 XFF/Forwarded。前端遇到 403 或 503 只展示后端安全文案，不自行构造来源 Header 重试。

该 delta 尚待 QA/PM 书面签署，且未完成 Go、Nginx、DirectMail 或 E2E 验收；不得标记为前端或功能实现完成。

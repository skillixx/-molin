...
- 生产 Compose 把 API、Bifrost 节点和 LB 的环境文件分离：节点只获得加密密钥与两个上游密钥，LB 只获得内部 Token。API↔LB、LB↔节点分别使用两个内部专网，API 和监控只能经 LB 访问，无法直连节点；节点的公网出站使用仅 Bifrost 可加入的独立网络，不与公开前端共享。
- 隔离生产形态预演实际注册 Token 模块并使用临时 JWT 请求文字接口，关闭态必须返回 HTTP 503 与业务码 50330；双 Fake Bifrost 节点同时验证 LB 未授权拒绝和授权转发，避免以匿名 404 代替总闸证据。
- Nginx 边缘 kill switch 默认阻断公开文字、工作台 Agent 和有状态会话调用入口；预演在候选应用、旧版回滚边缘和候选恢复三个阶段分别验证 50330，避免旧版不识别应用总闸时重新开放流量。
- 边缘关闭响应保持 `code=50330`、`error_type=ai_gateway_traffic_closed` 和 Nginx 请求 ID；隔离预演对四类入口逐项验证候选与旧版回滚关闭态。

...
- `CHG-G8-TEST-READONLY-STAGING-EVIDENCE-20260812-006` 正在准备仓库候选：复用并冻结 004 的 inode/fd/最终目录项竞态安全读取算法，把早期失败细分为六类固定门禁枚举，避免再次退化为宽泛 `remote_evidence_failed`。当前未连接测试服，尚未完成独立评审、QA、产品、精确 HEAD CI、合并或用户授权，不得把候选当作暂存 UNKNOWN 已关闭。

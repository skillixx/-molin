...
- 024 使用 `CHG-G8-TEST-READONLY-SSH-DIAGNOSTIC-20260816-024` 设计最小 SSH 连接诊断候选。候选不恢复 023，不运行 Docker 审计；唯一远端能力是固定 `printf` 回执，并用固定低敏分类区分认证、host key、连接超时、拒绝、网络不可达、其他传输和远端探针失败。当前为 `PENDING_ENGINEERING_REVIEW / REMOTE_NOT_AUTHORIZED`，仅允许本地离线工程、CI、评审和合并；尚未执行 024 或任何测试服操作，不能据此关闭测试服运行态或 `G8_SOFTWARE_CLOSED_LOOP`。

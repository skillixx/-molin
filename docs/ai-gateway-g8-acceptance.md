...
- 023 使用 `CHG-G8-TEST-READONLY-RUNTIME-AUDIT-DROP-20260815-023` 改用开发机现有免交互 SSH 认证链，不再要求固定客户端私钥、公钥或客户端指纹；仍固定目标和服务器 host key，并保持 BatchMode、单会话、零重试和只读远端脚本。当前仅完成工程候选设计与离线门禁，状态为 `PENDING_ENGINEERING_REVIEW / REMOTE_NOT_AUTHORIZED`；不得据此认定测试服运行态或 `G8_SOFTWARE_CLOSED_LOOP` 已完成。

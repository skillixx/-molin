...
- `CHG-G8-TEST-READONLY-STAGING-EVIDENCE-20260812-004` 已完成 PR #341、CI run `31597619593` 12/12、独立安全/QA/产品验收，并以 merge commit `7e27e88b1c4b63630a0be346e41226c449d033e4` 合并。包装器只允许一次固定 SSH，以隔离 Python 读取固定主机身份、部署根及 003 暂存路径；不存在返回 `ABSENT`，存在且五文件类型/属主/权限/大小/摘要全部匹配返回 `PRESENT/PASS`，存在但任一项不一致返回固定低敏 `PRESENT/MISMATCH` 并以退出码 3 阻断后续动作。当前仍未获得用户执行授权、未再次连接测试服；测试服暂存状态和原运行态 P1/UNKNOWN 尚未关闭。

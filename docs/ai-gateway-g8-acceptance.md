...
- 018 以 `CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-018` 建立全新候选，017 保持永久墓碑。018 在不降低既有 SSH、known_hosts、sudo、no-clobber 和信号回滚控制的前提下，将本地失败收敛为六类固定低敏原因，并以 `PRE_SSH_GATE=PASS` 和紧邻唯一 SSH 调用的 `SSH_ATTEMPTED=YES` 明确 SSH 到达边界。PR #388 已通过 Windows、断网 Linux、全部适用 CI 与独立三方零缺陷复评，并以 merge commit `ef9f65f851cc657eaa6fba7df866ba3c4c7a0912` 合入主干；合并后原始 Git blob 与冻结命令摘要复核一致。当前为 `PENDING_USER_APPROVAL / REMOTE_NOT_AUTHORIZED`，尚未连接测试服或安装，必须等待新的独立精确安装授权。

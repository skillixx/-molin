...
- 011 交互 sudo 方案仅处于仓库工程门禁阶段：新候选绑定 `DROP_SSH_INTERACTIVE_SUDO`，未来仅允许一次独立 SFTP 和一次交互 SSH；sudo 密码只能由操作者响应 `sudo -k -v` 的 TTY 提示，绝不进入仓库资产。本轮没有连接测试服或执行安装，状态为 `PENDING_ENGINEERING_GATES_AND_USER_APPROVAL`；工程合并仍不构成执行授权。

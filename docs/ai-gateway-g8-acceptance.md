...
- 014 结果与墓碑已由 PR #379 按 merge commit `97ee6037cafa90577be619fc67e78866c4d75efe` 合入 main。015 正在准备独立的最小只读入口安装候选：只允许复核 011 暂存后通过一次人工 sudo 认证创建两个 root-owned 工具和一条精确 sudoers，并执行固定 self-test；当前仍为 `REMOTE_NOT_AUTHORIZED`，未连接、未安装，也不关闭任何运行态 UNKNOWN/P1。见 `docs/ai-gateway-g8-test-readonly-access-install-authorization-20260814-015.md`。

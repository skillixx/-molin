...
- 021 使用 `CHG-G8-TEST-READONLY-RUNTIME-AUDIT-DROP-20260815-021` 建立固定启动器候选：保留 020 的无安装、无 sudo、固定 Docker 只读能力，但把工程 merge blob 复核、命令冻结、PowerShell 5.1 解析与启动合并为仓库内唯一入口，禁止临时外层包装。当前仅完成工程实现，状态为 `PENDING_ENGINEERING_REVIEW / REMOTE_NOT_AUTHORIZED`；尚无新的测试服运行态证据，不得据此认定软件闭环完成。

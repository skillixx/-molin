...
- 013 仓库候选把可重复本地诊断与一次性远端授权完全拆分。本地诊断器无 ChangeId，不含 SSH/SFTP 或远端命令；013 不再提供 `--local-check`，未来只有独立用户授权后的单次只读 SSH。当前状态为 `PENDING_ENGINEERING_GATES_AND_USER_APPROVAL`，未读取真实身份材料、未连接测试服，011 暂存继续为 `UNKNOWN`。本 PR 仅用状态机和验收矩阵规划后续清理、审计入口安装、测试候选部署和运行态审计，不改变测试服 P1=3、其他 UNKNOWN 或商业验收状态。

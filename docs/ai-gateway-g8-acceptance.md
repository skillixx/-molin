...
- 015 工程候选由 PR #380 合入 main 后取得用户独立授权，但唯一人工本地段出现可信 Windows 路径正则错误；该错误默认非终止，身份材料读取、SSH、sudo、安装和远端影响均缺少可绑定证据，保持 `UNKNOWN`，重试 0，015 已消费并墓碑化。016 修复正则并把 PowerShell 错误停止/恢复写入真实生成段，继续以完整 011 暂存为唯一输入；PR #381 最终 HEAD `04abab350afc3163a66dd5faee86005e7c2b02c3` 已通过 CI run `31776890876` 和独立复评，并按 merge commit `2f407cbf3a9c5fea987eeb2f82ebb41630db9e35` 合入 main。合并后冻结 Git blob 与生成命令摘要复核一致，当前为 `PENDING_USER_APPROVAL / REMOTE_NOT_AUTHORIZED`；尚未安装，不关闭任何运行态 UNKNOWN/P1。见 `docs/ai-gateway-g8-test-readonly-access-install-attempt-20260814-015.md` 和 `docs/ai-gateway-g8-test-readonly-access-install-authorization-20260814-016.md`。

...
- 018 以 `CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-018` 建立独立候选并完成 PR #388、CI、三方复评、main 合并和原始 blob 复核。用户独立授权后的唯一人工本地段窗口直接关闭且没有可见输出；SSH 启动/连接只能记为 `UNKNOWN / 最多 1`，远端固定段、sudo、安装器、post-check、业务、上游和费用均为 0。018 已失败关闭消费并墓碑化。019 使用 `CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-019`，以唯一 `ssh -tt` 自动携带无秘密 Base64 远端脚本并把 TTY 留给一次 sudo 提示，父 PowerShell 不主动退出。019 已通过 PR #390、CI run `31829691838` 和三轴独立评审，以 merge commit `70485d893fd86db00be4dbb9e324f9d4322d55b0` 合入 main；合并后原始 Git blob 与冻结命令摘要复核一致。当前为 `PENDING_USER_APPROVAL / REMOTE_NOT_AUTHORIZED`，没有执行 019 的远端动作，测试服安装仍未确认。

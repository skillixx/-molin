...
- 修复候选改为从 Windows 系统 API 获取可信 `SystemRoot` 与公共数据目录，拒绝调用方伪造的相对或 UNC 环境路径；真实本地材料诊断复测 PASS。旧 013 授权清单失效且入口已墓碑化，任何调用都在解析、材料和联网前返回 `change_id_consumed`。014 已由 PR #377 按 merge commit `3c9c5cf489b28e45b789c114243e45936a0d81d2` 合入 main，精确 HEAD CI 与独立复评均通过，合并后摘要无漂移；当前处于 `PENDING_USER_APPROVAL / REMOTE_NOT_AUTHORIZED`，仍须用户对独立清单再次明确批准才可执行一次远端只读 SSH。

...
- 020 使用 `CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260815-020` 建立新的独立工程候选：以可信用户目录耐久低敏回执保留阶段证据，规范化 `Null` ActionPreference，并在交互 sudo 前只读判定 live 为 `EXACT / ABSENT / DRIFT`。当前状态为 `PENDING_ENGINEERING_REVIEW / REMOTE_NOT_AUTHORIZED`；本轮不授权任何 SSH、sudo、安装器、post-check 或测试服操作，不能据此认定测试服已安装或软件闭环完成。

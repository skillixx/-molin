...
- 018 以 `CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-018` 建立全新候选，017 保持永久墓碑。018 在不降低既有 SSH、known_hosts、sudo、no-clobber 和信号回滚控制的前提下，将本地失败收敛为六类固定低敏原因，并以 `PRE_SSH_GATE=PASS` 和紧邻唯一 SSH 调用的 `SSH_ATTEMPTED=YES` 明确 SSH 到达边界。本地 Windows 与断网 Linux 门禁已通过，当前仅为 `PENDING_ENGINEERING_REVIEW / REMOTE_NOT_AUTHORIZED` 工程状态，尚未连接测试服或安装；其合并后仍须原始 Git blob 与冻结命令摘要复核，并等待新的独立精确安装授权。

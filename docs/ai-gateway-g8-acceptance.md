...
- `CHG-G8-TEST-READONLY-STAGING-EVIDENCE-20260812-004` 正在完成仓库工程门禁。候选包装器只允许一次固定 SSH，以隔离 Python 读取固定主机身份、部署根及 003 暂存路径；不存在返回 `ABSENT`，存在且五文件类型/属主/权限/大小/摘要全部匹配返回 `PRESENT/PASS`，存在但任一项不一致返回固定低敏 `PRESENT/MISMATCH` 并以退出码 3 阻断后续动作。当前尚未获得执行授权，未再次连接测试服；本候选不允许 SFTP、下载、删除、sudo、Docker、数据库、队列、服务或业务请求。测试服暂存状态和原运行态 P1/UNKNOWN 尚未关闭。

...
- 023 使用 `CHG-G8-TEST-READONLY-RUNTIME-AUDIT-DROP-20260815-023` 改用开发机现有免交互 SSH 认证链，不再要求固定客户端私钥、公钥或客户端指纹；仍固定目标和服务器 host key，并保持 BatchMode、单会话、零重试和只读远端脚本。工程 HEAD `9a969d4dd2881e659c50ab694a4d35b57adba803` 已经 PR #404、CI run `31892659673 completed/success` 和三项独立零缺陷评审，以 merge commit `1eb23c8b87720cceea64dcfc349b0a9b9c04de4b` 合入 main；合并后原始 blob、LF 与冻结命令摘要复核一致。当前状态为 `PENDING_USER_APPROVAL / REMOTE_NOT_AUTHORIZED`，023 尚未执行、尚未消费；不得据此认定测试服运行态或 `G8_SOFTWARE_CLOSED_LOOP` 已完成。

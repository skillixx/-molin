...
- 022 使用 `CHG-G8-TEST-READONLY-RUNTIME-AUDIT-DROP-20260815-022` 修复 021 的耐久回执目录校验。离线复现证明旧普通 Python 字符串把 PowerShell 正则末尾反斜杠折叠为无效表达式；新候选改用可信 LocalApplicationData、固定盘、逐级非 reparse、唯一 GUID CreateNew，并区分目录/权限、预占、写入和刷盘故障。工程 HEAD `fc0344283813bd873aa70520e0b8fcd1da424500` 已经 PR #401、CI run `31884793587` 和三项独立评审，以 merge commit `84ae5b0ad87958ee63fbfa709c4f164baca39a1b` 合入 main，合并后摘要复核一致。当前状态为 `PENDING_USER_APPROVAL / REMOTE_NOT_AUTHORIZED`；022 尚未执行、尚未消费，不形成测试服运行态证据。

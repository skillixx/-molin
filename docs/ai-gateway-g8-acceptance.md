...
- `CHG-G8-TEST-READONLY-HOST-IDENTITY-DIAG-20260812-007` 正在完成仓库工程门禁。候选只读取固定 `/etc/machine-id` 最多 4097 字节，在内存比较既有批准摘要，只输出 `READABLE_MATCH`、`READABLE_MISMATCH` 或 `UNREADABLE`；不得输出当前原文或摘要，不得自动修改批准基线，也不得读取 003 暂存目录。当前未连接测试服，授权清单为 `PENDING_ENGINEERING_GATES_AND_USER_APPROVAL`；代码、测试、CI 或合并均不等于取得一次只读 SSH 授权。

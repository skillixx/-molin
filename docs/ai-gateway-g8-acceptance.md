...
- 012 是尚未执行的 Drop 暂存只读取证工程候选，只检查固定 011 暂存的存在性及五文件、manifest、回执完整性；不读取物理主机身份，不具备上传、下载、sudo、业务或生产能力。当前为 `PENDING_ENGINEERING_GATES_AND_USER_APPROVAL`，没有运行 local-check 或 SSH，011 继续为 `UNKNOWN`。工程门禁不关闭 API、schema、数据库、Bifrost、监控、备份或账务事实，也不授权清理、安装或运行态审计。

...
14. 022 使用新的独立 ChangeId `CHG-G8-TEST-READONLY-RUNTIME-AUDIT-DROP-20260815-022`。离线动态复现确认 021 回执目录正则在普通 Python 字符串中发生反斜杠折叠，生成的 PowerShell 正则无效；022 删除该正则，改用 LocalApplicationData、固定盘与逐级非 reparse 校验，并在唯一 GUID CreateNew 文件中先耐久写入 `STARTED`、`PRE_SSH_GATE` 和 `SSH_ATTEMPTED`。目录/权限、预占、写入和刷盘故障各有固定低敏原因。PR #401 已以 merge commit `84ae5b0ad87958ee63fbfa709c4f164baca39a1b` 合入 main，合并后摘要复核一致；当前状态为 `PENDING_USER_APPROVAL / REMOTE_NOT_AUTHORIZED`。未取得新的独立精确授权前，禁止正式入口、SSH 或任何测试服能力。

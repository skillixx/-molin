...
001 至 011 均已消费。008 曾把固定 003 暂存状态收敛为 `ABSENT`；后续 010 暂存成功但 root 安装未开始，011 的唯一正式暂存包装器调用又以低敏 `invalid_request` 停止，当前 011 暂存状态为 `UNKNOWN`。012 已获设计与实现授权，但仍处于工程门禁阶段，禁止 local-check 和测试服连接。Drop 映射下旧的物理主机身份核验顺序不再适用；当前必须依次完成：

1. 禁止重放 011 或通过新的本地诊断推断远端状态；包装器、命令生成器和候选生成器必须保持消费态。
2. 先完成 012 的代码安全、QA、产品、精确 HEAD CI 和 merge commit；这些仓库证据不构成执行授权。

...
15. 023 使用新的独立 ChangeId `CHG-G8-TEST-READONLY-RUNTIME-AUDIT-DROP-20260815-023`，移除固定客户端私钥、公钥和指纹配对门禁，使用开发机现有 OpenSSH 免交互认证链；`-F none`、固定目标、冻结 known_hosts、服务器 host key、BatchMode、单会话和零重试不变。工程阶段仅运行 PowerShell 5.1 假 SSH 与 Linux 断网测试；当前 `REMOTE_NOT_AUTHORIZED`，禁止执行正式入口或连接测试服务器。

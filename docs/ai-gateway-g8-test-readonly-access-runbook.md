...
15. 023 使用新的独立 ChangeId `CHG-G8-TEST-READONLY-RUNTIME-AUDIT-DROP-20260815-023`，移除固定客户端私钥、公钥和指纹配对门禁，使用开发机现有 OpenSSH 免交互认证链；`-F none`、固定目标、冻结 known_hosts、服务器 host key、BatchMode、单会话和零重试不变。工程 HEAD `9a969d4dd2881e659c50ab694a4d35b57adba803` 经 PR #404、CI run `31892659673 completed/success` 和三项独立零缺陷评审后，以 merge commit `1eb23c8b87720cceea64dcfc349b0a9b9c04de4b` 合入 main，合并后摘要复核一致；当前状态为 `PENDING_USER_APPROVAL / REMOTE_NOT_AUTHORIZED`。未取得新的独立精确授权前，禁止执行正式入口、SSH 或任何测试服能力。

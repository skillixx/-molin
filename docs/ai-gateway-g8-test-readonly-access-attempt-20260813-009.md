...
继续安装准备必须使用新 ChangeId，并先完成以下仓库工程门禁：

1. 设计 Windows 私钥冻结方式，必须同时保持内容、密钥对和严格 NTFS ACL；不得回退为调用方可替换路径、SSH Agent、AskPass 或隐式密钥发现。
2. 增加真实 Windows 动态测试，证明冻结私钥可由固定 `ssh-keygen -y` 读取，且宽松 ACL、密钥替换和摘要漂移均失败关闭。
3. 新包装器仍须保持一次本地检查、一次 SSH、一次 SFTP、零重试和有界低敏输出，并完成独立代码安全、QA、产品/规格、精确 HEAD CI 与 merge commit。
4. 工程门禁全部通过后，仍需用户对新 ChangeId 重新独立授权；不得复用 009 候选、回执或执行授权。

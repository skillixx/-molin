...
2. **分页结构**：后端甲（一~五章）已 **D-95 扁平化**，`data` 顶层即 `{ items, page, page_size, total }`，**没有** `pagination` 子对象、**没有** `list` 字段。类型层据此定义：
   ...
   ```
   > ⚠️ 后端乙（六~八章：商品/订单/钱包）暂未扁平化，仍是嵌套 `pagination`。前端乙若同时对接乙模块需注意区分，**不要写一个全局假设**。
...
| B1-2 | 类型 | `PageResult<T>`（注意区分甲扁平 / 乙嵌套） | 见 §1.2 注 | 0.5d |
...
2. **甲/乙分页结构不一致**：甲扁平、乙（商品/订单/钱包）仍嵌套，前端乙若跨对接乙模块切勿用同一假设。

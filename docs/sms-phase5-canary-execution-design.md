...
6. 本地生成器已经补充失败输出顺序回归：未来 runner 必须先输出远端低敏结果和精确退出码，再失败关闭。新 ChangeId `20260805T164138Z` 修正版计划和 runner 已按独立授权在仓库外生成并通过静态验证；生成授权不等于执行授权。
7. 只有新候选按完整 SHA-256 取得独立批准并执行，且账号状态、管理员直接角色/权限、双目标白名单和关闭态均通过，才可另行批准五场景真实执行；任何本地生成、只读预检或白名单变更授权均不得继承为短信发送授权。
...
`d00ff59ab40d23b20fb350557cd436db0cd86641553dc76ea3afc7e868687f34`。独立验证确认计划仍为 `receipt_only`、五场景各一次、总量 5、零重试；PowerShell/Bash 语法、默认关闭、合成值、只读 SQL、固定身份契约、失败证据输出顺序和敏感字面量均通过。生成与复验期间实际手机号输入、测试服连接、上传、业务 POST、配置修改、邮件和短信均为 0。该候选尚未取得执行授权。

...
> 当前状态：`CONSUMED_LOCAL_SNAPSHOT_FAILED`。PR #358 精确 HEAD `2efb809ba090c9af780d8c6be2f75ee707b92d6b` 已通过 CI run `31665135810` 12/12 和独立代码安全、QA、产品/规格 P0/P1/P2=0，并按 merge commit `1f0c2d11dc705be9496eb18c73688d21ee0e8ab5` 合入主干。用户随后批准执行 009；唯一一次本地检查 PASS，唯一正式调用在 Windows 冻结私钥副本的 NTFS ACL 门禁处以 `invalid_request`、退出码 2 停止，且离线证据证明发生在 SSH/SFTP 之前。009 与本清单全部命令均已消费、作废并禁止重放；完整记录见 `docs/ai-gateway-g8-test-readonly-access-attempt-20260813-009.md`。
...
以下步骤仅保留为 009 历史授权计划，当前全部禁止执行。009 的唯一正式调用已经消费；不得因修复本地 ACL、候选仍存在或再次取得口头确认而重放，继续必须使用新 ChangeId：

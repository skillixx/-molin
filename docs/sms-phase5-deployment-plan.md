...
本文是可执行计划，不代表已授权执行。测试服部署、远端配置变更、服务重启、真实短信、生产部署、生产开关、
Git 推送/PR/合并必须分别取得项目负责人明确授权。默认状态始终为 `SMS_ENABLED=false`。
...
当前候选代理网络为 `172.20.250.0/28`，管理端和用户端计划固定为 `.2`、`.3`。该值必须在部署窗口
重新对照 `ip -4 route show` 和全部 `docker network inspect`；只允许在确认无重叠后创建精确 `/28` 网络，
不得信任默认 `172.17.0.0/16`、整个 RFC1918、VPC 网段或 `0.0.0.0/0`。部署前可执行：

...
| 产品内聊天 | `POST /api/agents/{id}/chat` | 登录态 / sk | 门面**有状态 tool-use 编排** | 门面自动执行 |
...
**M4（Week 9）｜整合验收**
- 测试：两路径端到端 + 三计费 + tool-use 多轮回归 + 并发安全（worktree 隔离）
- PM：第二阶段业务验收
...
| # | 项 | 结论/建议 |
|---|---|---|
| 1 | 计费方式 | ✅ 按量 + 按次 + 套餐 全上（2026-06-20） |
| 2 | sk 开放 | ✅ 本期开放（2026-06-20），加按 sk 限流 + 余额阈值防透支 |
| 3 | 概念分层 | ✅ Agent/Skill/插件三层（2026-06-20），角色=Agent |
| 4 | 多模态范围 | ✅ 本期仅 chat（2026-06-20），图片/视频→第三阶段 |
| 5 | 创建权 | ✅ 运营配置 + 用户自建 Agent（2026-06-20） |
| 6 | 按次计费循环口径 | 建议：按用户提问计 1 次（非上游轮数）；待 PM 终确认 |
| 7 | tool-use 最大轮数 | 建议默认 5，可配置；防失控成本 |
| 8 | 插件安全 | 外部插件 endpoint 需白名单/超时/SSRF 防护，凭证加密；列 M3 安全评审项 |
| 9 | 套餐额度单位 | 建议 token 数（与计费同维度），余额耗尽即拒；待 PM 确认 token 数 vs 金额 |

**红线复用提醒**：权限码必建 seed；上游/插件凭证 AES-256-GCM、响应不返回；sk/refresh 只存 HMAC、明文只回一次、支持吊销；后端字段变更同步前端契约；并发多 actor 用 worktree 隔离。

---

## 10. 下一步

1. 同步更新 `backend-sk-auth-contract.md`（套餐进场→sk 支持 prepaid）与 `frontend-api-reference.md` §14（套餐/按次/Agent chat 契约）—— 本轮随路线图一并修订。
2. 待 PM 确认 §9 #6/#7/#9 后，可产出：
   - 套餐额度扣减对接契约（后端丙）
   - `agent`/`skill`/`plugin` 三模块迁移 SQL + 接口契约 + tool-use 编排实现规范（后端丁）
   - 按次计费规则 seed + 门面次数事件上报（乙+丁）

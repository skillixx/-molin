...
本目录定义墨灵项目的系统级 Agent。Agent 表示项目角色，不绑定具体 AI 工具；Codex、Claude 或其他经批准的工具都可以承担任一角色。Codex 可以替代 Claude 执行后端开发，但仍必须遵守所承担角色的职责边界。

每个 Agent 只在自己的职责边界内工作。用户明确要求全栈或跨角色开发时，执行工具必须先声明角色切换，并通过接口文档、任务单或评审意见完成阶段交接，禁止在没有契约和验证的情况下混改多个角色负责的业务代码。

...
观察快照 ChangeId `20260806T060345Z` 随后按 runner SHA-256 `9b89eab7bde8461d3002422f58672ce4adf2b318e0c4c961423d8f6139faa636` 的限定授权执行 5m、15m、30m、2h 四个窗口，每个窗口各执行一次、各建立一次固定 SSH stdin 连接且没有重试。窗口名称表示“Canary 完成后至少经过该时长”的门禁；实际分别在 1812、1821、1830、13965 秒采集。四次均返回 health/ready 200、累计发送 `21/20/1`、当前进程 Provider 调用/非受理为 `0/0`、活动 SMS/Alertmanager 告警 0、通知失败增量 0，并且配置修改、服务信号/重启、业务 POST、邮件和短信均为 0。快照 SHA-256 依次为：5m `80a84638ba2412acf7eda15ee5ba9d2f5263a578353afe96da001ada0a27bf78`、15m `13ccd424716feaf8703e557522078711397552db3b4ff95a0549f50a68be9320`、30m `7833cc5252c3f560993f0b4f667357fbe8c080ebe4f2b1c62e6a8d8886ab4e8a`、2h `1c14fed55aba8fdbb5b125baf30c69826c988a180d475de37b30a75c6feb3cc6`。仅 24h 窗口尚未执行，禁止把前四个快照解释为完整观察通过。

补充观察 ChangeId `20260806T131125Z` 随后按 runner SHA-256 `09edeef7fbd3f76fba2c2094d7f9b4f874f91b1c965a4e050e2db078796ef441` 单次执行 2h 最小窗口并通过，实际采集时已距 Canary 完成 75594 秒。结果仍为 health/ready 200、累计发送 `21/20/1`、当前进程 Provider `0/0`、活动 SMS/Alertmanager 告警 0、通知失败增量 0，且配置修改、服务操作、业务 POST、邮件、短信提交和真实短信均为 0；快照 SHA-256 为 `f99749b50c0109cc5ef268921c6badd54392e846aab73ab30154520d346f88f6`。该独立快照只作为补充观察，不进入五窗口连续前缀或最终证据组装，也不得替代 24h 快照。
...
为让未满五档的观察也能重复验证，新增纯离线观察进度验证器。它复用最终组装器的摘要、敏感字段和快照结构门禁，只接受从 5m 开始的连续窗口前缀，核验时间严格递增、持久发送始终为 `21/20/1`、恢复后当前进程 Provider `0/0` 不增长、health/ready 200、零活动告警和零通知失败。3 项契约通过；使用源 Canary 结果及 5m/15m/30m/2h 四份真实完整摘要执行得到 `phase5_observation_progress=passed`、`through_window=2h`、`snapshots_verified=4`，网络连接和真实短信均为 0。

五窗口观察快照候选 ChangeId `20260806T060345Z` 已绑定同一源结果生成，runner SHA-256 `9b89eab7bde8461d3002422f58672ce4adf2b318e0c4c961423d8f6139faa636`。同一 runner 覆盖 5m/15m/30m/2h/24h，每个窗口只允许创建一个结果，拒绝提前执行且内部不 sleep。PowerShell、默认关闭和 SelfTest 通过；5m/15m/30m/2h 已各执行一次并生成结果，24h 仍须等待时间门禁和独立授权。

原 runner 的 24h 窗口在时间门禁到达后按一次性授权执行：固定 SSH 连接仅一次且未重试，远端退出码为 0，但本地 wrapper 因 stderr 非空而失败关闭，退出码为 1，未创建 24h 快照。原失败路径没有输出白名单 stdout 和 stderr 布尔值，故禁止根据远端退出码推定观察通过。生成器已补充失败低敏摘要：任何 stderr 仍阻断快照创建，但抛错前输出已验证白名单行、`remote_stderr_present`、`readonly_exit_code` 和单连接计数，不输出 stderr 内容。新候选 ChangeId `20260807T061605Z`、runner SHA-256 `87b0a38f14c10161d7f58fa52a733f96537daeaca5fa2004276681ac2b80bd28` 已离线生成，默认关闭、SelfTest 与 3 项观察候选契约通过，外部连接和消息发送均为 0；尚未获准执行。

该修正版随后单次执行并完整输出低敏结果：`observation_snapshot=passed`、窗口 24h、实际经过 87693 秒、health/ready 200、累计发送 `21/20/1`、Provider `0/0`、零告警、零通知失败和零副作用；唯一阻断仍为 `remote_stderr_present=true`，因此没有创建快照。检查确认 SSH 传输参数未显式使用 `-T`，与 OpenSSH 的非终端 stdin 警告模式一致。第三版候选现固定 `-T` 与 `NumberOfPasswordPrompts=0`，仍要求 stderr 绝对为空；ChangeId `20260807T062052Z`、runner SHA-256 `b4ca5febc35b6af93ebd03120148d57bce24e558141a8c0ea8d7bd7741df378e` 已离线通过验证，尚未执行。

第三版单次执行同样证明 24h 观察字段和零副作用全部通过，但 stderr 仍非空，因此 `-T` 没有关闭失败门禁且不得推定根因。后续候选使用隔离远端环境和无启动文件 Bash，仍对任意 stderr 失败；若失败，仅输出固定分类、计数和不可逆 SHA-256，不输出正文。中间 ChangeId `20260807T064511Z` 已在执行前由自测更完整的候选替代；当前候选 ChangeId `20260807T064628Z`、runner SHA-256 `8cb7068967f1993351b07b874d33a162f663f25fa3007c4db11a4682c46c4506` 已离线通过，尚未取得执行授权。

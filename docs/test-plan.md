...
- G8 工程就绪要求独立规格、代码安全、QA、产品对同一 PR HEAD 签署，P0/P1=0；CI 变更范围分类与该 HEAD 的全部适用门禁必须成功，未命中重型 Job 只能由失败关闭分类器标记为 `skipped`，随后按 merge commit 合并。G8 核心代码、脚本、基础设施或工作流变更始终触发完整 G8 回归。
- G8 商业验收另要求至少四周观察、3 家设计客户、2 家真实 API 集成、1 家自有资金真实付费连续使用、成功率不低于 99.5%、正常账单差额持续为 0、毛利达标和凭据回收完成。
...
- 测试服只读入口候选包必须从精确 Git 提交归档生成，以固定 Go 环境连续构建两次并要求摘要一致；输出目录必须全新且失败不留半成品，包内不得包含 Secret、SSH/SFTP、sudo、安装、Docker 或服务控制逻辑。生成入口只能接受当前 003、冻结来源提交和全新绝对目录；已消费 001/002 的普通生成必须失败关闭，历史复现只能按精确 ChangeId 在临时目录完成并自动销毁。暂存包装器必须先离线核对候选、known_hosts、显式密钥对，再调用固定 OpenSSH 一次完成只读预检；只有完整 PASS 才调用固定 SFTP 一次，以原子 `mkdir` 创建全新暂存目录后上传五文件。远端脚本固定绝对命令和环境，不得使用易受跨 shell 引号影响的 `cut`/`awk`；任一步失败、超时、stderr、额外 stdout、身份或权限漂移均低敏失败且零重试。CI 必须使用 `python -I` 执行两套单测、语法、自检、当前候选真实双构建、五文件白名单、`SHA256SUMS`、冻结摘要及 001/002 历史复现；验证 PASS 不等于远端安装或测试服验收。

...
| SMS-P5-06 | 告警规则 | 失败率、配置异常、网络异常和平均延迟规则由 promtool/Prometheus 加载 | 同版本 promtool 语法与阈值正反例通过，运行实例加载 4 条且抓取目标 up；通知链路演练待授权窗口 |
| SMS-P5-07 | 测试服 Canary | 白名单、总上限 10 条、五入口、OTP 单次消费、结束恢复开关 | 待单独授权 |
| SMS-P5-08 | 页面四宽度 | 1440/1024/768/390 的权限、MFA、模板、日志及五态可操作 | 本地真实浏览器无横向溢出、控制台错误 0；测试服 5 个管理 GET 与代理链已通过，浏览器认证后的真实数据显示仍待复验 |
| SMS-P5-09 | 回滚 Dry Run | 先关开关、保留证据、默认不 down migration；无外部副作用 | 本地 test/production 两套通过；测试服备份完整，应用实际回滚演练待独立授权 |

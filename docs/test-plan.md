...
| SMS-P5-09 | 回滚 Dry Run 与恢复材料 | 先关开关、保留证据、默认不 down migration；固定备份可验证且业务配置零变更，允许只读访问审计日志增长 | 本地 test/production 两套通过；测试服材料、容器快照和引用镜像完整。旧环境整份恢复失败关闭；候选生成器行为测试证明会保留代理、关闭短信、删除废弃键并排他写入，但远端生成未执行。静态前置、恢复运行时及实际回滚演练均待独立授权完成 |

...
| SMS-P5-06 | 告警规则与通知链 | 失败率、配置异常、网络异常和平均延迟规则由 promtool/Prometheus 加载；通知接收链可达 | 规则部分通过：同版本 promtool 阈值正反例、运行实例 4 条规则和抓取目标 up；通知部分未就绪：Alertmanager 引用/容器/进程/9093 均为 0，需先授权接收渠道与关闭态部署，再授权触发演练 |

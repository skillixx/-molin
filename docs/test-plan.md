...
- G8 当前以 `G8_SOFTWARE_CLOSED_LOOP` 作为软件任务完成门禁：测试候选运行态无阻断性 UNKNOWN，管理员发布到用户调用、计量、扣费、账单、申诉、对账和回滚全链路通过，正常账单差额为 0，测试工程师与产品经理签收且 P0/P1=0。商业观察周期、设计客户、真实集成、真实付费、生产成功率和毛利改为后续 `G8_COMMERCIAL_ACCEPTED` 独立任务确认，不阻塞软件闭环。

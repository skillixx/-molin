...
- 009 已消费后不得恢复任何入口。010 是唯一活动候选，manifest 必须为 `TARGET_TRANSPORT=DROP_SSH_DIRECT`，并精确绑定新 ChangeId、来源提交、源码树和五文件回执。直连包装器不得复制、chmod 或修改私钥 ACL；必须显式引用原始身份路径，冻结 known_hosts/密钥对/文件元数据与摘要，并在 SSH/SFTP 边界重复复核。候选可单独复制到随机临时目录，SFTP 只读取该五文件快照。单元测试和 Linux `--network none` 门禁不得真实联网，工程合并不构成 010 执行授权。

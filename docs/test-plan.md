...
- 测试服只读入口候选包必须从精确 Git 提交归档生成，以固定 Go 环境连续构建两次并要求摘要一致；输出目录必须全新且失败不留半成品，包内不得包含 Secret、SSH/SCP、sudo、安装、Docker 或服务控制逻辑。生成入口只能接受当前获批准备的 ChangeId、来源提交和全新绝对目录；已消费 001 的普通生成必须继续失败关闭，历史复现只能在临时目录完成并自动销毁。CI 必须使用 `python -I` 执行单测、语法、自检、当前候选真实双构建、五文件白名单、`SHA256SUMS` 和冻结摘要校验；验证 PASS 不等于远端安装或测试服验收。

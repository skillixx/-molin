...
| SMS-P5-06 | 告警规则与通知链 | 失败率、配置异常、网络异常和平均延迟规则由 promtool/Prometheus 加载；通知接收链可达 | 规则与关闭态传输通过：同版本 promtool 阈值正反例、运行实例 4 条规则、抓取目标 up、Prometheus 活跃 Alertmanager 1；专用只读演练预检确认根路由 `discard`、Secret 权限和容器最小权限通过、活动告警和通知累计 0。实际邮件投递和值班确认仍待独立触发演练授权 |
...
| SMS-P5-09 | 回滚 Dry Run 与恢复材料 | 先关开关、保留证据、默认不 down migration；固定备份可验证且业务配置零变更，允许只读访问审计日志增长 | 本地 test/production 两套通过；测试服材料、容器快照和引用镜像完整。旧环境整份恢复失败关闭；基于当前环境的远端候选已排他生成并通过权限、摘要、固定代理、短信关闭和废弃键核验。静态前置仍受旧备份环境阻断，恢复运行时及实际回滚演练待独立授权 |

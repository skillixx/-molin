...
| SMS-P5-10 | 生产灰度与观察 | 生产先关闭；Canary 与开关分开授权；完成 5m/15m/30m/2h/24h 观察 | 部分通过：测试服真实收件 Canary 及 5m/15m/30m/2h 观察通过，仅缺不得早于北京时间 2026-08-07 13:57:50 的 24h；生产尚未开始 |
| SMS-P5-11 | PR 发布安全门禁 | Linux CI 执行 MySQL 8、Redis 7、全库 `-race`、Nginx、promtool、准备度和双环境回滚检查 | 通过：提交 `040ade8` 对应 PR #323 CI run `31099291634` 的 6 个 job 全部成功；PR 可合并但尚未合并 |

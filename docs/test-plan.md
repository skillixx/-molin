...
| SMS-P5-10 | 生产灰度与观察 | 生产先关闭；Canary 与开关分开授权；完成 5m/15m/30m/2h/24h 观察 | 部分通过：测试服真实收件 Canary 及 5m/15m/30m/2h 观察通过；另一次距 Canary 完成 75594 秒的独立补充观察继续保持健康、累计发送不变且零副作用，但不进入五档权威序列。四次 24h 单次执行均证明观察字段通过，但因 SSH stderr 非空失败关闭且未生成快照；三档最小差分诊断已确认 Windows PowerShell 5.1 流式 stdin 注入 UTF-8 BOM 为根因。生成器已改用受限临时文件重定向并通过无 BOM 静态契约；新候选 ChangeId `20260807T081228Z`、runner SHA-256 `cb7a5611c9382a026460a5e7a276f6732abb7318539012ecae919e5e6fbb7226` 仍须独立授权执行 24h 只读窗口。生产尚未开始 |

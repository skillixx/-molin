...
> 开发自测快照（2026-08-03）：九条路由及最小权限映射、九接口 401/403/MFA 矩阵、模板适配器、同步失败零写入、20 路同步去重、模板启停 CAS、固定五场景、两路同版本场景冲突、16 路测试发送并发单外呼、幂等隔离与冲突、白名单、双维限流失败关闭、审计/响应脱敏、全库 Go 测试、vet、依赖校验和敏感扫描已通过。MySQL 8 `1→59→58→59` 和 Linux race 已固化到 CI，但尚未远程执行；Redis Lua 仅有测试替身证据。下表“待执行”专指独立 QA 的 HTTP、真实 MySQL 8、真实 Redis 与获批真实阿里云验证，开发自测不能代替验收签字。

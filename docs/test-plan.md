...
- 测试服只读入口候选包必须从精确 Git 提交归档生成，以固定 Go 环境连续构建两次并要求摘要一致；输出目录必须全新且失败不留半成品，包内不得包含 Secret、SSH/SCP、sudo、安装、Docker 或服务控制逻辑。CI 必须使用 `python -I` 执行生成器单测、语法、自检、真实双构建和 `SHA256SUMS` 校验；候选包 PASS 不等于远端安装或测试服验收。

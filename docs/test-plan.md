...
- 测试服只读入口候选包必须从精确 Git 提交归档生成，以固定 Go 环境连续构建两次并要求摘要一致；输出目录必须全新且失败不留半成品，包内不得包含 Secret、SSH/SCP、sudo、安装、Docker 或服务控制逻辑。ChangeId 消费后，操作 CLI 必须拒绝再次生成持久包，CI 只能在临时目录重建校验并自动销毁。CI 必须使用 `python -I` 执行单测、语法、自检、真实双构建和冻结摘要校验；验证 PASS 不等于远端安装或测试服验收。

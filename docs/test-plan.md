...
| SMS-P5-09 | 回滚 Dry Run 与恢复材料 | 先关开关、保留证据、默认不 down migration；固定备份可验证且业务配置零变更，允许只读访问审计日志增长 | 本地 test/production 两套通过；测试服 11 文件、外部清单摘要、权限、新旧及运行 PID 哈希、架构和 health/ready 只读预检通过；恢复运行时及应用实际回滚演练待独立授权 |

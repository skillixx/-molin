...
- 测试服只读入口候选包必须从精确 Git 提交归档生成，以固定 Go 环境连续构建两次并要求摘要一致；输出目录必须全新且失败不留半成品。001、002、003 均已消费，普通生成和 003 stage 重放必须在读取候选或联网前失败关闭；历史复现只能按精确 ChangeId 在系统临时目录完成并自动销毁。004 暂存取证包装器必须先离线核对 known_hosts 和显式密钥对，再调用固定 OpenSSH 至多一次，以隔离 Python 只读固定 003 暂存路径；禁止 SFTP、下载、删除、sudo、Docker、数据库、队列、服务和业务请求。暂存不存在、完整匹配和存在但不匹配必须分别输出固定低敏 `ABSENT`、`PRESENT/PASS`、`PRESENT/MISMATCH`；不匹配必须以非零状态阻断后续动作，任一远端非零、超时、stderr、额外 stdout 或身份漂移均零重试。CI 必须使用 `python -I` 执行三套单测、语法和自检，并继续验证已消费候选的临时复现与冻结摘要；仓库 PASS 不等于取得远端执行、清理或安装授权。

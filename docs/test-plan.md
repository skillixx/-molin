...
| SMS-P5-04 | 应用可信代理 | 固定代理网段进入 `TRUSTED_PROXY_IPS`；合法单值通过，缺失/多值/非法值失败关闭 | 阻塞：测试服当前为空 |
| SMS-P5-05 | 短信指标 | 5 场景 × 8 结果共 40 条调用序列，5 场景各有耗时 sum/count，无敏感标签 | 本地专项通过，待部署 |
| SMS-P5-06 | 告警规则 | 失败率、配置异常、网络异常和平均延迟规则由 promtool/Prometheus 加载 | promtool 无落盘检查 7/7 通过；待部署加载 |
| SMS-P5-07 | 测试服 Canary | 白名单、总上限 10 条、五入口、OTP 单次消费、结束恢复开关 | 待单独授权 |
| SMS-P5-08 | 页面四宽度 | 1440/1024/768/390 的权限、MFA、模板、日志及五态可操作 | 本地真实浏览器无横向溢出、控制台错误 0；待部署复验 |
| SMS-P5-09 | 回滚 Dry Run | 先关开关、保留证据、默认不 down migration；无外部副作用 | 本地 test/production 两套通过 |
...
| SMS-P5-12 | 固定代理网络部署 | 手动前端工作流检查宿主路由/Docker 网段重叠，固定 `.2/.3`，失败自动恢复部署前镜像 | actionlint、远端 Bash 语法和只读网络预检通过；未执行部署 |

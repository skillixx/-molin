...
- 审计环境：Windows 本地独立工作树；PR #320 GitHub Actions Linux 隔离环境。`SMS_ENABLED=false`，未注入阿里云凭证，未连接测试服，未发送真实短信。
- 数据库版本基线：代码最新 migration 为 `000063`；阶段 4 E2E 顺序安装到短信结构 migration `000059`。功能提交 `ac33744` 已在 MySQL 8、Redis 7 与 Linux race 环境通过 run `30891459024`。
...
|---|---|---|---|---|---|---|---|---|---|---|
| `register` | 用户端 `/register` | `POST /api/auth/verification-codes/phone`，body `{phone,scene:register}` | `POST /api/auth/register` | 公开入口；注册同时要求手机和邮箱验证码 | `register` | `verification_codes`、`sms_scene_bindings`、`sms_templates`、`sms_send_logs`、`users`、`user_sessions` | 手机 OTP 原子消费；邮箱 OTP 消费；创建用户与会话 | `40000`、`40900`、`42900`、`50200`、`50300` | 路由/DTO/Service/Dispatcher、五模板 Mock、隔离 MySQL/Redis HTTP E2E 均通过；阶段 2 有历史真实收件 | 部署态真实短信验证待单独授权 |
| `login` | 用户端 `/login` 手机号登录 | `POST /api/auth/verification-codes/phone`，body `{phone,scene:login}` | `POST /api/auth/login/phone` | 公开入口；账号需存在且启用 | `login` | 上述短信四表、`users`、`user_sessions`、`user_login_logs` | OTP 原子消费；创建会话；登录账号脱敏落库 | `40000`、`40404`、`423/42901`、`42900`、`50200`、`50300` | 登录错误计数、Sender Mock、模板选择及隔离 MySQL/Redis HTTP E2E 均通过 | 部署态真实短信验证待单独授权 |
| `reset_password` | 用户端 `/reset-password` | `POST /api/auth/verification-codes/phone`，body `{phone,scene:reset_password}` | `POST /api/auth/password/reset` | 公开入口；目标账号需存在 | `reset_password` | 上述短信四表、`users`、`user_sessions` | OTP 原子消费；密码哈希更新；全部会话撤销 | `40000`、`40100`、`42900`、`50200`、`50300` | 密码变化、`user_sessions` 吊销、错误/过期/重放/并发 HTTP E2E 均通过 | 部署态真实短信验证待单独授权 |
| `bind_phone` | 用户端 `/profile` 手机号换绑 | `POST /api/me/verification-codes/phone`，body `{phone}` | `PATCH /api/me/phone` | Bearer Token；按用户限流 | `bind_phone` | 上述短信四表、`users`、`audit_logs` | OTP 原子消费；手机号更新并验证；旧手机 MFA 时间清空 | `40000`、`40001`、`40900`、`42900`、`50200`、`50300` | 隔离 MySQL/HTTP E2E、换绑状态及审计脱敏均通过 | 部署态真实短信验证待单独授权 |
| `admin_verify` | 管理后台 `/verify` | `POST /api/admin/auth/verification-codes/phone`，空 body | `POST /api/admin/auth/verify-phone` | Bearer Token + `user:manage`；本入口用于建立手机 MFA | `admin_verify` | 上述短信四表、`users`、`audit_logs` | OTP 原子消费；`admin_phone_verified_at` 更新；脱敏审计 | `40000`、`40001`、`40003`、`42900`、`50200`、`50300` | 权限、MFA、隔离 MySQL/Redis HTTP E2E 均通过 | 部署态真实短信验证待单独授权 |
...
| P4-04 | 最大错误次数 | 首轮审计发现缺口；当前已改为校验前 Redis 原子取得资格，手机号+场景十分钟最多五次，并发不能越界 | **已修复，真实 Redis CI 通过** | 单元测试及 16 路 Redis 并发用例通过 |
| P4-05 | 发码频率限制 | 已补手机号 HMAC+场景 60 秒原子冷却；公开入口 IP 桶改用可信来源解析器，禁止 XFF 伪造拆桶 | **已修复，真实 Redis HTTP CI 通过** | 重复发码和伪造头 10/11 边界通过 |
| P4-06 | 普通 OTP 重复发码契约 | 公开/认证态 OTP 不接受客户端幂等键；每次获准发送由服务端生成唯一 `business_request_no`，同手机号+场景 60 秒内重复请求返回 429 | **契约已明确；HTTP 429 CI 通过** | 管理端测试发送的 `Idempotency-Key` 另由既有幂等/并发测试覆盖，不把两类语义混写 |
| P4-07 | 五场景独立模板 | `Dispatcher.Prepare` 读取数据库绑定；阶段 2 已有五个独立模板 | Mock 与历史部署证据存在 | 当前提交隔离数据库重新核验五个模板 ID 均不同 |
| P4-08 | 模板异常与供应商错误 | 未绑定、未审核、停用、变量不符失败关闭；Sender 归类超时/限流/签名/模板/账户/网络 | 七类表驱动单元测试已通过 | 隔离 HTTP/CI 已通过 |
...
下表把 Goal 的 30 项要求映射到可执行行为证据。`本地通过` 表示相关单元/Mock/契约测试已执行；`CI 通过`
表示 PR #320 的 MySQL 8、Redis 7 与 Linux race 已在 run `30891459024` 实际执行成功。

| # | 强制验证项 | 主要行为证据 | 当前状态 |
|---:|---|---|---|
| 1 | 五场景正常流程 | `TestPhoneCodeFiveScenesUseIndependentDatabaseBindings`；`TestSMSPhase4FiveSceneBusinessE2E` | Mock 本地通过；MySQL/HTTP CI 通过 |
| 2 | 错误验证码 | 五场景 HTTP E2E 每个消费入口先提交错误码 | CI 通过 |
| 3 | 验证码过期 | `TestVerificationExpiryBoundaryIsStrict`、仓储 accepted/expiry 条件更新 | 本地通过 |
| 4 | 验证码重复使用 | 五场景 HTTP E2E 成功后逐场景重放 | CI 通过 |
| 5 | 最大错误次数 | `TestPhase4PhoneOTPBlocksAfterFiveWrongAttemptsAndClearsOnSuccess`、`TestRedisOTPGuardIntegration` | 单测及 Redis CI 通过 |
| 6 | 发码频率限制 | `TestPhase4PhoneSendRateLimitRejectsBeforeOTPAndProvider`、重复发码 HTTP 429 | 单测及 HTTP CI 通过 |
| 7 | 单号码、单 IP、单场景限流 | HMAC+scene OTP Guard；可信来源解析器；两条公开路由来源失败前置测试；轮换 XFF/X-Real-IP 的 `assertPhase4IPRateLimit` 真实 Redis 十次/十一次边界 | 单测及 Redis CI 通过 |
| 8 | 并发发码 | `TestRedisOTPGuardIntegration` 16 路并发只放行一次 | CI 通过 |
| 9 | 并发消费同一验证码 | `assertPhase4ConcurrentConsumption` 16 路 MySQL 并发恰好一次成功 | CI 通过 |
| 10 | 同一业务请求幂等 | 普通 OTP 由服务端生成唯一业务号并以 60 秒冷却拒绝重复；管理测试发送由 `TestAdminTestSendConcurrentReplayCallsProviderOnce` 验证同一幂等键 | 本地及 HTTP 429 CI 通过 |
...
| 16 | 五个启用场景误用同一模板 | `TestSMSAdminSceneConcurrentBindingRejectsSharedTemplate`、仓储唯一占用测试、E2E 五个不同 template_id | 本地及数据库 CI 通过 |
...
| 28 | 响应、日志、审计、前端无敏感信息 | 哈希/脱敏仓储断言、`TestProviderErrorDoesNotExposeRawMessage`、登录日志专项测试、差异扫描 | 本地及数据库 CI 通过 |
| 29 | 测试/生产配置隔离 | 两个 `vite-proxy-isolation` 测试；示例配置默认关闭；本轮不注入阿里云凭证 | 本地通过 |
| 30 | 五场景分别读取独立模板 | Dispatcher 五场景测试与 MySQL `COUNT(DISTINCT template_id)=5` | Mock 本地及 MySQL CI 通过 |
...
成功、重放、未登录、缺权限及重复发码 429；真实 MySQL/Redis 和 Linux race 已由 run `30891459024` 验证通过。

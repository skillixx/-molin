...
13. G3 原始交付不写旧 `token_usage_logs`，也不按多个 Usage 项分别扣钱包。G8 真实请求证据闭环增量已将该历史限制替换为：仅对成功提交为 `settled` 的请求，在同一结算事务中幂等同步一条查询兼容汇总；`ai_requests`、`ai_usage_items`、钱包 hold 和钱包流水仍是权威计费事实，兼容汇总不得参与再次扣费。

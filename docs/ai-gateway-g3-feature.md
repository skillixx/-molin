...
13. G3 原始交付不写旧 `token_usage_logs`，也不按多个 Usage 项分别扣钱包。G8 真实请求证据闭环增量已将该历史限制替换为：仅对成功提交为 `settled` 的请求，在同一结算事务中幂等同步一条查询兼容汇总；其金额精度与权威财务账本统一为 8 位小数。人工核定未知或取消执行时保留 `pending_reconcile` 来源状态，不得伪造执行成功；`ai_requests`、`ai_usage_items`、钱包 hold 和钱包流水仍是权威计费事实，兼容汇总不得参与再次扣费。
14. Bifrost 执行只从响应 JSON 或 SSE `data` JSON 的顶层 `id` 提取受长度与字符集约束的低敏上游引用，并写入执行尝试。该引用不进入公开响应、日志或错误信息；Bifrost 请求日志继续保持关闭。

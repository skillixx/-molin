...
> 状态：测试环境真实 E2E、CI 与独立 QA 已通过，产品整改复验中。G5 验收提交为 `60b569f`，G6 实际开发基线为 `origin/main@c4126a6`；最终状态以 `docs/ai-gateway-g6-acceptance.md` 为准。
...
- 用量总览：今日与本月请求量、输入/输出 Token、结算费用和预算使用情况。
- CSV 导出：最大时间跨度和最大行数限制；单元格以 `= + - @` 开头时增加安全前缀。
- 账单申诉：仅允许请求所有者创建；不接收密钥、提示词或响应正文。

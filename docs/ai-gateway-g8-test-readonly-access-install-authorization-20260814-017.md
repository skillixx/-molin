...
本清单仅冻结 017 本地工程候选，不是测试服执行授权。当前尚未创建 PR、运行精确 HEAD CI 或完成独立复评；当前仍禁止运行生成命令中的 SSH、交互 sudo 或安装段。只有工程门禁、主线合并和合并后原始 Git blob 摘要复核全部通过，并再次取得用户对 017 的独立精确授权后，才能执行一次。

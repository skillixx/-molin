...
> 当前状态：`CONSUMED_STOPPED_REMOTE_EVIDENCE`。用户已批准并完成唯一一次正式调用；本地检查 PASS，正式调用返回 `remote_evidence_failed` 后零重试停止。004 已消费，禁止重试或重放；完整记录见 `docs/ai-gateway-g8-test-readonly-staging-evidence-attempt-20260812-004.md`。
...
- 上述证据只证明 004 取证入口工程就绪；后续唯一正式调用未形成远端状态证据，也不授权生产、真实上游、通知、客户灰度或商业观察。

...
3. 确认至少有一个管理员用户；全新空库若没有用户，000062 不会自动发布默认策略，需通过后台创建并发布策略后才开放请求。
...
000062 是 expand migration。应用回滚时保留表和数据，down 文件故意不删除事实；不要手工 DROP 表。
...
应用回滚到 G3 版本时停止新流量，等待或标记运行中请求，保留 000062 表。恢复 G4 时先跑恢复扫描和账本对账，再开放请求。生产回滚、真实数据库变更和人工财务处置必须由负责人审批。

...
- Token 网关 G8 采用三层完成口径：`G8_ENGINEERING_READY` 证明代码、CI、隔离演练和独立评审完成；`G8_SOFTWARE_CLOSED_LOOP` 证明测试候选在受控环境完成发布、接入、调用、计量、扣费、账单、申诉、零差额对账和实际回滚，是当前软件实现任务的完成状态；`G8_COMMERCIAL_ACCEPTED` 属于后续生产灰度和商业确认，不阻塞软件闭环。
- `G8_SOFTWARE_CLOSED_LOOP` 不授权生产部署、真实付费上游、真实客户流量或真实通知。商业观察周期、客户数量、真实集成、真实付费、生产成功率和毛利在后续生产灰度立项时独立确认。
...
- 设备状态流转测试。
- Token 网关路由、限流、扣费测试。
- Token 网关测试候选真实后端全旅程、三项账务零差异、依赖故障恢复和候选→基线→候选实际回滚验收。
- Docker 部署。
- 灰度上线。

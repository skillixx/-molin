...
目标 Drop 端点的 ED25519 主机记录已经通过既有可信独立渠道核对且保持不变；不得修改信任库、自动接受未知主机密钥或关闭主机校验来绕过。修复后的诊断器在 014 唯一获批执行中返回精确 `PASS`，随后唯一一次只读 SSH 返回 `PRESENT / PASS / NONE`。014 已消费，禁止再次借由本地诊断结果连接测试服。
...
- 014 执行前 Windows 定向单测：诊断器 18 项通过、1 项按平台跳过；013 墓碑入口 1 项通过；014 只读取证包装器 23 项通过、6 项 POSIX 动态语义按平台跳过。
- 014 执行前 Linux 断网门禁：诊断器 17 项通过、2 项 Windows 路径语义按平台跳过；013 墓碑入口 1/1、014 只读取证包装器 29/29 全部通过。014 消费后，Windows 本地回归确认 014 墓碑入口 1/1 通过且 `Popen` 未启动；Linux 断网回归由本次 CI 提供。
- 离线 self-test：`PASS`。

...
```text
GATE=VID-G2
DECISION=AUTO_READY_FOR_PR_AUTH
...
GIT_MODE=COMMIT_AND_PUSH_AUTHORIZED_PR_NOT_AUTHORIZED
...
QA、产品、独立工程审查、全量Go回归、vet、敏感扫描和隔离MySQL race门禁均已通过。本地提交与分支推送已获授权完成，当前只达到`AUTO_READY_FOR_PR_AUTH`；创建PR和合并仍分别需要独立授权。VID-G2停止于已推送分支，不进入VID-G3。

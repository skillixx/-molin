...
主线（复用底座 / 薄转发 / 三层免费 / 唯一 token 收费）清晰自洽。但有 4 个「不定就会出事」的缺陷在进入实现前必须拍板，1 个渠道单点需从隐含风险升为显式排期，另有若干体验/正确性问题建议修订。**不阻塞规划合并，阻塞 M1 开工**——下列 🔴 项应在 W5 第一天前定案。
...
### R3. 插件外部成本 = 不设防的成本黑洞
- **现状**：「插件外部 API 成本平台承担」（billing 契约红线）+「Agent/插件免费」+「sk 可调 `/agents/{id}/chat` 编排端点」（§14.8）三者叠加，且插件调用无配额。
...
### R5. SSE 中途断开 → 收不到 usage，计费丢失
- 透传/编排均靠「末尾 chunk 取 usage」。客户端中断（SSE 常见）则 usage 读不到，本次 token 不计费；编排多轮放大。
- **建议**：服务端始终读完上游流再结算（即使客户端断开，后端继续消费到 usage）；无 usage 的成功调用按 max_tokens 兜底计费。
...
| # | 决策点 | 选项 | 建议 |
|---|---|---|---|
| D1 | 并发透支方案（R1） | 预扣保证金 / 并发上限+余额门槛 | **预扣保证金**（复用钱包 freeze） |
| D2 | sk 是否开放编排端点（R3） | 开放 / 仅透传端点开放 sk | **编排端点限登录态**，sk 仅透传 |
| D3 | 付费插件成本与配额（R3） | 平台全担无限 / 平台担+配额上限 | **平台担 + 每用户每日上限 + 计入限流** |
| D4 | code_exec 是否本期做（R4） | 做(需沙箱) / 本期不做 | **本期不做**，示例改无副作用 skill |
| D5 | entitlement 幂等表（R8） | 新建表 / 复用流水 | **新建 entitlement_consume_logs** |
| D6 | 渠道故障转移（R2） | 本期做一对多 / 显式延后 | 本期至少**显式登记延后**，能做则做 |
...
| 风险 | 待改文档 | 待改/新增任务 |
|---|---|---|
| R1 | billing §4.3、sk §9 | S2-丁5 扩展（预扣逻辑）、S2-测* 验收明确 |
...
1. PM/团队就 §4 的 D1–D6 拍板（D1/D3/D4/D5 优先，阻塞 M1）。
2. 拍板结果由我吸收进对应契约（roadmap/sk/billing/chat-workbench）并同步 §14。
3. 据此调整 `backend-stage2-task-schedule.md`（新增/扩展任务卡），再进入实现。

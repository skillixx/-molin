...
- 018 已完成工程合并、唯一人工执行失败关闭及墓碑归档，不再等待或接受任何 018 远端授权。后续 019 当时须独立完成工程门禁、合并后复核并取得新的精确授权；019 现已执行失败并永久消费，不得再次授权、重试或重放，继续只能使用新的独立 ChangeId。不得把工程合并、CI 或离线测试表述为测试服已安装、运行态通过或 `G8_SOFTWARE_CLOSED_LOOP` 完成。

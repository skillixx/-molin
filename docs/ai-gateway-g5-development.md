...
模型发布会锁定 `token_models` 行，依次校验两份健康文档、唯一生效价格和健康生效路由，使用锁定当前读取得活动版本，并按 JSON 语义判断目录快照是否重复；随后退役旧快照、创建新快照，再以旧 `release_version_no` 条件更新目录。相同快照的重复或并发发布返回同一既有版本，数据库只产生一次写入；版本号被不一致记录占用或乐观条件变化时返回明确的可恢复状态冲突。模型回滚读取历史快照但不修改历史行，而是创建新的 active 发布版本。

发布失败分类固定为：`40910` 文档未就绪、`40911` 当前生效价格数量不是一、`40912` 无健康生效路由、`40913` 模型状态已变化。前端收到失败后刷新目录；对本地即可判定的文档缺失，会在发送发布请求前给出提示。

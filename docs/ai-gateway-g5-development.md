...
模型发布会锁定 `token_models` 行，依次校验两份健康文档、唯一生效价格和健康生效路由，使用锁定当前读取得活动版本，并按 JSON 语义判断目录快照是否重复；随后以目录指针和历史最大版本的较大值加一分配新版本，退役旧快照、创建新快照，再以旧 `release_version_no` 条件更新目录。相同快照的重复或并发发布返回同一既有版本，数据库只产生一次有效写入；孤儿或退役历史记录不会永久阻塞首次发布，残余唯一键竞态和乐观条件变化统一返回明确的可恢复状态冲突。模型回滚同样跳过已占用历史编号，读取历史快照但不修改历史行，而是创建新的 active 发布版本。

发布失败分类固定为：`40910` 文档未就绪、`40911` 当前生效价格数量不是一、`40912` 无健康生效路由、`40913` 模型状态已变化。发布、下架和回滚发生残余并发状态变化时均使用 `40913`；前端收到失败后刷新目录，对本地即可判定的文档缺失会在发送发布请求前给出提示。
...
路由传输失败计数存放在 MySQL 共享表。只有健康渠道上的活动路由可以承载请求；模型一旦存在 G5 路由记录，全部路由熔断、停用或渠道异常时必须失败关闭，只有从未配置 G5 路由的旧模型可以兼容回退。仅 `request_not_sent` 且 `result_unknown=false` 的失败进入安全重试；达到阈值后熔断 30 秒，下一请求选择备用路由。成功调用复位计数，超时、流中断、HTTP 错误和结果未知不自动重试。正常结算依次保存原始 Usage `provider/sequence_no=0`、销售计量 `provider/sequence_no=1` 和上游成本 `provider/sequence_no=2`；内容治理免单的上游成本继续使用 `provider_cost/sequence_no=0`。经营指标读取这些冻结事实，不使用当前价格反算历史成本。

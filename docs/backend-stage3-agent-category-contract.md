...
- 分类字典 CRUD（`agent:manage` 或新增 `agent_category:manage`，**建议复用 `agent:manage` 避免新权限码**）：
  ...
  - 本期也可**只 seed 不做 CRUD 页**（4 个分类够用），CRUD 作为后续；最小版只需 §2 seed + §3.1 列表 + §3.2 agent 带分类。
...
- **删分类的处理**：软关联无强 FK——删除某分类时，建议 service 层把引用它的 Agent `category_code` 置 NULL（或禁止删除"仍被引用"的分类，返回 40000 提示先迁移），**二选一需 PM 定**；推荐"禁止删除被引用分类"，更安全。
...
## 7. 待确认（开发前）
- 是否需要分类字典的**管理端 CRUD**，还是本期只 seed 固定 4 类（后续再加 CRUD）？
- 删除被引用分类的策略：**禁止删除**（推荐）还是**置引用为未分类**？
- 是否要支持一个 Agent **多分类**（多标签）；本期默认单分类。
- 迁移序号：第三阶段起点（与可见性契约同批，按合并顺序排）。

...
015 的生成器与安装器已改为固定 `change_id_consumed` 墓碑：其唯一获批本地段出现 PowerShell 正则转义错误，但该错误默认非终止，后续身份材料读取、SSH 和远端影响保持 `UNKNOWN`，015 禁止重放。`infra/scripts/prepare-ai-gateway-g8-test-readonly-access-016-command.py` 与 `infra/scripts/g8-test-readonly-access-install-016.sh` 是修复后的 016 工程候选；真实生成段冻结 PowerShell 错误停止/恢复，生成器本身不连接测试服、不读取身份材料，只生成冻结双段人工命令；安装器只从已由 014 证明完整的 011 暂存以 no-clobber 方式安装两个 root-owned 工具和一条精确 sudoers，失败时撤销本次 live 创建。当前清单为 `PENDING_ENGINEERING_REVIEW / REMOTE_NOT_AUTHORIZED`，只允许运行单测、`bash -n`、`py_compile` 和 016 生成器 `--self-test`；禁止运行生成命令中的 SSH、sudo 或安装段。详见 `docs/ai-gateway-g8-test-readonly-access-install-attempt-20260814-015.md` 和 `docs/ai-gateway-g8-test-readonly-access-install-authorization-20260814-016.md`。

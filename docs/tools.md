...
018 工具使用新 ChangeId `CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-018`，只继续使用 014 已证明完整的固定 011 暂存资产。`prepare-ai-gateway-g8-test-readonly-access-018-command.py` 只离线生成冻结人工命令，不连接网络；它将可信路径、材料摘要、known_hosts、密钥配对、材料漂移和 SSH 会话失败分别输出为固定低敏原因，并仅在完整 SSH 前门禁通过后形成 `PRE_SSH_GATE=PASS`、紧邻唯一 SSH 调用形成 `SSH_ATTEMPTED=YES`。配套安装器继续执行 no-clobber、sudo 最小权限和 HUP/TERM/INT 不可重入回滚。工程合并与合并后冻结复核已经完成，当前为 `PENDING_USER_APPROVAL / REMOTE_NOT_AUTHORIZED`，且 `CHANGE_ID_CONSUMED=False`、`REMOTE_EXECUTION_AUTHORIZED=False`；只能运行离线测试、自检和摘要复核，禁止执行生成命令或任何 SSH/sudo/安装动作。授权清单见 `docs/ai-gateway-g8-test-readonly-access-install-authorization-20260814-018.md`。

...
022 工具使用 ChangeId `CHG-G8-TEST-READONLY-RUNTIME-AUDIT-DROP-20260815-022`。`prepare-ai-gateway-g8-test-readonly-runtime-audit-022-command.py` 在内存中生成固定单 SSH 审计脚本，`run-ai-gateway-g8-test-readonly-runtime-audit-022.py` 是唯一正式启动入口。022 修复 021 普通 Python 字符串造成的 PowerShell 回执正则反斜杠折叠，使用可信 LocalApplicationData、固定盘、逐级非 reparse 与唯一 GUID CreateNew 回执，并分别收敛目录/权限、预占、写入和刷盘失败。PR #401、CI run `31884793587`、merge commit `84ae5b0ad87958ee63fbfa709c4f164baca39a1b` 及合并后原始 blob/冻结命令摘要复核均已通过；当前为 `PENDING_USER_APPROVAL / REMOTE_NOT_AUTHORIZED`。未取得新的独立精确授权前，不得执行正式入口、SSH、测试服 Docker/HTTP/数据库、sudo、安装、业务、上游或费用动作。见 `docs/ai-gateway-g8-test-readonly-runtime-audit-authorization-20260815-022.md`。

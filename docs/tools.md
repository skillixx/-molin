...
018 工具在唯一人工本地段窗口直接关闭、没有可见输出后已失败关闭消费；SSH 启动/连接为 `UNKNOWN / 最多 1`，远端固定段、sudo、安装器与 post-check 均为 0，两个历史入口均为固定 `change_id_consumed` 墓碑。019 使用独立 ChangeId `CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-019`；`prepare-ai-gateway-g8-test-readonly-access-019-command.py` 只离线生成单会话命令，把无秘密 Base64 远端脚本作为唯一 `ssh -tt` 的参数，TTY 只留给 sudo，且父 PowerShell 不主动退出。当前为 `PENDING_ENGINEERING_REVIEW / REMOTE_NOT_AUTHORIZED`；只允许离线测试和 CI，禁止执行生成命令或任何 SSH/sudo/安装动作。见 `docs/ai-gateway-g8-test-readonly-access-install-attempt-20260815-018.md` 与 `docs/ai-gateway-g8-test-readonly-access-install-authorization-20260815-019.md`。

...
015 与 016 的生成器、安装器均已改为固定 `change_id_consumed` 墓碑。015 的唯一获批本地段出现 PowerShell 正则错误且下游影响保持 `UNKNOWN`；016 经 PR #381 合并和冻结摘要复核后获得独立授权，但唯一人工第一段在模块自动加载的 `Get-FileHash` 处以终止错误停止，控制流没有到达 SSH，远端影响为 0。`infra/scripts/prepare-ai-gateway-g8-test-readonly-access-017-command.py` 与 `infra/scripts/g8-test-readonly-access-install-017.sh` 是新的 017 工程候选：冻结材料摘要改用 Windows PowerShell 5.1 可用的纯 .NET 流式 SHA-256，并收紧加密私钥提示、低敏输出和信号回滚。PR #384 最终 HEAD `ee947fd61919215500ef516488d56e01ad2ea72d` 通过 CI run `31791430839` 与三方零缺陷复评，按 merge commit `e2a7e4f89c4115b3e32dc27292b0bc11d7d09a57` 合入 main；合并后原始 Git blob 与冻结命令复核一致。当前清单为 `PENDING_USER_APPROVAL / REMOTE_NOT_AUTHORIZED`，只允许运行单测、`bash -n`、`py_compile` 和 017 生成器 `--self-test`；017 尚未安装、尚未消费，禁止运行生成命令中的 SSH、sudo 或安装段。详见 `docs/ai-gateway-g8-test-readonly-access-install-attempt-20260814-016.md` 和 `docs/ai-gateway-g8-test-readonly-access-install-authorization-20260814-017.md`。

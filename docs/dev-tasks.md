...
| A-14 | 待排期 | `GET /api/admin/audit-logs` 响应补充 `request_summary` 字段：`audit_logs.request_summary`（JSON 字符串，记录操作参数）已正确写入数据库，但 `IAMHandler.ListAuditLogs`（`server/internal/modules/iam/handler/iam_handler.go` ~398-407）返回的 map 中未包含该字段，导致管理后台审计日志页面无法展示操作详情 | `feature/backend-a-audit-log-request-summary` | ⏳ 待开始 | 2026-06-13 A-13 验收（PR#39）发现：4 个 IAM 单条操作（assign_role/revoke_role/set_permission_override/delete_permission_override）及既有批量操作的 `request_summary` 均已正确写入，仅响应未返回；建议在响应 map 中新增 `request_summary` 字段，可直接返回原始 JSON 字符串或反序列化为对象（需与前端约定格式），`model.AuditLog.RequestSummary` 字段已存在（`*string`，`gorm:"type:json"`） |
...
|---|---|---|---|---|
| 后端工程师甲 | 14 | 13（9 已审查 + 4 已验收，PR#31/#32/#38/#39）| 0 | 1 |
| 后端工程师乙 | 6 | 0 | 0 | 6 |
| 后端工程师丙 | 5 | 0 | 0 | 5 |
| 前端工程师甲 | 8 | 3（已审查）| 0 | 5 |
| 前端工程师乙 | 8 | 0 | 0 | 8 |

...
若 Alertmanager 的通知/请求失败指标均为 0，但负责人确认邮件未收到，结论仍必须是投递失败。运维只能进行以下不发送检查：

1. 在发件服务商控制台核对同一 UTC 时间窗口是否存在 SMTP 接受、退信、限流或风控记录，不导出正文或完整地址。
2. 本地计算预期收件地址 SHA-256，与测试服配置的脱敏摘要双人比对；禁止在聊天、工单或仓库粘贴完整地址。
3. 由收件人检查垃圾箱、QQ 邮箱拦截记录、黑名单和 126 发件人白名单，并记录“收到/未收到”布尔结果。
4. 原 ChangeId 及其单次额度立即封存；任何重试必须使用新 ChangeId、修正后的有效载荷，并重新取得一次 firing/resolved
   的独立授权。不得把 SMTP DATA 被接受、Alertmanager 失败指标为 0 或 HTTP 200 当作最终投递成功。

...
> G0/G1 增量：`000060` 已冻结 `ai_projects`、`ai_requests`、`ai_usage_items`、`ai_execution_attempts` 的 Expand Schema 与 Go 模型，并补充 Native/Bifrost 等价和双上游配置契约。新表尚未接入现有转发读写，真实 Linux 双上游 POC 仍以 `docs/ai-gateway-g1-poc-report.md` 为准；未关闭前不得进入 G2。
>
...
软件闭环完成后，再由独立生产灰度任务确认观察周期和正式门槛。以下指标作为候选基线，不阻塞 `G8_SOFTWARE_CLOSED_LOOP`：

- 至少确认 3 家小型企业设计客户。
- 至少 2 家完成真实 API 集成，不以控制台演示代替。
- 至少 1 家使用自有资金完成真实充值；连续观察周期在生产灰度立项时确认。
...
| 8 | 测试候选运行闭环、修复、回滚演练和文档收口 | 软件闭环报告、部署/回滚清单 | G8：达到 `G8_SOFTWARE_CLOSED_LOOP` 后结束当前软件任务 |

软件闭环完成后，生产灰度与商业观察另行立项，观察周期届时确认。图片阶段仍须另行立项，不得以软件闭环完成自动授权多模态开发或生产开放。
...
本节为后续 `G8_COMMERCIAL_ACCEPTED` 候选基线，不作为 `G8_SOFTWARE_CLOSED_LOOP` 的完成条件；正式指标和观察周期在生产灰度立项时确认。

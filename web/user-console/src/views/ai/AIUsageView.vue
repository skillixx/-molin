...
async function loadFilters() {
  const [projectResult, modelResult] = await Promise.all([listAIProjects({ page: 1, page_size: 100 }), listAIModels({ page: 1, page_size: 100 })])
...
function search() { page.current = 1; loadRows() }
...
function money(value?: string) { return `¥${Number(value || 0).toFixed(8)}` }
...
<template>
  <section class="usage-page">
    ...
    <div class="filters">
      ...
      <el-select v-model="filters.status" clearable placeholder="全部状态"><el-option label="成功" value="succeeded" /><el-option label="失败" value="failed" /><el-option label="待结算" value="settlement_pending" /><el-option label="已结算" value="settled" /><el-option label="安全拒绝" value="rejected" /><el-option label="待对账" value="exception" /></el-select>
    ...
    <div v-loading="loading" class="mobile-list">
      <article v-for="row in rows" :key="row.request_id" class="request-card" @click="openDetail(row.request_id)"><div><code>{{ row.request_id }}</code><RequestStatusTag :status="row.billing_status" /></div><strong>{{ row.logical_model_code }}</strong><p>{{ row.project_name }} · {{ row.api_key_name }}（{{ row.api_key_prefix }}）</p><footer><span>{{ row.input_tokens }} / {{ row.output_tokens }} Token</span><b>{{ money(row.settled_amount) }}</b></footer></article>
    ...
    <el-drawer v-model="detailVisible" title="请求账单详情" size="min(640px, 100%)">
      <div v-loading="detailLoading" class="detail-body">
        <template v-if="detail">
          <dl class="detail-grid"><div><dt>请求 ID</dt><dd>{{ detail.request_id }}</dd></div><div><dt>模型</dt><dd>{{ detail.logical_model_code }}</dd></div><div><dt>Project</dt><dd>{{ detail.project_name }}</dd></div><div><dt>API Key</dt><dd>{{ detail.api_key_name }} · {{ detail.api_key_prefix }}</dd></div><div><dt>执行状态</dt><dd><RequestStatusTag :status="detail.execution_status" /></dd></div><div><dt>结算状态</dt><dd><RequestStatusTag :status="detail.billing_status" /></dd></div><div><dt>价格版本</dt><dd>v{{ detail.price_version_no }}</dd></div><div><dt>结算金额</dt><dd class="money-cell">{{ money(detail.settled_amount) }}</dd></div></dl>
...
.filters { display: grid; grid-template-columns: repeat(4, minmax(120px, .7fr)) minmax(260px, 1.4fr) auto; gap: 8px; margin-bottom: 14px; }.filter-actions { display: flex; gap: 8px; }.filter-actions :deep(.el-button + .el-button) { margin-left: 0; }.prefix { display: block; color: var(--color-text-disabled); }.request-link { max-width: 100%; overflow: hidden; text-overflow: ellipsis; }.money-cell { color: var(--color-accent-warm); font-variant-numeric: tabular-nums; }.mobile-list { display: none; }.el-pagination { justify-content: center; margin-top: 22px; }
...
@media (max-width: 720px) { .desktop-table { display: none; }.mobile-list { display: grid; gap: 10px; }.request-card { padding: 15px 0; border-bottom: 1px solid var(--color-border); cursor: pointer; }.request-card > div { display: flex; align-items: center; justify-content: space-between; gap: 10px; }.request-card code { min-width: 0; overflow: hidden; text-overflow: ellipsis; color: var(--color-primary); }.request-card > strong { display: block; margin-top: 10px; overflow-wrap: anywhere; }.request-card p { color: var(--color-text-muted); }.request-card footer { display: flex; justify-content: space-between; gap: 12px; font-size: 13px; }.request-card footer b { color: var(--color-accent-warm); }.detail-grid { grid-template-columns: 1fr; }.detail-grid div:nth-child(odd) { border-right: 0; } }

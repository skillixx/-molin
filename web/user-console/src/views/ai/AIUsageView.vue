...
function meterSourceLabel(value: string) { return ({ provider_confirmed: '上游确认用量', reconciled: '人工核定用量' } as Record<string, string>)[value] || value }
...
<template>
  <section class="usage-page">
    ...
    <el-drawer v-model="detailVisible" title="请求账单详情" size="min(640px, 100%)" class="ai-billing-detail-drawer">
...
:global(.ai-billing-detail-drawer) { background-color: var(--color-bg-soft) !important; background-image: linear-gradient(180deg, rgba(14,21,33,.99), rgba(7,11,18,.99)); color: var(--color-text) !important; }:global(.ai-billing-detail-drawer .el-drawer__header) { margin-bottom: 0; padding: 18px 20px; border-bottom: 1px solid var(--color-border); color: var(--color-text) !important; }:global(.ai-billing-detail-drawer .el-drawer__title) { color: var(--color-text) !important; font-weight: 700; }:global(.ai-billing-detail-drawer .el-drawer__close-btn) { color: var(--color-text-muted) !important; }:global(.ai-billing-detail-drawer .el-drawer__body) { background: transparent !important; color: var(--color-text) !important; }

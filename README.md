...
- `CHG-G8-TEST-READONLY-HOST-IDENTITY-DIAG-20260812-007` 完成仓库工程门禁后，用户批准并执行唯一一次本地检查和正式只读 SSH：本地检查 PASS，正式结果为固定低敏 `BLOCKED / READABLE_MISMATCH`，随后零重试停止；业务请求、上游请求、费用为 `0 / 0 / 0 CNY`。该结果不输出当前 machine-id 原文或摘要，不证明既有批准基线或当前值哪一方正确；未读取暂存目录，003 暂存继续为 `UNKNOWN`。007 已消费，继续须以新 ChangeId 通过阿里云 root/CMDB 等独立受控来源核验，证据见 `docs/ai-gateway-g8-test-readonly-host-identity-diagnostic-attempt-20260813-007.md`。

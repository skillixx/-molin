...
- PR #380 已按 merge commit `102e833b1975d56100faf4d8e4add0150a3edc39` 合入 015 工程候选并删除远端分支。用户独立批准后，015 人工本地段出现 PowerShell 路径正则末尾转义错误；该错误默认非终止，现有记录不能证明后续身份材料读取、SSH 或远端影响，相关状态保持 `UNKNOWN`，重试 0，015 已消费并墓碑化。修复后的 016 把错误停止与恢复策略写入真实生成段；PR #381 最终 HEAD `04abab350afc3163a66dd5faee86005e7c2b02c3` 的 CI run `31776890876` completed/success，独立代码安全、QA、产品/规格复评均允许合并，并按 merge commit `2f407cbf3a9c5fea987eeb2f82ebb41630db9e35` 合入 main，远端功能分支已删除。合并后 4 个冻结 Git blob 和纯内存生成命令摘要复核一致；016 当前状态为 `PENDING_USER_APPROVAL / REMOTE_NOT_AUTHORIZED`，不继承 015 执行授权，尚未执行 SSH、sudo 或安装。失败记录与新清单见 `docs/ai-gateway-g8-test-readonly-access-install-attempt-20260814-015.md`、`docs/ai-gateway-g8-test-readonly-access-install-authorization-20260814-016.md`。

...
- 013 采用本地与远端授权解耦方案：无 ChangeId 本地诊断器可重复且没有 SSH/SFTP/远端访问能力；一次性 013 包装器不提供 `--local-check`，只在未来另行明确授权后最多执行一次固定只读 SSH。工程候选最终 HEAD `1b542dc656b09ace80bcdd370fac360ba19b4091` 经 CI run `31719189481` 12/12 SUCCESS 及独立代码安全、QA、产品/规格 P0/P1/P2=0 后，由 PR #374 按 merge commit `d0349353342bc37a912b1942d743e0c45c75ea80` 合入主干，状态收敛为 `PENDING_USER_APPROVAL`。尚未运行真实本地材料诊断或连接测试服，011 暂存仍为 `UNKNOWN`；合并不构成 013、清理、安装、运行态审计、生产或商业授权。

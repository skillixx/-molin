...
**CI 策略：** PR 先按精确变更路径执行失败关闭分类。同一 PR 的旧运行会被新事件自动取消；Draft 只运行统一轻量质量检查和受影响模块的定向测试，转为 Ready 后才重新运行全部适用重型门禁并生成 `CI 必选门禁汇总`。现有测试没有删除，Draft PASS 不能替代 Ready 完整证据。详细规则见 `docs/git-workflow.md` §8.2c。
...
- G8 已由项目负责人完成最终阶段验收归档：`G8_STAGE_ACCEPTANCE=PASS`、`G8_SOFTWARE_CLOSED_LOOP=COMPLETED`、`G8_TEST_ENV_USABLE=YES`、`G8_REAL_PROVIDER_SETTLEMENT=PASS`、`ACCEPTED_EXCEPTIONS=YES`。真实 Provider 调用、执行、Usage、计费结算、钱包流水、Outbox 与低敏证据持久化主链路作为已有技术证据通过；`RESPONSE_MATCH=NO`、未配置临时 SK 的手工脚本及追加账单/争议查询核对由负责人接受为非阻断，未被改写为测试通过。测试服对账、失败补偿、双闸门、回滚演练以及 Prometheus/Grafana、告警规则、备份周期和 RabbitMQ ready 消息转入后续运维专项。测试服真实流量闸门当前保持开启，任何实际请求都可能调用真实 Provider 并产生真实费用；生产开放和 `G8_COMMERCIAL_ACCEPTED` 仍需独立授权。证据边界见 `docs/ai-gateway-g8-acceptance.md` 和 `docs/ai-gateway-g8-software-closure.md`。
- G8 测试服最小只读入口候选包门禁已收口：PR #333 精确 HEAD `c0479f607c9dbd5713c9fbbde7b3fb83ac2a3adc` 的 CI run `31566629193` 为 9/9 SUCCESS，独立代码安全、QA、产品/规格均为 P0/P1/P2=0，并按 merge commit 合并为 `69439c4c9b14c67bf8a17dd8822d80ecdc784a27`。该结论只证明仓库候选包可复现且失败关闭；测试服 live 入口尚未安装。003 暂存上传状态后续已由 008 收敛为 `ABSENT`，但现有 API 停止、运行态 P1=3 及 schema/Bifrost/监控/账务 UNKNOWN 仍未关闭。
...
- `CHG-G8-TEST-READONLY-TRANSPORT-DIAG-20260812-005` 已获独立批准并完成唯一一次本地检查和正式只读 SSH：本地检查 PASS，正式结果为 `ZERO / EXACT / stderr EMPTY / diagnostic PASS`，零重试且业务请求、上游请求、费用为 `0 / 0 / 0 CNY`。该结果证明固定 SSH 与远端隔离 Python 标记可用，但没有读取暂存目录，不能关闭 003 暂存 `UNKNOWN`。005 已消费，继续须使用新 ChangeId 完成工程门禁并再次取得授权；证据见 `docs/ai-gateway-g8-test-readonly-transport-diagnostic-attempt-20260812-005.md`。
- `CHG-G8-TEST-READONLY-STAGING-EVIDENCE-20260812-006` 用户批准后完成唯一一次本地检查和正式只读 SSH：本地检查 PASS，正式调用返回固定 `BLOCKED / MACHINE_ID` 后零重试停止，业务请求、上游请求、费用为 `0 / 0 / 0 CNY`。暂存查找未执行，003 暂存继续为 `UNKNOWN`；006 已消费并在身份读取和联网前失败关闭。执行证据提交 `7157ee0f4a92b73a06855b0a8f35f12f07575ce4` 经独立代码安全、QA、产品/规格 P0/P1/P2=0 和 CI run `31613370496` 12/12 SUCCESS 后，由 PR #347 按 merge commit `2399f58143b683b95fdf3011be8a535bfedef222` 合入主干，远端功能分支已删除。完整记录见 `docs/ai-gateway-g8-test-readonly-staging-evidence-attempt-20260812-006.md`。
- `CHG-G8-TEST-READONLY-HOST-IDENTITY-DIAG-20260812-007` 完成仓库工程门禁后，用户批准并执行唯一一次本地检查和正式只读 SSH：本地检查 PASS，正式结果为固定低敏 `BLOCKED / READABLE_MISMATCH`，随后零重试停止；业务请求、上游请求、费用为 `0 / 0 / 0 CNY`。该结果不输出当前 machine-id 原文或摘要，不证明既有批准基线或当前值哪一方正确；未读取暂存目录，003 暂存继续为 `UNKNOWN`。执行证据 HEAD `6edbd89c3c6c1c8392262a775b2ac087caee3df7` 的 CI run `31650387182` 为 12/12 SUCCESS，独立代码安全、QA、产品/规格对仓库变更均为 P0/P1/P2=0；PR #351 已按 merge commit `492b56b9345592f1b5580e6de9fb1a1dfc540b93` 合入主干并删除远端分支。007 已消费；执行当时要求后续通过阿里云 root/CMDB 等独立来源核验，但该要求已被下一条 Drop 场景确认替代，不再是 008 的前置门禁。历史证据见 `docs/ai-gateway-g8-test-readonly-host-identity-diagnostic-attempt-20260813-007.md`。
- 测试入口实际由 Drop 服务映射，底层物理主机身份不是适用门禁；007 的 `READABLE_MISMATCH` 作为历史事实保留，但不再登记为当前测试服运行态 P1。008 Drop 暂存只读取证在独立授权后完成唯一一次本地检查和唯一一次只读 SSH，结果为 `ABSENT / NOT_APPLICABLE / NONE`、stderr 为空、零重试、业务请求/上游请求/费用为 `0 / 0 / 0 CNY`；固定 003 暂存状态从 `UNKNOWN` 关闭为 `ABSENT`，未执行也无需执行清理，008 已消费并禁止重放。执行证据 HEAD `f0d726ad5d347dd1f35f2ebec2e118f0093e958f` 已通过 CI run `31661245959` 12/12 和独立代码安全、QA、产品/规格 P0/P1/P2=0，并由 PR #356 按 merge commit `6b2a1fa438dbad2e7d0a15b33d4c8c0d8ff8b7be` 合入主干。该结果不关闭 API、数据库、Bifrost、监控、备份或账务运行态门禁；继续安装准备必须使用新 ChangeId。证据见 `docs/ai-gateway-g8-test-readonly-drop-staging-evidence-attempt-20260813-008.md`。
- 009 Drop 只读入口安装候选的仓库工程门禁已完成：ChangeId `CHG-G8-TEST-READONLY-ACCESS-DROP-20260813-009`，冻结来源 `7f3325e2d6801567fea34a2049a2f3ada114e348`、源码树 `4563feb59850dca87789adfb5eea820f78b1a209` 和本地回执 `840bdbed48edab6d70d351fa232b7426903bf3f3098f682e2884f513b9cd0efd`。PR #358 精确 HEAD `2efb809ba090c9af780d8c6be2f75ee707b92d6b` 经 CI run `31665135810` 12/12 SUCCESS 及独立代码安全、QA、产品/规格 P0/P1/P2=0 后，以 merge commit `1f0c2d11dc705be9496eb18c73688d21ee0e8ab5` 合入主干。用户批准后，唯一一次 `--local-check` PASS；唯一正式调用在 Windows 冻结私钥副本的 NTFS ACL 校验处以 `invalid_request`、退出码 2 停止，离线最小复现确认固定 `ssh-keygen -y` 因副本权限退出 255，失败发生在 SSH/SFTP 调用之前。未连接测试服务、未上传、未进入 root 通道、未安装或执行 sudo self-test；009 已消费并禁止重放。消费证据 HEAD `bab8f89a317f9bcb7ca1fd1f534f3fa6a9545f49` 经 CI run `31667550392` 12/12 SUCCESS 及独立代码安全、QA、产品/规格 P0/P1/P2=0 后，由 PR #360 按 merge commit `c9402d94129da4042e3fb1bb978d63018af4a439` 合入主干。009 当时提出的私钥冻结修复已被后续用户批准的 010 直连方案替代；历史证据见 `docs/ai-gateway-g8-test-readonly-access-attempt-20260813-009.md`。
- 010 Drop 直连候选的工程门禁与合并已完成。用户授权后，一次本地检查、一次只读 SSH 与一次原子 SFTP 均 PASS，五文件已暂存；唯一 root 安装编排在本地参数构造阶段停止，未建立 root 连接、未发送安装脚本、未创建 root-only/live/sudoers 目标，也未执行 visudo、sudo 范围、Docker 组或固定 self-test。零重试，业务/上游/费用为 `0 / 0 / 0 CNY`。010 状态为 `CONSUMED_STAGED_ROOT_NOT_RUN`，禁止重放；直接改用 `pc` 不能满足 root-owned 与 sudoers 契约，未执行。见 `docs/ai-gateway-g8-test-readonly-access-attempt-20260813-010.md`。
- 011 方案 A 的仓库工程门禁与合并已完成：PR #365 最终 HEAD `30cf58083088628c0ad8ac321cca3078f39b5341` 经 CI run `31685942115` 12/12 SUCCESS 及独立代码安全、QA、产品/规格 P0/P1/P2=0 后，以 merge commit `018f7344a5a52ccc6c23b478555a7ddc02f5ba63` 合入主干。用户批准后唯一一次 local-check 为 PASS；唯一正式暂存包装器调用以 `invalid_request`、退出码 2、stderr 为空停止并零重试。低敏结果不能区分 SFTP 是否启动或远端五文件是否部分上传，011 暂存保持 `UNKNOWN`；交互 SSH、sudo 认证、root 安装和 self-test 均未执行，业务请求/上游请求/费用为 `0 / 0 / 0 CNY`。011 已消费并禁止重放；消费证据 HEAD `03d53a4dfb22510808b1723d04b69172fce07450` 经 CI run `31689994630` 12/12 SUCCESS 和独立三方 P0/P1/P2=0 后，由 PR #367 按 merge commit `eba0116ad5e790a4d29e32bc68d3151d9d22dc06` 合入主干，远端功能分支已删除。继续诊断或清理须使用新 ChangeId 和独立授权，见 `docs/ai-gateway-g8-test-readonly-access-attempt-20260813-011.md`。
- 012 Drop 暂存只读取证的仓库工程门禁与合并已完成：PR #369 最终 HEAD `6823a6be77c77290732a417935c85af4d213f708` 经 CI run `31697655486` 12/12 SUCCESS 及独立代码安全、QA、产品/规格 P0/P1/P2=0 后，以 merge commit `247c637c2b5bce82377ce1ad1431b4b520187068` 合入主干。用户批准后唯一一次 local-check 返回固定低敏 `evidence_unavailable`、退出码 2、stderr 为空，随即零重试停止；SSH 为 `0/1`，未连接测试服，未形成远端三态证据，011 暂存继续为 `UNKNOWN`。012 已消费并在参数解析、身份材料读取和联网前失败关闭；消费证据 HEAD `3fb4117433b656e5b95ecd7c607002a470441e06` 经 CI run `31700050048` 12/12 SUCCESS 及独立代码安全、QA、产品/规格 P0/P1/P2=0 后，由 PR #371 按 merge commit `80cac83310d97c87e02f80b61e385428e7ed7471` 合入主干，远端功能分支已删除。继续取证必须使用新 ChangeId 和独立授权，见 `docs/ai-gateway-g8-test-readonly-drop-staging-evidence-attempt-20260813-012.md`。
- 013 原方案已因 Windows 本地诊断器最小环境缺陷失效并在代码层墓碑化。修复候选改从 Windows 系统 API 获取可信系统目录，复测本地诊断 PASS；PR #377 已完成 CI、独立复评与主线合并。closeout PR #378 最终 HEAD `ca513432bdd2f5106b9ed19c5872be3207058de8` 的 CI run `31763766230` 成功且独立安全、QA、产品复评均为 P0/P1/P2=0，已按 merge commit `d05012f5c2b1983bd7e60562bbfd816e5f14f550` 合入 main 并删除远端分支。用户随后独立批准 014；唯一一次本地诊断 PASS，唯一一次只读 SSH 返回 `PRESENT / PASS / NONE`、退出码 0，零重试停止，011 暂存从 `UNKNOWN` 收敛为存在且五文件完整。014 已消费并在代码层墓碑化；本轮未执行清理、安装、运行态审计、生产、真实付费、客户流量或真实通知。执行记录见 `docs/ai-gateway-g8-test-readonly-drop-staging-evidence-attempt-20260814-014.md`。
- PR #380 已按 merge commit `102e833b1975d56100faf4d8e4add0150a3edc39` 合入 015 工程候选并删除远端分支。用户独立批准后，015 人工本地段出现 PowerShell 路径正则末尾转义错误；下游影响无法绑定，015 已消费并墓碑化。修复后的 016 经 PR #381、CI run `31776890876` 和独立复评后按 merge commit `2f407cbf3a9c5fea987eeb2f82ebb41630db9e35` 合入 main，合并后冻结摘要复核一致；用户随后批准 016，但唯一人工第一段因交互 PowerShell 无法解析 `Get-FileHash` 终止。该错误发生在唯一 SSH 调用之前，SSH/sudo/安装/业务/上游/费用均为 0，016 已消费并墓碑化。017 新候选改用 Windows PowerShell 5.1 可用的纯 .NET 流式 SHA-256，并关闭加密私钥提示、低敏输出和信号回滚边界；PR #384 最终 HEAD `ee947fd61919215500ef516488d56e01ad2ea72d` 通过 CI run `31791430839` 与三方零缺陷复评，按 merge commit `e2a7e4f89c4115b3e32dc27292b0bc11d7d09a57` 合入 main，合并后原始 blob 与冻结命令复核一致。用户对 017 作出一次独立精确授权后，唯一人工本地段返回固定低敏 `local_gate_failed` 并退出 2；事后纯本地 SSH 前同构门禁通过，但现有审计证据不能证明当时是否已启动 SSH。远端第二段、sudo、安装器、post-check、业务、上游和费用均为 0，安装未确认。017 因 SSH 到达边界不确定而失败关闭消费并墓碑化，禁止重放；继续须使用新 ChangeId。证据见 `docs/ai-gateway-g8-test-readonly-access-install-attempt-20260814-016.md`、`docs/ai-gateway-g8-test-readonly-access-install-attempt-20260814-017.md` 和 `docs/ai-gateway-g8-test-readonly-access-install-authorization-20260814-017.md`。
- 018 工程候选经 PR #388、CI run `31811990627` attempt 2 和独立三方零缺陷复评后，以 merge commit `ef9f65f851cc657eaa6fba7df866ba3c4c7a0912` 合入主干并完成原始 blob 复核。用户独立授权后的唯一人工本地段窗口直接关闭且无可见输出；SSH 启动/连接保持 `UNKNOWN / 最多 1`，远端固定段、sudo、安装器、post-check、业务、上游和费用均为 0。018 已失败关闭消费并墓碑化，禁止重试或重放。019 使用新的 ChangeId `CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-019`，把完整远端固定段作为无秘密 Base64 参数随唯一 `ssh -tt` 调用传入，TTY 仅保留给最多一次 sudo 密码提示；工程候选通过 PR #390、CI run `31829691838` 和独立评审，以 merge commit `70485d893fd86db00be4dbb9e324f9d4322d55b0` 合入主干并完成原始 blob 与冻结命令摘要复核。用户精确授权后，本地门禁和冻结摘要通过，唯一可见 PowerShell 最终在恢复 `$ErrorActionPreference` 时因保存值为 `Null` 失败；窗口固定标志不可恢复，因此 SSH、远端预检、sudo、安装器与 post-check 均保持 `UNKNOWN / 最多 1`。重试为 0，019 已失败关闭消费并墓碑化，测试服安装状态未知，`G8_SOFTWARE_CLOSED_LOOP` 仍未完成。见 `docs/ai-gateway-g8-test-readonly-access-install-attempt-20260815-018.md`、`docs/ai-gateway-g8-test-readonly-access-install-attempt-20260815-019.md` 与 `docs/ai-gateway-g8-test-readonly-access-install-authorization-20260815-019.md`。
- 020 改用独立 ChangeId `CHG-G8-TEST-READONLY-RUNTIME-AUDIT-DROP-20260815-020`：不安装受控入口、不写 `/usr/local/libexec` 或 sudoers、不使用 sudo，由 `pc` 通过既有 Docker 权限执行冻结的内存只读审计。PR #394 工程 merge `3c63539279a34ae2365fc9d7e26e207dd728c4ba`、CI run `31861762018` 与合并后摘要复核均已通过。用户对精确冻结命令作出独立授权后，本地生成和摘要门禁 PASS，但外层 PowerShell 包装在语法解析阶段因缺少右括号失败，未调用 Windows PowerShell 5.1、冻结命令或 SSH。SSH、Docker/HTTP/数据库查询、sudo、安装、宿主写入、业务/上游/费用均为 0，重试 0；020 已按 `CONSUMED_LOCAL_WRAPPER_PARSE_FAILED_SSH_NOT_STARTED` 失败关闭消费并墓碑化，禁止重试或重放。测试服运行态仍无新证据，`G8_SOFTWARE_CLOSED_LOOP` 仍未完成。见 `docs/ai-gateway-g8-test-readonly-runtime-audit-attempt-20260815-020.md`。
- 021 使用独立 ChangeId `CHG-G8-TEST-READONLY-RUNTIME-AUDIT-DROP-20260815-021` 修复 020 的仓库外启动包装缺口：工程 HEAD `c73ef139721bcfc693ffb31caa6fe803be526286` 经 PR #398、CI run `31867790659 completed/success` 和独立评审后，以 merge commit `8bc05cbf3bc71a8954087dc7f26732f836e5212e` 合入 main；合并后原始 blob、LF 与 32009 字节冻结命令摘要均已复核无漂移。唯一授权尝试固定结果为 `CONSUMED_LOCAL_RECEIPT_UNAVAILABLE_SSH_NOT_STARTED`：PowerShell 启动一次后因本地耐久回执不可用失败关闭，`PRE_SSH_GATE`、`SSH_ATTEMPTED`、SSH 与远端命令均为 0，Docker、HTTP、数据库、sudo、安装、业务、上游和费用动作均为 0，重试为 0。021 已永久消费并墓碑化，不得再次授权、重试或重放；未形成测试服运行态证据，`G8_SOFTWARE_CLOSED_LOOP` 仍未完成。见 `docs/ai-gateway-g8-test-readonly-runtime-audit-authorization-20260815-021.md` 与 `docs/ai-gateway-g8-test-readonly-runtime-audit-attempt-20260815-021.md`。
- 022 使用独立 ChangeId `CHG-G8-TEST-READONLY-RUNTIME-AUDIT-DROP-20260815-022` 修复 021 的本地耐久回执缺口。工程 HEAD `fc0344283813bd873aa70520e0b8fcd1da424500` 经 PR #401、CI run `31884793587 completed/success` 和三项独立零缺陷评审后，以 merge commit `84ae5b0ad87958ee63fbfa709c4f164baca39a1b` 合入 main；合并后原始 blob、LF 与 34027 字节冻结命令摘要均已复核无漂移。唯一授权调用在 PowerShell 启动后因 `identity_pair_failed` 于 SSH 前失败关闭；消费归档 TDD 随后错误触发一次未授权本地重放，亦在同一门禁停止。固定状态为 `CONSUMED_LOCAL_IDENTITY_PAIR_FAILED_SSH_NOT_STARTED`：本地正式入口/PowerShell 总调用 `2 / 2`，未授权本地重放 `1`，两份非空耐久回执均无 `PRE_SSH_GATE`/`SSH_ATTEMPTED`，SSH、远端命令及全部测试服能力为 0。022 已永久消费并墓碑化，不得再次授权、重试或重放；`G8_SOFTWARE_CLOSED_LOOP` 仍未完成。见 `docs/ai-gateway-g8-test-readonly-runtime-audit-authorization-20260815-022.md` 与 `docs/ai-gateway-g8-test-readonly-runtime-audit-attempt-20260815-022.md`。
- 023 使用独立 ChangeId `CHG-G8-TEST-READONLY-RUNTIME-AUDIT-DROP-20260815-023`，取消固定客户端私钥、公钥和指纹配对门禁，改用开发机现有 OpenSSH 免交互认证链。工程 merge `1eb23c8b87720cceea64dcfc349b0a9b9c04de4b` 与 32954 字节冻结命令经复核后取得一次精确授权；唯一正式调用形成 `PRE_SSH_GATE=PASS` 与 `SSH_ATTEMPTED=YES`，随后 SSH 会话非零并固定返回 `ssh_session_failed`，零重试停止。固定状态为 `CONSUMED_SSH_SESSION_FAILED_REMOTE_AUDIT_NOT_PROVEN`：SSH 调用 `1`、会话成功 `0`，远端固定脚本与 Docker 只读查询为 `UNKNOWN / 最多启动 1 次`；sudo、安装、Docker 变更、宿主写入、业务 HTTP、数据库写入、migration、真实上游和费用动作均为 0。023 已永久消费并墓碑化，不得再次授权、重试或重放；`G8_SOFTWARE_CLOSED_LOOP` 仍未完成。
- 024 使用独立 ChangeId `CHG-G8-TEST-READONLY-SSH-DIAGNOSTIC-20260816-024`，把 023 的笼统 `ssh_session_failed` 收敛为最小 SSH 连接诊断：不固定 `-i`/`IdentitiesOnly`，保留 OpenSSH 默认身份文件与当前 `SSH_AUTH_SOCK`，同时以 `-F none` 隔离可执行任意本地命令或扩张信任面的用户 `ssh_config`；固定目标、唯一 host key 信任源、BatchMode、单会话和零重试，只允许远端固定 `printf` 回执，并将认证、host key、超时、拒绝、网络不可达、其他传输及远端回执失败映射为固定低敏原因。工程 HEAD `97876c03baeed226362aaa304fb1a30e959ac42a` 经 PR #407、CI run `31897233312 completed/success` 和三项独立零缺陷评审后，以 merge commit `ffca18aace03fd9185280fb7a2b2807d337a590d` 合入 main；合并后原始 blob、SHA-256 与 LF 摘要复核一致，远端工程分支已删除。当前为 `PENDING_USER_APPROVAL / REMOTE_NOT_AUTHORIZED`；尚未执行 024、SSH、Docker、sudo、安装、HTTP、数据库或业务操作，`G8_SOFTWARE_CLOSED_LOOP` 仍未完成。
- G8 真实请求证据闭环修复当前为本地工程候选：新发生的成功结算会在同一事务内幂等同步 `token_usage_logs`，并以 8 位金额精度逐字段校验其 Token 汇总、销售金额与权威 Usage、`settled_amount` 一致；人工核定未知或取消执行时保留待对账来源状态，不伪造执行成功。Bifrost 驱动仅从固定白名单响应字段提取低敏上游请求引用并写入执行尝试，继续保持 `enable_logging=false`。该候选尚未部署测试服，不修改或补写历史请求，也未重发 Chat 或调用 Provider；代码与 CI 通过不能替代测试服运行态验收，`G8_SOFTWARE_CLOSED_LOOP` 仍未完成。
...
| [视频网关 VID-G6 内部回调合同](docs/video-gateway-vid-g6-callback-contract.md) | Fake专用验签、nonce与原Callback/Task同事务；真实HTTP基础通过，完整故障及兼容验收进行中 |
| [视频网关 VID-G6 长期副本读取](docs/video-gateway-vid-g6-saved-read-contract.md) | 当前资格、原Key归属、独立存储和共享下载租约验证中；不代表永久商业保存承诺 |
...
> 最后更新：2026-08-24
>
> **后端进度与接口现状（2026-08-24）**：已按 `main@2889e81` 重新盘点源码路由、模块装配条件和验收边界。当前识别 303 条模块路由注册，加 3 条基础健康/版本路由；其中平台 SK、Token 网关、工作台和有状态会话受密钥与上游装配条件控制，不等于默认全部开放。本地 Go 1.26.5 执行 `go test ./...` 全部通过。模块进度、接口内容、条件路由和“代码/测试服/生产/商业”分层结论见 [`docs/backend-development-progress-and-api-status.md`](docs/backend-development-progress-and-api-status.md)。
>

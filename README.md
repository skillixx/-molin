...
- G8 真实请求证据闭环修复当前为本地工程候选：新发生的成功结算会在同一事务内幂等同步 `token_usage_logs`，并逐字段校验其 Token 汇总、销售金额与权威 Usage、`settled_amount` 一致；Bifrost 驱动仅从固定白名单响应字段提取低敏上游请求引用并写入执行尝试，继续保持 `enable_logging=false`。该候选尚未部署测试服，不修改或补写历史请求，也未重发 Chat 或调用 Provider；代码与 CI 通过不能替代测试服运行态验收，`G8_SOFTWARE_CLOSED_LOOP` 仍未完成。

...
- G8 测试服最小只读入口候选包门禁已收口：PR #333 精确 HEAD `c0479f607c9dbd5713c9fbbde7b3fb83ac2a3adc` 的 CI run `31566629193` 为 9/9 SUCCESS，独立代码安全、QA、产品/规格均为 P0/P1/P2=0，并按 merge commit 合并为 `69439c4c9b14c67bf8a17dd8822d80ecdc784a27`。该结论只证明仓库候选包可复现且失败关闭；测试服 live 入口尚未安装，003 暂存上传状态为 `UNKNOWN`，现有 API 停止、运行态 P1=3 及 schema/Bifrost/监控/账务 UNKNOWN 仍未关闭。
...
- 测试入口实际由 Drop 服务映射，底层物理主机身份不是适用门禁；007 的 `READABLE_MISMATCH` 作为历史事实保留，但不再登记为当前测试服运行态 P1。008 Drop 暂存只读取证候选仅验证固定 SSH 传输端点、登录用户、部署根和 003 固定五文件，不读取 hostname 或 machine-id。PR #353 最终 HEAD `59cc103862e710e56749c7e1875a734783e4d754` 已通过 CI run `31658983454` 12/12 和独立代码安全、QA、产品规格零缺陷门禁，并以 merge commit `670b39dd316a53af7c7baa639c9822b1a65994aa` 合并；当前状态为 `PENDING_USER_APPROVAL`，尚未执行本地检查或连接测试服务，003 暂存仍为 `UNKNOWN`。执行仍需另获独立授权，见 `docs/ai-gateway-g8-test-readonly-drop-staging-evidence-authorization-20260813-008.md`。

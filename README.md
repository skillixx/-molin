...
- PR #380 已按 merge commit `102e833b1975d56100faf4d8e4add0150a3edc39` 合入 015 工程候选并删除远端分支。用户独立批准后，015 人工本地段出现 PowerShell 路径正则末尾转义错误；该错误默认非终止，现有记录不能证明后续身份材料读取、SSH 或远端影响，相关状态保持 `UNKNOWN`，重试 0，015 已消费并墓碑化。修复后的 016 把错误停止与恢复策略写入真实生成段，仅为新的工程候选；当前状态为 `PENDING_ENGINEERING_REVIEW / REMOTE_NOT_AUTHORIZED`，不继承 015 执行授权。失败记录与新清单见 `docs/ai-gateway-g8-test-readonly-access-install-attempt-20260814-015.md`、`docs/ai-gateway-g8-test-readonly-access-install-authorization-20260814-016.md`。

...
- `CHG-G8-TEST-READONLY-HOST-IDENTITY-DIAG-20260812-007` 正在完成仓库工程门禁：候选只把 006 的 `MACHINE_ID` 阻断低敏区分为 `READABLE_MATCH`、`READABLE_MISMATCH` 或 `UNREADABLE`，不输出当前 machine-id 原文或摘要，也不读取暂存目录。当前未连接测试服；授权清单状态为 `PENDING_ENGINEERING_GATES_AND_USER_APPROVAL`，工程合并不等于取得单次 SSH 授权。

...
- 023 使用独立 ChangeId `CHG-G8-TEST-READONLY-RUNTIME-AUDIT-DROP-20260815-023`，取消固定客户端私钥、公钥和指纹配对门禁，改用开发机现有 OpenSSH 免交互认证链；固定目标、服务器 host key、BatchMode、单会话和零重试保持不变。当前为 `PENDING_ENGINEERING_REVIEW / REMOTE_NOT_AUTHORIZED`，只允许离线假 SSH、CI 与评审，不授权执行 023 或连接测试服务器；`G8_SOFTWARE_CLOSED_LOOP` 仍未完成。

...
| 阿里云短信验证码阶段 5（灰度部署、代理核验与观察） | `modules/sms/`、内部 metrics、`infra/nginx/`、`infra/prometheus/`、`docs/sms-phase5-*` | 🟡 阶段 5A 关闭态部署已完成：固定代理、可信配置、新后端、4 条规则、管理 GET/鉴权、真实页面和四档响应式均通过；11 份回滚材料、外部固定清单摘要、文件哈希、架构、三份容器快照及其镜像、当前运行 PID/监听只读预检通过。旧环境虽保持短信关闭，但缺少固定代理信任且含两项废弃模板键，禁止整份恢复；应基于当前环境保留代理配置并排除废弃键，恢复运行时仍未验证。journald 正常运行且使用持久目录，但容量、保留空间、留存时间和轮转周期四项显式策略均缺失，配置完整性和策略验收均未通过。写请求、敏感展示、业务配置变更、数据库/Provider 增量及真实短信均为 0；只读访问可能增加 SSH/API 审计日志。当前无 Alertmanager 引用、实例或 9093 监听，通知链需先配置接收渠道；日志策略变更、实际回滚、真实 Canary、生产发布、24 小时观察、QA/产品验收及 PR/CI/合并仍待独立门禁 |

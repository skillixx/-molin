...
- 022 使用独立 ChangeId `CHG-G8-TEST-READONLY-RUNTIME-AUDIT-DROP-20260815-022` 修复 021 的本地耐久回执缺口。本地离线复现确认，021 生成的 PowerShell 回执目录正则因普通 Python 字符串反斜杠折叠而无效；022 改用 LocalApplicationData、固定盘和逐级非 reparse 校验，在唯一 GUID 文件上执行 CreateNew，并将目录/权限、预占、写入和刷盘故障分别固定低敏收敛。当前仅为 `PENDING_ENGINEERING_REVIEW / REMOTE_NOT_AUTHORIZED` 工程候选，尚未执行或消费；不授权 SSH、测试服 Docker、HTTP、数据库、sudo、安装、migration、业务、上游或费用动作，`G8_SOFTWARE_CLOSED_LOOP` 仍未完成。见 `docs/ai-gateway-g8-test-readonly-runtime-audit-authorization-20260815-022.md`。

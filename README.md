...
- 013 原方案已因 Windows 本地诊断器最小环境缺陷失效并在代码层墓碑化，任何调用都在参数解析、材料读取和联网前返回 `change_id_consumed`。2026-08-14 首次真实本地诊断未联网并暴露 `PROGRAMDATA` 缺失问题；修复候选改从 Windows 系统 API 获取可信系统目录，复测本地诊断 PASS。PR #377 最终 HEAD `477a2d3da9b672c1fbc8b792de5eef7a4ed29af1` 的 CI run `31762838448` 全绿且独立安全、QA、产品复评均为 P0/P1/P2=0，已按 merge commit `3c9c5cf489b28e45b789c114243e45936a0d81d2` 合入 main 并删除远端分支。新的 014 只读取证处于 `PENDING_USER_APPROVAL / REMOTE_NOT_AUTHORIZED`，尚未授权连接测试服；011 暂存继续为 `UNKNOWN`。本轮也不授权清理、安装、运行态审计、生产、真实付费、客户流量或真实通知。

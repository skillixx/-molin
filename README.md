...
| 阿里云短信验证码阶段 2（模板同步、场景绑定、9 个管理 API 与安全测试发送） | `modules/sms/`、`modules/auth/`、`modules/iam/`、`server/migrations/000059*` | ⛔ PR #315 的代码、CI、部署和受控收件证据已形成；开发分支已补充“同一模板不得绑定多个启用场景”的后端约束，但测试服仍只有 1 个合格模板且五场景共用。QA/PM 验收不通过，禁止合并及进入阶段 3 |

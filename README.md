...
- 023 使用独立 ChangeId `CHG-G8-TEST-READONLY-RUNTIME-AUDIT-DROP-20260815-023`，取消固定客户端私钥、公钥和指纹配对门禁，改用开发机现有 OpenSSH 免交互认证链；固定目标、服务器 host key、BatchMode、单会话和零重试保持不变。工程 HEAD `9a969d4dd2881e659c50ab694a4d35b57adba803` 经 PR #404、CI run `31892659673 completed/success` 和三项独立零缺陷评审后，以 merge commit `1eb23c8b87720cceea64dcfc349b0a9b9c04de4b` 合入 main；合并后原始 Git blob、LF 与 32954 字节冻结命令摘要均已复核无漂移。当前为 `PENDING_USER_APPROVAL / REMOTE_NOT_AUTHORIZED`，尚未执行或消费；不授权执行 023、SSH 或任何测试服操作，`G8_SOFTWARE_CLOSED_LOOP` 仍未完成。

...
- 018 工程候选经 PR #388、CI run `31811990627` attempt 2 和独立三方零缺陷复评后，以 merge commit `ef9f65f851cc657eaa6fba7df866ba3c4c7a0912` 合入主干并完成原始 blob 复核。用户独立授权后的唯一人工本地段窗口直接关闭且无可见输出；SSH 启动/连接保持 `UNKNOWN / 最多 1`，远端固定段、sudo、安装器、post-check、业务、上游和费用均为 0。018 已失败关闭消费并墓碑化，禁止重试或重放。019 使用新的 ChangeId `CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-019`，把完整远端固定段作为无秘密 Base64 参数随唯一 `ssh -tt` 调用传入，TTY 仅保留给最多一次 sudo 密码提示；父 PowerShell 只输出固定结果和设置退出码，不主动关闭窗口。019 工程候选已通过 PR #390、CI run `31829691838` 和独立代码安全、QA、产品/规格评审，以 merge commit `70485d893fd86db00be4dbb9e324f9d4322d55b0` 合入主干并完成原始 blob 与冻结命令摘要复核。当前为 `PENDING_USER_APPROVAL / REMOTE_NOT_AUTHORIZED`，尚未执行 019 的 SSH、sudo 或安装，`G8_SOFTWARE_CLOSED_LOOP` 仍未完成。见 `docs/ai-gateway-g8-test-readonly-access-install-attempt-20260815-018.md` 与 `docs/ai-gateway-g8-test-readonly-access-install-authorization-20260815-019.md`。

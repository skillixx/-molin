...
- 020 改用独立 ChangeId `CHG-G8-TEST-READONLY-RUNTIME-AUDIT-DROP-20260815-020`：不安装受控入口、不写 `/usr/local/libexec` 或 sudoers、不使用 sudo，由 `pc` 在单次非交互 SSH 中通过既有 Docker 权限执行冻结的内存只读审计。Docker 权限接近宿主 root，因此工程门禁反向禁止容器变更、任意参数入口、宿主写入、migration、业务请求和费用动作，并以耐久低敏回执保留 SSH 到达边界。当前仅为 `PENDING_ENGINEERING_REVIEW / REMOTE_NOT_AUTHORIZED` 工程候选；019 永久墓碑保持不变，020 尚未执行、尚未消费，本轮不授权 SSH、Docker 命令、HTTP、数据库查询或任何测试服操作，`G8_SOFTWARE_CLOSED_LOOP` 仍未完成。见 `docs/ai-gateway-g8-test-readonly-runtime-audit-authorization-20260815-020.md`。

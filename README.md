...
- 024 使用独立 ChangeId `CHG-G8-TEST-READONLY-SSH-DIAGNOSTIC-20260816-024`，把 023 的笼统 `ssh_session_failed` 收敛为最小 SSH 连接诊断：移除阻断用户认证配置的 `-F none`，保留固定目标、固定 host key、BatchMode、单会话和零重试，只允许远端固定 `printf` 回执，并将认证、host key、超时、拒绝、网络不可达、其他传输及远端回执失败映射为固定低敏原因。当前为 `PENDING_ENGINEERING_REVIEW / REMOTE_NOT_AUTHORIZED`；本轮未执行 024、SSH、Docker、sudo、安装、HTTP、数据库或业务操作，`G8_SOFTWARE_CLOSED_LOOP` 仍未完成。

...
- `CHG-G8-TEST-READONLY-STAGING-EVIDENCE-20260812-006` 仓库候选已通过 PR #345、CI 12/12 和独立代码安全/QA/产品门禁并合入主干。用户随后批准唯一一次本地检查和正式只读 SSH：本地检查 PASS，正式调用返回固定 `BLOCKED / MACHINE_ID` 后零重试停止，业务请求、上游请求、费用为 `0 / 0 / 0 CNY`。暂存查找未执行，003 暂存继续为 `UNKNOWN`；006 已消费，禁止重放。证据见 `docs/ai-gateway-g8-test-readonly-staging-evidence-attempt-20260812-006.md`。

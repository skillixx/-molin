...
- 已开始准备 `CHG-G8-TEST-READONLY-STAGING-EVIDENCE-20260812-004` 的完全只读暂存取证候选：只允许一次固定 SSH，核对主机、部署根、003 暂存路径、五文件白名单、类型、属主、权限、大小和冻结摘要；结果严格区分 `ABSENT`、`PRESENT/PASS` 与 `PRESENT/MISMATCH`。当前仅为仓库候选，尚未取得远端执行授权，未再次连接测试服；不得下载、删除、sudo、读取业务数据或顺带安装。授权清单见 `docs/ai-gateway-g8-test-readonly-staging-evidence-authorization-20260812-004.md`。

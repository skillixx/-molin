...
- 021 使用独立 ChangeId `CHG-G8-TEST-READONLY-RUNTIME-AUDIT-DROP-20260815-021` 修复 020 的仓库外启动包装缺口：工程候选把 merge 原始 blob 复核、冻结命令内存生成、大小/SHA-256 校验、PowerShell 5.1 解析和唯一启动收敛为仓库内固定 Python 入口；Windows 以假 SSH 动态验证解析失败零触达与成功单次调用，Linux 以只读断网容器验证。当前为 `PENDING_ENGINEERING_REVIEW / REMOTE_NOT_AUTHORIZED`，尚未执行、尚未消费，不授权 SSH、测试服 Docker、HTTP、数据库、sudo、安装、业务、上游或费用动作，`G8_SOFTWARE_CLOSED_LOOP` 仍未完成。见 `docs/ai-gateway-g8-test-readonly-runtime-audit-authorization-20260815-021.md`。

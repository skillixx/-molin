...
- 018 工程候选使用独立 ChangeId `CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-018`，只继续使用 014 已证明完整的固定 011 暂存资产。候选把低敏失败细分为可信系统路径、材料摘要、known_hosts、密钥配对、材料漂移与 SSH 会话六类，并在唯一 SSH 调用前固定形成 `PRE_SSH_GATE=PASS` 与紧邻调用的 `SSH_ATTEMPTED=YES`，从而避免再次出现 SSH 到达边界不可诊断。PR #388 最终 HEAD `715376dd330324891ef7568ea0da06c8b8d0a95e` 经 CI run `31811990627` attempt 2 全部适用门禁成功及独立三方零缺陷复评后，以 merge commit `ef9f65f851cc657eaa6fba7df866ba3c4c7a0912` 合入主干；合并后原始 blob 与纯内存命令摘要复核一致。当前状态为 `PENDING_USER_APPROVAL / REMOTE_NOT_AUTHORIZED`；017 墓碑永久保留，018 尚未安装，工程合并不构成远端授权，也不代表 `G8_SOFTWARE_CLOSED_LOOP` 完成。冻结范围见 `docs/ai-gateway-g8-test-readonly-access-install-authorization-20260814-018.md`。

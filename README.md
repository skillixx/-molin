...
- 011 方案 A 已进入仓库工程门禁：使用新 ChangeId 和独立五文件候选，把未来执行拆分为一次原子 SFTP 与一次交互 SSH；密码只允许由 `sudo -k -v` 从操作者 TTY 读取，仓库脚本、命令、日志和文档均不接收密码。当前状态为 `PENDING_ENGINEERING_GATES_AND_USER_APPROVAL`，尚未执行 local-check、SFTP、SSH、sudo 或安装，见 `docs/ai-gateway-g8-test-readonly-access-install-authorization-20260813-011.md`。

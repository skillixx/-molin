...
- 018 工程候选使用独立 ChangeId `CHG-G8-TEST-READONLY-ACCESS-INSTALL-DROP-20260814-018`，只继续使用 014 已证明完整的固定 011 暂存资产。候选把低敏失败细分为可信系统路径、材料摘要、known_hosts、密钥配对、材料漂移与 SSH 会话六类，并在唯一 SSH 调用前固定形成 `PRE_SSH_GATE=PASS` 与紧邻调用的 `SSH_ATTEMPTED=YES`，从而避免再次出现 SSH 到达边界不可诊断。本地 Windows 与断网 Linux 门禁已通过，当前状态为 `PENDING_ENGINEERING_REVIEW / REMOTE_NOT_AUTHORIZED`；017 墓碑永久保留，018 尚未安装，本地测试或工程合并都不构成远端授权，也不代表 `G8_SOFTWARE_CLOSED_LOOP` 完成。冻结范围见 `docs/ai-gateway-g8-test-readonly-access-install-authorization-20260814-018.md`。

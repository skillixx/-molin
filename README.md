...
- `CHG-G8-TEST-READONLY-HOST-IDENTITY-DIAG-20260812-007` 仓库工程门禁已完成：功能 HEAD `21cc6698df1aa0b48de66213c97a4a31d074fcb5` 的 CI run `31618217697` 为 12/12 SUCCESS，独立代码安全、QA、产品/规格均为 P0/P1/P2=0；PR #349 已按 merge commit `9f5c028232e03bd1d8ad99fb99211680af790625` 合入主干并删除远端分支。候选只把 006 的 `MACHINE_ID` 阻断低敏区分为 `READABLE_MATCH`、`READABLE_MISMATCH` 或 `UNREADABLE`，不输出当前 machine-id 原文或摘要，也不读取暂存目录。当前未再次连接测试服；授权清单状态为 `PENDING_USER_APPROVAL`，工程合并不等于取得单次 SSH 授权。

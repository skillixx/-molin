...
- 013 采用本地与远端授权解耦方案：无 ChangeId 本地诊断器可重复且没有 SSH/SFTP/远端访问能力；一次性 013 包装器不提供 `--local-check`，只在未来另行明确授权后最多执行一次固定只读 SSH。当前仅完成仓库候选，状态为 `PENDING_ENGINEERING_GATES_AND_USER_APPROVAL`，尚未运行真实本地材料诊断或连接测试服，011 暂存仍为 `UNKNOWN`。后续清理、审计入口安装、测试候选部署和运行态审计分别使用新的 ChangeId 与独立授权。

...
- 测试入口实际由 Drop 服务映射，底层物理主机身份不是适用门禁；007 的 `READABLE_MISMATCH` 作为历史事实保留，但不再登记为当前测试服运行态 P1。008 Drop 暂存只读取证候选仅验证固定 SSH 传输端点、登录用户、部署根和 003 固定五文件，不读取 hostname 或 machine-id。当前仅完成仓库候选，尚未连接测试服务，003 暂存仍为 `UNKNOWN`；执行仍需另获独立授权，见 `docs/ai-gateway-g8-test-readonly-drop-staging-evidence-authorization-20260813-008.md`。

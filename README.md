...
- 013 原方案已因 Windows 本地诊断器最小环境缺陷失效并在代码层墓碑化，任何调用都在参数解析、材料读取和联网前返回 `change_id_consumed`。2026-08-14 首次真实本地诊断未联网并暴露 `PROGRAMDATA` 缺失问题；修复候选改从 Windows 系统 API 获取可信系统目录，复测本地诊断 PASS。新的 014 只读取证仍处于工程评审阶段，未授权连接测试服；011 暂存继续为 `UNKNOWN`。本轮也不授权清理、安装、运行态审计、生产、真实付费、客户流量或真实通知。

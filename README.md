...
- 012 Drop 暂存只读取证处于仓库工程阶段：ChangeId `CHG-G8-TEST-READONLY-STAGING-EVIDENCE-DROP-20260813-012` 只面向固定 011 暂存，冻结五文件、manifest、回执、Drop 端点和客户端 ED25519 信任。当前状态为 `PENDING_ENGINEERING_GATES_AND_USER_APPROVAL`，未运行 local-check、未连接测试服；工程测试、CI、评审或合并都不构成远端执行、清理、安装或生产授权。授权边界见 `docs/ai-gateway-g8-test-readonly-drop-staging-evidence-authorization-20260813-012.md`。

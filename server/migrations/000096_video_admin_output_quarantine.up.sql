-- 行政隔离保留原审核和标识结论；准备凭据与资产CAS、后审计在服务事务内完成。
CREATE TABLE IF NOT EXISTS ai_video_admin_output_quarantines (
 ...
 UNIQUE KEY uk_video_admin_output_command(actor_user_id,command_key_hash),
 UNIQUE KEY uk_video_admin_output_before(before_audit_id),
 UNIQUE KEY uk_video_admin_output_after(after_audit_id),
 CONSTRAINT fk_video_admin_output_actor FOREIGN KEY(actor_user_id) REFERENCES users(id),
 CONSTRAINT fk_video_admin_output_asset FOREIGN KEY(asset_id,user_id,project_id) REFERENCES ai_gateway_assets(id,user_id,project_id),
 CONSTRAINT fk_video_admin_output_task FOREIGN KEY(task_id,request_id,user_id,project_id) REFERENCES ai_gateway_tasks(id,request_id,user_id,project_id),
 CONSTRAINT fk_video_admin_output_key FOREIGN KEY(api_key_id,project_id,user_id) REFERENCES api_keys(id,project_id,user_id),
 CONSTRAINT fk_video_admin_output_before FOREIGN KEY(before_audit_id) REFERENCES audit_logs(id),
 CONSTRAINT fk_video_admin_output_after FOREIGN KEY(after_audit_id) REFERENCES audit_logs(id),
 CONSTRAINT chk_video_admin_output_envelope CHECK(initial_version>0 AND final_version=initial_version+1 AND initial_state IN ('temporary','available') AND snapshot_sha256 REGEXP '^[0-9a-f]{64}$' AND command_key_hash REGEXP '^[0-9a-f]{64}$' AND reason_hmac REGEXP '^[0-9a-f]{64}$' AND reason_length BETWEEN 1 AND 256 AND OCTET_LENGTH(key_version)>0 AND OCTET_LENGTH(ciphertext) BETWEEN 17 AND 1040 AND ciphertext_sha256=LOWER(SHA2(ciphertext,256)) AND aad_sha256 REGEXP '^[0-9a-f]{64}$' AND ((status='prepared' AND version_no=1 AND after_audit_id IS NULL) OR (status='completed' AND version_no=2 AND after_audit_id IS NOT NULL AND after_audit_id<>before_audit_id)))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
DELIMITER $$
DROP PROCEDURE IF EXISTS vid_g6_admin_output_schema$$
CREATE PROCEDURE vid_g6_admin_output_schema()
BEGIN
 IF NOT EXISTS(SELECT 1 FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='ai_gateway_assets' AND column_name='admin_quarantine_command_id') THEN
  ALTER TABLE ai_gateway_assets ADD COLUMN admin_quarantine_command_id BIGINT UNSIGNED NULL COMMENT '行政隔离原命令，不对普通DTO公开';
 END IF;
 IF NOT EXISTS(SELECT 1 FROM information_schema.table_constraints WHERE constraint_schema=DATABASE() AND table_name='ai_gateway_assets' AND constraint_name='fk_video_asset_admin_quarantine') THEN
  ALTER TABLE ai_gateway_assets ADD CONSTRAINT fk_video_asset_admin_quarantine FOREIGN KEY(admin_quarantine_command_id) REFERENCES ai_video_admin_output_quarantines(id);
 END IF;
 -- 在同一ALTER中替换约束，保留原图片与G4失败隔离分支，仅增加受触发器和FK保护的视频行政分支。
 ALTER TABLE ai_gateway_assets DROP CHECK chk_ai_gateway_assets_quarantine,
 ADD CONSTRAINT chk_ai_gateway_assets_quarantine CHECK(lifecycle_state<>'quarantined' OR moderation_status IN ('rejected','error') OR (modality='video' AND moderation_status='passed' AND (explicit_label_status='failed' OR implicit_label_status='failed')) OR (modality='video' AND admin_quarantine_command_id IS NOT NULL));
 IF NOT EXISTS(SELECT 1 FROM information_schema.table_constraints WHERE constraint_schema=DATABASE() AND table_name='ai_gateway_assets' AND constraint_name='chk_video_asset_admin_quarantine_shape') THEN
  ALTER TABLE ai_gateway_assets ADD CONSTRAINT chk_video_asset_admin_quarantine_shape CHECK(admin_quarantine_command_id IS NULL OR (modality='video' AND lifecycle_state='quarantined'));
 END IF;
END$$
CALL vid_g6_admin_output_schema()$$
DROP PROCEDURE vid_g6_admin_output_schema$$
DROP TRIGGER IF EXISTS trg_video_admin_output_prepare$$
CREATE TRIGGER trg_video_admin_output_prepare BEFORE INSERT ON ai_video_admin_output_quarantines FOR EACH ROW
BEGIN
 IF NEW.status<>'prepared' OR NEW.version_no<>1 OR NEW.after_audit_id IS NOT NULL OR NOT EXISTS(
 SELECT 1 FROM ai_gateway_assets a JOIN ai_gateway_tasks t ON t.id=a.task_id AND t.request_id=a.request_id AND t.user_id=a.user_id AND t.project_id=a.project_id
 ...
 WHERE a.id=NEW.asset_id AND a.task_id=NEW.task_id AND a.request_id=NEW.request_id AND a.user_id=NEW.user_id AND a.project_id=NEW.project_id
 AND a.modality='video' AND t.capability='video.generate' AND r.capability='video.generate' AND r.modality='video' AND t.api_key_id<=>NEW.api_key_id
 AND a.lifecycle_state=NEW.initial_state AND a.version_no=NEW.initial_version AND a.admin_quarantine_command_id IS NULL
 AND NEW.snapshot_sha256=LOWER(SHA2(CAST(JSON_OBJECT('public_id',a.public_id,'user_id',a.user_id,'project_id',a.project_id,'request_id',a.request_id,'task_id',a.task_id,'result_index',a.result_index,'asset_role',a.asset_role,'parent_asset_id',a.parent_asset_id,'is_billable_output',a.is_billable_output,'bucket',a.bucket,'object_key',a.object_key,'mime_type',a.mime_type,'size_bytes',a.size_bytes,'sha256',a.sha256,'width',a.width,'height',a.height,'modality',a.modality,'duration_seconds',a.duration_seconds,'frame_rate',a.frame_rate,'container',a.container,'video_codec',a.video_codec,'audio_codec',a.audio_codec,'has_audio',a.has_audio,'source',a.source,'moderation_status',a.moderation_status,'moderation_policy_version',a.moderation_policy_version,'explicit_label_status',a.explicit_label_status,'explicit_label_version',a.explicit_label_version,'implicit_label_status',a.implicit_label_status,'implicit_label_version',a.implicit_label_version,'retention_policy_id',a.retention_policy_id,'created_at',a.created_at) AS CHAR CHARACTER SET utf8mb4),256))
 AND b.operator_id=NEW.actor_user_id AND b.module='token_gateway' AND b.action='video_admin_output_quarantine_before' AND b.target_type='video_output_asset' AND b.target_id=a.public_id
 AND JSON_UNQUOTE(JSON_EXTRACT(b.request_summary,'$.command_key_hash'))=NEW.command_key_hash AND JSON_UNQUOTE(JSON_EXTRACT(b.request_summary,'$.reason_hmac'))=NEW.reason_hmac
 ) THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='video_admin_output_prepare_invalid'; END IF;
END$$
DROP TRIGGER IF EXISTS trg_video_asset_admin_insert$$
CREATE TRIGGER trg_video_asset_admin_insert BEFORE INSERT ON ai_gateway_assets FOR EACH ROW
BEGIN
 IF NEW.admin_quarantine_command_id IS NOT NULL THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='video_admin_output_insert_forbidden'; END IF;
END$$
...
BEGIN
 IF OLD.admin_quarantine_command_id IS NOT NULL THEN
 ...
 END IF;
END$$
DROP TRIGGER IF EXISTS trg_video_admin_output_complete$$
CREATE TRIGGER trg_video_admin_output_complete BEFORE UPDATE ON ai_video_admin_output_quarantines FOR EACH ROW
BEGIN
 ...
 OR NOT (NEW.id<=>OLD.id) OR NOT (NEW.actor_user_id<=>OLD.actor_user_id) OR NOT (NEW.command_key_hash<=>OLD.command_key_hash) OR NOT (NEW.asset_id<=>OLD.asset_id) OR NOT (NEW.task_id<=>OLD.task_id) OR NOT (NEW.request_id<=>OLD.request_id) OR NOT (NEW.user_id<=>OLD.user_id) OR NOT (NEW.project_id<=>OLD.project_id) OR NOT (NEW.api_key_id<=>OLD.api_key_id) OR NOT (NEW.initial_version<=>OLD.initial_version) OR NOT (NEW.final_version<=>OLD.final_version) OR NOT (NEW.initial_state<=>OLD.initial_state) OR NOT (NEW.snapshot_sha256<=>OLD.snapshot_sha256) OR NOT (NEW.reason_hmac<=>OLD.reason_hmac) OR NOT (NEW.reason_length<=>OLD.reason_length) OR NOT (NEW.key_version<=>OLD.key_version) OR NOT (NEW.nonce<=>OLD.nonce) OR NOT (NEW.ciphertext<=>OLD.ciphertext) OR NOT (NEW.aad_sha256<=>OLD.aad_sha256) OR NOT (NEW.ciphertext_sha256<=>OLD.ciphertext_sha256) OR NOT (NEW.before_audit_id<=>OLD.before_audit_id) OR NOT (NEW.created_at<=>OLD.created_at)
 OR NOT EXISTS(SELECT 1 FROM ai_gateway_assets a JOIN audit_logs b ON b.id=NEW.after_audit_id WHERE a.id=NEW.asset_id AND a.admin_quarantine_command_id=NEW.id AND a.lifecycle_state='quarantined' AND a.version_no=NEW.final_version AND NEW.snapshot_sha256=LOWER(SHA2(CAST(JSON_OBJECT('public_id',a.public_id,'user_id',a.user_id,'project_id',a.project_id,'request_id',a.request_id,'task_id',a.task_id,'result_index',a.result_index,'asset_role',a.asset_role,'parent_asset_id',a.parent_asset_id,'is_billable_output',a.is_billable_output,'bucket',a.bucket,'object_key',a.object_key,'mime_type',a.mime_type,'size_bytes',a.size_bytes,'sha256',a.sha256,'width',a.width,'height',a.height,'modality',a.modality,'duration_seconds',a.duration_seconds,'frame_rate',a.frame_rate,'container',a.container,'video_codec',a.video_codec,'audio_codec',a.audio_codec,'has_audio',a.has_audio,'source',a.source,'moderation_status',a.moderation_status,'moderation_policy_version',a.moderation_policy_version,'explicit_label_status',a.explicit_label_status,'explicit_label_version',a.explicit_label_version,'implicit_label_status',a.implicit_label_status,'implicit_label_version',a.implicit_label_version,'retention_policy_id',a.retention_policy_id,'created_at',a.created_at) AS CHAR CHARACTER SET utf8mb4),256))
 AND b.operator_id=NEW.actor_user_id AND b.module='token_gateway' AND b.action='video_admin_output_quarantine_after' AND b.target_type='video_output_asset' AND b.target_id=a.public_id
 AND JSON_UNQUOTE(JSON_EXTRACT(b.request_summary,'$.command_key_hash'))=NEW.command_key_hash AND JSON_UNQUOTE(JSON_EXTRACT(b.request_summary,'$.reason_hmac'))=NEW.reason_hmac)
 THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='video_admin_output_completion_invalid'; END IF;
END$$
DROP TRIGGER IF EXISTS trg_video_admin_output_delete$$
CREATE TRIGGER trg_video_admin_output_delete BEFORE DELETE ON ai_video_admin_output_quarantines FOR EACH ROW
BEGIN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='video_admin_output_immutable'; END$$
DELIMITER ;

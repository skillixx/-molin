...
func TestOpenRouterImageAdapterStrictRequestAndResponse(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if body["model"] != "provider/image" || body["stream"] != false || provider["allow_fallbacks"] != false || body["prompt"] != "内存测试" {
	...
	adapter, err := newOpenRouterImageAdapter(OpenRouterImageAdapterConfig{APIKey: "fake-openrouter-key-123456", ProviderTag: "seed", Timeout: time.Second, ModelMap: map[string]string{"molin/image": "provider/image"}}, server.URL+"/api/v1/images", client, true)
	if err != nil {
		t.Fatal(err)
	}
	result, err := adapter.Generate(context.Background(), ProviderImageRequest{RequestID: "req-1", ModelCode: "molin/image", Prompt: "内存测试", Count: 1, Resolution: "2K", AspectRatio: "1:1", Quality: "standard"})
	if err != nil || calls.Load() != 1 || len(result.Images) != 1 || result.ProviderRequestID != "or-test-1" {
...
func TestOpenRouterImageAdapterZeroRetryAndUnknown(t *testing.T) {
	...
	adapter, err := newOpenRouterImageAdapter(OpenRouterImageAdapterConfig{APIKey: "fake-openrouter-key-123456", ProviderTag: "seed", Timeout: time.Second, ModelMap: map[string]string{"provider/image": "provider/image"}}, "https://test.invalid/images", client, true)
	if err != nil {
		t.Fatal(err)
	}
...
func TestOpenRouterImageAdapterRejectsUnsafeConfigurationAndBody(t *testing.T) {
	if _, err := NewOpenRouterImageAdapter(OpenRouterImageAdapterConfig{APIKey: "short", ProviderTag: "seed", Timeout: time.Second}); err == nil {
		t.Fatal("弱Key配置必须拒绝")
	}
	if _, err := newOpenRouterImageAdapter(OpenRouterImageAdapterConfig{APIKey: "fake-openrouter-key-123456", ProviderTag: "seed", Timeout: time.Second}, "http://127.0.0.1/images", http.DefaultClient, false); err == nil {
	...
	defer server.Close()
	adapter, _ := newOpenRouterImageAdapter(OpenRouterImageAdapterConfig{APIKey: "fake-openrouter-key-123456", ProviderTag: "seed", Timeout: time.Second, ModelMap: map[string]string{"provider/image": "provider/image"}}, server.URL, server.Client(), true)
	result, err := adapter.Generate(context.Background(), ProviderImageRequest{RequestID: "req-1", ModelCode: "provider/image", Prompt: "测试", Count: 1})

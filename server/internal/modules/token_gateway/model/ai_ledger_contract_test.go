...
func TestAILedgerTableNamesAndOrthogonalStates(t *testing.T) {
	if (AIProject{}).TableName() != "ai_projects" ||
		...
		t.Fatal("AI 网关 G1 领域模型必须与 000058 Expand Migration 表名一致")
	}
...
func TestAIGatewayMigration000058Contract(t *testing.T) {
	_, currentFile, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("无法定位 AI 账本契约测试文件")
	}
	migrationsDir := filepath.Clean(filepath.Join(filepath.Dir(currentFile), "..", "..", "..", "..", "migrations"))
	up := readMigrationForTest(t, filepath.Join(migrationsDir, "000058_create_ai_gateway_ledger_expand.up.sql"))
	down := readMigrationForTest(t, filepath.Join(migrationsDir, "000058_create_ai_gateway_ledger_expand.down.sql"))
	...
	for _, required := range []string{
		...
		if !strings.Contains(up, required) {
			t.Fatalf("000058 Expand Migration 缺少契约片段: %s", required)
		}
	}
	...
	for _, destructive := range []string{"drop table", "delete from", "truncate table"} {
		if strings.Contains(lowerDown, destructive) {
			t.Fatalf("000058 回滚必须保留审计账本，禁止破坏性语句: %s", destructive)
		}
	}
	if !strings.Contains(down, "ai_gateway_expand_schema_retained") {
		t.Fatal("000058 down 必须显式声明保留 Expand Schema")
	}
}

...
type ProductPrice struct {
	...
	// role_id / membership_level_id 不加 omitempty：未配置时稳定输出 null，
	// 供前端区分价格层级（默认价 role_id=null 且 membership_level_id=null > 角色价 > 会员价）。
	RoleID            *uint64         `gorm:"index" json:"role_id"`
	MembershipLevelID *uint64         `gorm:"index" json:"membership_level_id"`

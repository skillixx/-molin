...
// visible_scope 取值（本期实现 all/groups/roles；members/users 预留拒绝）。
const (
	scopeAll    = "all"
	scopeGroups = "groups"
	scopeRoles  = "roles"
...
// 由 bootstrap 注入（查 user_group_members / user_groups）；nil 时定向 groups 的 Agent 一律判为不可见（fail-safe）。
...
// RoleResolver 解析用户的全局 IAM 角色 code 列表，并校验角色存在性（复用 content 同款 iam 适配器）。
// nil 时定向 roles 的 Agent 一律判为不可见（fail-safe）。
...
// parseTargetAudience 解析 agents.target_audience_json 原文（nil/空 → 空结构）。
...
// visibilityInput 写入侧的定向参数（从 admin create/update / PUT visibility DTO 收集）。
...
// agentVisibleTo 判定官方 Agent 对用户 U 是否可见（统一供列表/详情/编排三处调用）。
//
// 调用方负责处理「本人自建恒可见」与「官方 active」前置；本函数只判 scope 维度：
//   - all                  → 可见
//   - groups（含组内角色）  → 用户属任一目标分组（group_roles 非空时还须组内角色命中）
//   - roles                → 用户全局角色 ∩ role_codes ≠ ∅
//   - 其他（members/users/未知） → 不可见
//
// fail-safe：resolver 为 nil 或出错时，groups/roles 一律不可见（绝不泄漏定向 Agent）。
func agentVisibleTo(ctx context.Context, a *model.Agent, userID uint64, gr GroupResolver, rr RoleResolver) bool {
	switch a.VisibleScope {
	case "", scopeAll:
		return true
	case scopeGroups:
		return groupHit(ctx, a, userID, gr)
	case scopeRoles:
		return roleHit(ctx, a, userID, rr)
	default:
		// members/users 预留未实现，或脏数据 → 不可见。
		return false
	}
}

// groupHit 判定用户是否命中 groups 定向（分组 + 可选组内角色）。
func groupHit(ctx context.Context, a *model.Agent, userID uint64, gr GroupResolver) bool {
	if gr == nil {
		return false // fail-safe
	}
	ta, err := parseTargetAudience(a.TargetAudience)
	...
	for _, gr := range ta.GroupRoles {
		roleFilter[gr] = struct{}{}
	}
...
func roleHit(ctx context.Context, a *model.Agent, userID uint64, rr RoleResolver) bool {
	if rr == nil {
		return false // fail-safe
	}
	ta, err := parseTargetAudience(a.TargetAudience)

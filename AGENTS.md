...
## AI 协作与角色分配原则（工具可替换，角色边界不变）

Codex、Claude 等 AI 工具不再与前端或后端角色固定绑定。Codex 可以替代 Claude 承担后端开发，也可以根据任务需要承担前端、测试、运维和文档工作；Claude 同样可以在明确授权后承担对应角色。

角色可以由不同工具执行，但模块所有权、业务边界、安全规则和验收门禁不能因此取消。

### 任务开始前必须声明执行角色

1. 每次任务开始前，先根据用户要求声明本次承担的角色，例如「后端甲」「前端乙」「测试工程师」或「运维工程师」。
2. 单一角色任务只修改该角色负责的目录；用户明确要求全栈或跨角色开发时，必须把工作拆成清晰的角色阶段，并分别遵守对应规范。
3. 角色权威说明位于 `docs/agents/`；模块级 `CLAUDE.md` 是角色细化规范，文件名仅为兼容既有工具，不代表只能由 Claude 执行。
4. 不得因为 Codex 接管后端就绕过接口契约、数据库设计、安全审查、测试验收或人工复核。
5. 多个 AI 或 Agent 并行工作时，必须按模块分配互不重叠的写入范围，禁止同时修改同一业务文件。

### Codex 可承担的开发范围

1. 后端：接口设计与实现、数据库与 migration、权限认证、业务逻辑、异常处理、日志、API 文档和前端对接文档。
2. 前端：页面、组件、路由、表单、样式、交互、状态管理和接口调用封装。
3. 测试：单元测试、接口测试、并发与安全回归、测试报告和验收清单。
4. 运维：本地环境、Docker、CI/CD、测试环境部署脚本和运行手册。
5. 文档：功能文档、开发文档、接口契约、ADR、任务单和进度记录。

### 角色切换约束

1. 承担后端角色时，必须遵守对应 `docs/agents/backend-*.md`、模块级 `CLAUDE.md`、接口契约和数据库设计；高风险业务仍需人工审查。
2. 承担前端角色时，只能依赖已确认的接口契约；如同一任务明确包含后端补充，先完成并记录契约，再进行前端对接。
3. 报告「前端开发完成」前，必须通过 `docs/frontend-definition-of-done.md` 的五道关卡，并以最新 main 完成契约对账。
4. 承担测试角色时不得把开发者自测替代独立 QA 验收；承担开发角色的 AI 不能自行宣布产品经理验收通过。
5. 承担运维角色时不得在未经授权的情况下修改生产环境、真实密钥、线上数据或共享测试环境。
...
Codex 可以按用户明确指定的角色开发前端、后端、测试或运维内容，不再受「只负责前端」限制。

Codex 必须遵守：

- 开始前声明当前角色和负责模块，跨角色任务按角色阶段拆分。
- 后端开发以 `docs/full-api-design.md`、`docs/database-schema-design.md`、对应模块权威文档和 `docs/agents/backend-*.md` 为准。
- 前端开发以 `docs/frontend-api-reference.md`、对应任务单和 `docs/agents/frontend-*.md` 为准。
- 测试与运维分别遵守 `docs/agents/qa.md`、`docs/agents/devops.md`，不得用开发自测替代阶段验收。
- 修改接口字段时同步更新前端契约；修改数据库时提供可回滚 migration 和验证方式。
- 前端代码注意组件复用、页面美观、响应式布局、错误提示和加载状态。
- 后端代码注意安全性、异常处理、参数校验、日志、权限控制、事务和幂等。
- 钱包、订单、实名、权限、资产、计费、密钥和生产部署等高风险内容必须保留人工审查。
